// ============================================================
//  04c-lyrics.js  (2.5d · classic script, extracted from legacy-music.js)
// ============================================================
//  Owns the 3D stage lyric renderer (stageLyrics), desktop lyrics window,
//  lyric glow/particle palette, shader/mask/glow textures and lyric mesh
//  builders, plus lyric-layout save/restore and control-panel read/write.
//
//  LOAD ORDER (hard invariant):
//    AFTER 04a-three-host.js  (uses host scene/camera/renderer globals)
//    AFTER 04b-shelf-3d.js
//    BEFORE 08-fx-visual.js   (08 MUST stay last; its top-level scene.add
//                               needs host scene, and lyrics must not
//                               top-level-reference 08 globals).
//  Runtime references to 08 uniforms (e.g. via updateStageLyrics3D called
//  from the deferred animate loop) resolve fine because all classic scripts
//  have executed before DOMContentLoaded fires.
//
//  This file references legacy globals (lyricsLines, customLyricMap,
//  readCustomLyricMap, scene/camera/renderer, getCoverDepthCache, ...) via
//  closure; they are defined in legacy (loaded first) or 04a (host).
// ============================================================
'use strict';
var stageLyrics = {
  group: null,
...
// normalizeCoverResolution: used at runtime by legacy (readUserFxArchives, now deferred to
// DOMContentLoaded so there is no top-level load-order hazard) and by 04c/08. Defined here (04c
// loads after legacy) is safe because the legacy read runs only after all classic scripts load.
...
// === [03-lyrics · 布局/字体/调色板/着色器块] 区域横幅（2026-08-07 标订；2.3 仅标记边界，不搬迁） ===
...
function saveLyricLayout() {
  try {
    ...
    localStorage.setItem(LYRIC_LAYOUT_STORE_KEY, JSON.stringify({
      ...
      lyricNextLine: fx.lyricNextLine !== false,
      cinema: !!fx.cinema,
      bloom: !!fx.bloom,
      edge: !!fx.edge,
      ...
      shapeMaterialMedia: normalizeShapeMaterialMedia(fx.shapeMaterialMedia),
...
function normalizeShapeMaterialMedia(value) {
  var media = normalizeCustomBackgroundMedia(value);
  return media && media.type === 'image' ? media : null;
}
var shapeMaterialCanvas = null;
var shapeMaterialLoadingSrc = '';
var shapeMaterialLoadToken = 0;
function currentShapeMaterialMedia() {
  return normalizeShapeMaterialMedia(fx.shapeMaterialMedia);
}
function shapeMaterialMediaLabel(media) {
  media = normalizeShapeMaterialMedia(media);
  return media ? '上传图片底片' : '当前歌曲封面';
}
function loadShapeMaterialCanvas(media, silent) {
  media = normalizeShapeMaterialMedia(media);
  var src = media && media.type === 'image' ? media.src : '';
  var token = ++shapeMaterialLoadToken;
  if (!src) {
    shapeMaterialCanvas = null;
    shapeMaterialLoadingSrc = '';
    syncCustomShapeLayer(true);
    return;
  }
  if (shapeMaterialCanvas && shapeMaterialCanvas._shapeMaterialSrc === src) return;
  if (shapeMaterialLoadingSrc === src) return;
  shapeMaterialLoadingSrc = src;
  var img = new Image();
  img.onload = function() {
    if (token !== shapeMaterialLoadToken) return;
    var maxSide = 1200;
    ...
    cv._shapeMaterialSrc = src;
    shapeMaterialCanvas = cv;
    shapeMaterialLoadingSrc = '';
    syncCustomShapeLayer(true);
    if (!silent) showToast('形态素材已应用，不会铺满整屏背景');
  };
  img.onerror = function() {
    if (token !== shapeMaterialLoadToken) return;
    shapeMaterialCanvas = null;
    shapeMaterialLoadingSrc = '';
    showToast('形态素材读取失败，已保留当前歌曲封面');
    syncCustomShapeLayer(true);
  };
  img.src = src;
}
function ensureShapeMaterialCanvas() {
  var media = currentShapeMaterialMedia();
  var src = media && media.type === 'image' ? media.src : '';
  if (!src) return null;
  if (shapeMaterialCanvas && shapeMaterialCanvas._shapeMaterialSrc === src) return shapeMaterialCanvas;
  loadShapeMaterialCanvas(media, true);
  return null;
}
function setShapeMaterialMedia(media, silent) {
  media = normalizeShapeMaterialMedia(media);
  fx.shapeMaterialMedia = media;
  refreshCreativeSandboxBuilder();
  saveLyricLayout();
  loadShapeMaterialCanvas(media, silent);
  if (!media && !silent) showToast('形态素材已切回当前歌曲封面');
}
function clearShapeMaterialMedia(silent) {
  setShapeMaterialMedia(null, silent);
}
function readShapeMaterialImageFile(file) {
  if (!file || !/^image\//i.test(file.type || '')) {
    showToast('形态素材目前请选择图片文件');
    return;
  }
  ...
  reader.onload = function(e) {
    ...
    img.onload = function() {
      ...
      var out = '';
      try { out = cv.toDataURL('image/webp', 0.86); } catch (err) {}
      if (!/^data:image\/webp/i.test(out)) {
        try { out = cv.toDataURL('image/jpeg', 0.88); } catch (err2) { out = String(e.target.result || ''); }
      }
      setShapeMaterialMedia({ type: 'image', src: out });
    };
    img.onerror = function(){ showToast('形态素材读取失败'); };
    img.src = e.target.result;
  };
  reader.onerror = function(){ showToast('形态素材读取失败'); };
  reader.readAsDataURL(file);
}
var CUSTOM_BG_DB_NAME = 'stellaflix-custom-background-v1';
...
function buildNextLyricMesh(text) {
  text = String(text || '').replace(/\s+/g, ' ').trim();
  if (!text) return null;
  var mask = makeLyricMask(text);
  var worldW = 6.10;
  ...
  var material = new THREE.MeshBasicMaterial({
    map: mask.texture,
    color: lyricThreeColor(stageLyrics.palette.secondary || stageLyrics.palette.primary, '#a9b8c8', 0.36),
    transparent: true,
    opacity: 0,
    depthWrite: false,
    depthTest: false,
  ...
  var plane = new THREE.Mesh(new THREE.PlaneGeometry(worldW, worldH, 1, 1), material);
  ...
  group.add(plane);
  group.userData.age = 0;
  group.userData.lyricNext = {
    material: material,
    worldH: worldH,
    textWorldH: worldH * ((mask.textHeight || mask.fontSize) / mask.height)
  };
  return group;
}
...
function showStageNextLine(text, redrawOnly) {
  createLyricsParticles();
  if (!stageLyrics.group) return;
  text = String(text || '').replace(/\s+/g, ' ').trim();
  if (!text) {
  ...
  if (stageLyrics.next) disposeLyricMesh(stageLyrics.next);
  stageLyrics.nextText = text;
  var mesh = buildNextLyricMesh(text);
  if (!mesh) return;
  if (stageLyrics.current) {
    var currentData = stageLyrics.current.userData ? stageLyrics.current.userData.lyric : null;
    var nextData = mesh.userData ? mesh.userData.lyricNext : null;
    var currentTextH = currentData ? (currentData.textWorldH || currentData.worldH || 0.62) : 0.62;
    var nextTextH = nextData ? (nextData.textWorldH || nextData.worldH || 0.52) : 0.52;
    var lineGap = clampRange(currentTextH * 0.50 + nextTextH * 0.29 + 0.16, 0.54, 1.10);
    mesh.position.set(stageLyrics.current.position.x, stageLyrics.current.position.y - lineGap, stageLyrics.current.position.z - 0.020);
    mesh.rotation.z = stageLyrics.current.rotation.z;
  }
  stageLyrics.group.add(mesh);
  stageLyrics.next = mesh;
}

function clearStageNextLyric() {
...
function refreshCurrentLyricStyle() {
  ...
  if (stageLyrics.nextText) showStageNextLine(stageLyrics.nextText, true);
}
...
function updateStageLyrics3D(dt) {
  ...
  function tickNextLyricMesh(mesh) {
    if (!mesh || !mesh.userData || !mesh.userData.lyricNext) return;
    var data = mesh.userData.lyricNext;
    mesh.userData.age += dt;
    var anchor = stageLyrics.current;
    var currentData = anchor && anchor.userData ? anchor.userData.lyric : null;
    var currentTextH = currentData ? (currentData.textWorldH || currentData.worldH || 0.62) : 0.62;
    var nextTextH = data.textWorldH || data.worldH || 0.52;
    var lineGap = clampRange(currentTextH * 0.50 + nextTextH * 0.29 + 0.16, 0.54, 1.10);
    var targetOpacity = fx.lyricNextLine !== false && anchor
      ? shelfDetailLyricProfile.opacity * 0.44
      : 0;
    data.material.opacity += (targetOpacity - data.material.opacity) * (targetOpacity > data.material.opacity ? 0.18 : 0.26);
    data.material.color.copy(lyricThreeColor(stageLyrics.palette.secondary || stageLyrics.palette.primary, '#a9b8c8', 0.36));
    if (!anchor) return;
    var nextScale = skullMouthLyrics ? 0.50 : 0.58;
    mesh.scale.setScalar(nextScale);
    mesh.position.x += (anchor.position.x - mesh.position.x) * 0.24;
    mesh.position.y += (anchor.position.y - lineGap - mesh.position.y) * 0.24;
    mesh.position.z += (anchor.position.z - 0.020 - mesh.position.z) * 0.24;
    mesh.rotation.z += (anchor.rotation.z - mesh.rotation.z) * 0.22;
  }
  tickNextLyricMesh(stageLyrics.next);
...
function selectCurrentLyricWindow() {
  if (!window.MineradioLyricTimeline || typeof MineradioLyricTimeline.selectLyricWindow !== 'function') return null;
  return MineradioLyricTimeline.selectLyricWindow(lyricsLines, audio ? audio.currentTime : 0, {
    introText: currentLyricFallbackText(),
    nextEnabled: fx.lyricNextLine !== false,
    duration: audio && Number.isFinite(audio.duration) ? audio.duration : 0,
    isPlaceholder: isNoLyricText
  });
}

function tickLyricsParticles() {
  if (!fx.particleLyrics) {
    if (stageLyrics.current || stageLyrics.next || stageLyrics.currentText || (stageLyrics.outgoing && stageLyrics.outgoing.length)) clearStageLyrics();
    return;
  }
  if (!audio) {
    clearStageLyrics();
  ...
  if (!playing && currentIdx < 0 && !stageLyrics.currentText) {
    clearStageLyrics();
    return;
  }
  var windowState = selectCurrentLyricWindow();
  if (!windowState || !windowState.current) {
    clearStageLyrics();
    return;
  }
  var currentText = windowState.current.text || '';
  if (windowState.currentIndex !== stageLyrics.currentIdx || currentText !== stageLyrics.currentText) {
    stageLyrics.currentIdx = windowState.currentIndex;
    showStageLine(currentText);
  }
  if (stageLyrics.current) updateLyricMeshProgress(stageLyrics.current, windowState.progress);
  var nextText = windowState.next ? (windowState.next.text || '') : '';
  if (!nextText) {
    clearStageNextLyric();
  } else if (windowState.nextIndex !== stageLyrics.nextIdx || nextText !== stageLyrics.nextText) {
    stageLyrics.nextIdx = windowState.nextIndex;
    showStageNextLine(nextText);
  }
}
