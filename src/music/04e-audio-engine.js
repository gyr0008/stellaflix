...
//  01-part2: 以下单元已迁自 legacy-music.js 区块 [音频上下文 & 频谱分析]。
//  加载顺序：04a → 04b → 04c → 04d → 04e → 08（04e 在 legacy(music.js) 之后加载）。
//  共享状态(audio/audioCtx/analyser/beatAnalyser/gainNode/audioReady 及 equalizer*/frequencyData 等)
//  仍声明于 legacy 顶层，本模块经全局作用域闭包读写；legacy 运行时调用点经全局解析。
// ============================================================
...
function initAudio() {
  ...
  var candidateGraph = null;
  try {
    candidateGraph = createEqualizerAudioGraph();
    analyser.connect(candidateGraph.filters[0]);
    candidateGraph.limiter.connect(gainNode);
    equalizerFilters = candidateGraph.filters;
    equalizerHeadroom = candidateGraph.headroom;
    equalizerLimiter = candidateGraph.limiter;
    equalizerAudioSupported = true;
  } catch (equalizerError) {
    disconnectEqualizerAudioGraph(candidateGraph);
...
function primeManualAudioPlayback() {
  try {
    if (!audio) {
      audio = new Audio();
    ...
    audio.loop = true;
    audio.preload = 'auto';
    if (audio.src !== MANUAL_AUDIO_UNLOCK_SRC) audio.src = MANUAL_AUDIO_UNLOCK_SRC;
    try { audio.currentTime = 0; } catch (_) {}
    manualAudioUnlockActive = true;
...
function releaseManualAudioPlaybackPrime() {
  manualAudioUnlockActive = false;
  try { if (audio) audio.loop = false; } catch (e) {}
}
...
function bindVolumeControls() {
  ...
  if (slider) {
    slider.addEventListener('input', function(){
      if (window.StellaflixVideo && StellaflixVideo.player && StellaflixVideo.player.isOpen && StellaflixVideo.player.isOpen()) { StellaflixVideo.player.setVolume(parseFloat(slider.value)); return; }
      setVolume(slider.value, true);
    });
