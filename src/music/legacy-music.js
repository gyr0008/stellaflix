...
// 2.5b-regression-fix: declare fx early so legacy/host TOP-LEVEL sync init does not
// throw ReferenceError before 08-fx-visual.js (loaded LAST) defines the real fx object.
// 08 reassigns the real fx (Object.assign({}, fxDefaults, readSavedLyricLayout())) later;
// between now and then fx is a safe empty object, so `fx && fx.x` guards pass harmlessly
// and `fx.x = y` writes succeed. Without this, host init (`fx && fx.performanceQuality`)
// and legacy updateCamera() throw `fx is not defined` because classic-script `var` only
// hoists within its own file's parse — 08's `var fx` is not visible during legacy/host load.
var fx = {};

// 2.5b-regression-fix: forward-declare fxDefaults (08-fx-visual.js owns the real object,
// loaded LAST) with a safe minimal default. legacy normalizePerformanceQuality() references
// fxDefaults.performanceQuality and is reachable from host top-level init (renderer pixel
// ratio) before 08 loads; without this it throws `fxDefaults is not defined`. 08 reassigns
// the full real fxDefaults later (same default performanceQuality), so no behavior drift.
var fxDefaults = { performanceQuality: 'high' };

// 2.5b-regression-fix: forward-declare 08-owned globals referenced by legacy TOP-LEVEL init
// (createPackagedDefaultUserFxArchiveSlot at legacy:17545 calls clonePackagedDefaultFxSnapshot()
// and reads PACKAGED_DEFAULT_*). These are defined in 08-fx-visual.js (loaded LAST), so referencing
// them at legacy top-level would throw ReferenceError. 08 reassigns the real values/functions later.
var PACKAGED_DEFAULT_USER_FX_ARCHIVE_NAME = '';
var PACKAGED_DEFAULT_USER_FX_ARCHIVE_EXPORTED_AT = 0;
var PACKAGED_DEFAULT_USER_FX_ARCHIVE_SAVED_AT = 0;
function clonePackagedDefaultFxSnapshot() { return {}; }

// 2.5b-regression-fix: these are assigned inside runStartupSequence() (deferred to
// DOMContentLoaded). Declared at top level so the globals-verification gate stays green
// (top-level declarations must not be silently dropped/renamed by esbuild).
var startupLoginStatusPromise, collectNameInput, customLyricInput;

// 2.5d-regression-fix: clampRange was swept into 04c-lyrics.js during the lyrics extraction,
// but it is a GENERAL utility (used ~107x by legacy shape-recipe / performance code, including
// the top-level normalizeShapeRecipe() call chain). Because legacy loads BEFORE 04c, legacy
// top-level cannot reach a 04c-owned clampRange. Define it here (base module, loaded first) so
// both legacy and 04c resolve it via closure. Do NOT keep a duplicate definition in 04c.
...
var MANUAL_AUDIO_UNLOCK_SRC = 'data:audio/wav;base64,UklGRigAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQQAAAAAgICA';
var manualAudioUnlockActive = false;
...
var kugouLoginStatus = { provider: 'kugou', platform: 'lite', loggedIn: false, nickname: '酷狗概念版', userId: '', avatar: '', vipType: 0, tokenReady: false, deviceReady: false };
var kugouMusicLoginStatus = { provider: 'kugouMusic', platform: 'music', loggedIn: false, nickname: '酷狗音乐', userId: '', avatar: '', vipType: 0, tokenReady: false, deviceReady: false };
var qqLoginAutoRefreshTimer = null;
var kugouLoginAutoRefreshTimer = null;
var kugouMusicLoginAutoRefreshTimer = null;
var qqLoginWasLoggedIn = false;
var kugouLoginWasLoggedIn = false;
var kugouMusicLoginWasLoggedIn = false;
...
var userPlaylists = [], qqPlaylists = [], kugouPlaylists = [], kugouMusicPlaylists = [], qishuiImportedPlaylists = [], myPodcastCollections = [], myPodcastItems = {}, playlistCoverCache = {};
var CUSTOM_COVER_STORE_KEY = 'stellaflix-custom-covers';
var CUSTOM_LYRIC_STORE_KEY = 'stellaflix-custom-lyrics-v1';
var CUSTOM_LYRIC_PREF_STORE_KEY = 'stellaflix-custom-lyric-prefs-v1';
var LYRIC_LAYOUT_STORE_KEY = 'stellaflix-lyric-layout-v1';
var VISUAL_PRESET_SCHEMA = 'skull-preset-v2';
var PLAYBACK_QUALITY_STORE_KEY = 'stellaflix-playback-quality-v1';
var UPLOAD_TIP_STORE_KEY = 'stellaflix-upload-tip-seen';
var DIY_MODE_STORE_KEY = 'stellaflix-diy-player-mode-v1';
var PLAYLIST_PANEL_PIN_STORE_KEY = 'stellaflix-playlist-panel-pinned-v1';
var USER_CAPSULE_AUTO_HIDE_STORE_KEY = 'stellaflix-user-capsule-auto-hide-v1';
var FX_FAB_AUTO_HIDE_STORE_KEY = 'stellaflix-fx-fab-auto-hide-v1';
var CONTROLS_AUTO_HIDE_STORE_KEY = 'stellaflix-controls-auto-hide-v1';
var FREE_CAMERA_STORE_KEY = 'stellaflix-free-camera-v1';
var HOTKEY_SETTINGS_STORE_KEY = 'stellaflix-hotkey-settings-v1';
var VISUAL_GUIDE_SEEN_STORE_KEY = 'stellaflix-visual-guide-seen-v2';
var LOCAL_BEATMAP_STORE_KEY = 'stellaflix-local-beatmaps-v1';
var LOCAL_BEAT_PREF_STORE_KEY = 'stellaflix-local-beatmap-prefs-v1';
var QISHUI_IMPORTED_PLAYLIST_STORE_KEY = 'stellaflix-qishui-imported-playlists-v1';
...
var collectTargetSong = null, collectTargetProvider = 'netease', collectBusy = false;
...
var HOME_LISTEN_STATS_KEY = 'stellaflix-listen-stats-v1';
var HOME_POSTER_STORE_KEY = 'stellaflix-home-personal-poster-v1';
var homePosterState = readHomePosterState();
var PLAYLIST_DETAIL_SORT_STORE_KEY = 'stellaflix-playlist-detail-sort-v1';
var PLAYLIST_DETAIL_SORT_OPTIONS = [
  { key: 'saved', label: '收藏时间' },
  { key: 'artist', label: '歌手名' },
  { key: 'plays', label: '本地播放次数' },
  { key: 'platformPlays', label: '平台播放次数' },
];
var playlistDetailSortMode = loadPlaylistDetailSortMode();
var playlistPlatformPlayCountState = {
  kugou: { loaded: false, loading: false, error: '', counts: {}, promise: null },
  kugouMusic: { loaded: false, loading: false, error: '', counts: {}, promise: null },
};
var HOME_WEATHER_CITY_KEY = 'stellaflix-weather-city';
...
function markAppPerf(name) {
  try {
    ...
    if (appPerfMarks.length <= 40) console.debug('[MineradioPerf]', name, Math.round(value) + 'ms');
  } catch (e) {}
}
...
function readHomePosterState() {
  try {
    var raw = localStorage.getItem(HOME_POSTER_STORE_KEY);
    var parsed = raw ? JSON.parse(raw) : {};
    var base = defaultHomePosterState();
    return {
      image: String(parsed && parsed.image || '').slice(0, 2800000),
      quote: String(parsed && parsed.quote || base.quote).trim().slice(0, 80) || base.quote,
    };
  } catch (e) {
    return defaultHomePosterState();
  }
}
...
function loadPlaylistDetailSortMode() {
  try {
    var mode = localStorage.getItem(PLAYLIST_DETAIL_SORT_STORE_KEY);
    return isPlaylistDetailSortMode(mode) ? mode : 'saved';
  } catch (e) {
    return 'saved';
  }
}
...
// 2.5b-regression-fix: defer the initial camera positioning until after host (04a-three-host.js)
// and 08-fx-visual.js have loaded. updateCamera() calls applyFreeCameraToCamera() (host) and
// reads fx (08), neither of which exists at legacy top-level load time. DOMContentLoaded fires
// after ALL classic scripts (music → globals-bridge → host → 08) have executed, so both are
// available. animate() also calls updateCamera() every frame, so the initial posture is safe
// to defer. (In the Electron app scripts sit at end of <body>, so readyState is 'loading' here.)
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function () { updateCamera(); });
} else {
  updateCamera();
}
...
function setControlsHidden(hidden) {
  ...
  if (!bar) return;
  if (hidden && performance.now() < controlsRevealGuardUntil) hidden = false;
...
function revealBottomControls(delay) {
  controlsRevealGuardUntil = performance.now() + 340;
...
// ============================================================
// [moved -> 08-fx-visual.js · region B]
// ============================================================
//  舞台歌词系统 v9 — Three.js 文字平面, 跟随专辑粒子 3D 运动
// ============================================================
// === [03-lyrics · 舞台 3D 歌词块] 区域横幅（2026-08-07 标订；2.3 仅标记边界，不搬迁） ===

// 2.5d: 歌词渲染模块已移至 src/music/04c-lyrics.js（classic script，加载于 04b 之后、08 之前）。
//      本区块仅保留顶部小状态 / 相机锁定助手 / localStorage 读取器（顶层引用，迁移风险高）。

// [moved -> 08-fx-visual.js · region C]
...
function applyCoverCanvas(cv, thumbSrc, opts) {
  ...
  function refreshCoverDependentColors() {
    ...
    refreshCustomShapeColorsFromCover();
  }
...
// ============================================================
//  [04d-audio-beat · tempo 加载器 + Worker URL 构建器]
//  01-part1: 以下单元已迁至 src/music/04d-audio-beat.js（classic script，04c 之后、08 之前加载）。
//  仅保留标记；运行时符号经全局作用域由 legacy analyzeMusicTempoInWorker 闭包引用。
// ============================================================
...
function beatMapSongKey(song) {
  ...
  if (songProviderKey(song) === 'kugou') return 'kugou:' + (song.hash || song.albumAudioId || song.album_audio_id || song.id || (song.name + '|' + song.artist));
  if (songProviderKey(song) === 'kugouMusic') return 'kugouMusic:' + (song.hash || song.albumAudioId || song.album_audio_id || song.id || (song.name + '|' + song.artist));
  if (songProviderKey(song) === 'qishui') return 'qishui:' + (song.qishuiTrackId || song.id || (song.name + '|' + song.artist));
...
function kugouSongUrlApiPath(song, qualityParam) {
  song = song || {};
  var hash = song.hash || song.FileHash || song.fileHash || song.id || '';
  var albumId = song.albumId || song.album_id || '';
  var albumAudioId = song.albumAudioId || song.album_audio_id || song.mixSongId || song.MixSongID || '';
  return '/api/kugou/song/url?hash=' + encodeURIComponent(hash) +
    '&albumId=' + encodeURIComponent(albumId) +
    '&albumAudioId=' + encodeURIComponent(albumAudioId) +
    (qualityParam || '');
}
function kugouMusicSongUrlApiPath(song, qualityParam) {
  song = song || {};
  var hash = song.hash || song.FileHash || song.fileHash || song.id || '';
  var albumId = song.albumId || song.album_id || '';
  var albumAudioId = song.albumAudioId || song.album_audio_id || song.mixSongId || song.MixSongID || '';
  return '/api/kugou-music/song/url?hash=' + encodeURIComponent(hash) +
    '&albumId=' + encodeURIComponent(albumId) +
    '&albumAudioId=' + encodeURIComponent(albumAudioId) +
    (qualityParam || '');
}
...
async function fetchBeatPrefetchAudioUrl(song) {
  ...
  var isKugou = provider === 'kugou' || provider === 'kugouMusic';
  var requestedQuality = normalizePlaybackQuality(playbackQuality);
  requestedQuality = providerPlaybackQualityValue(provider, requestedQuality);
  if (!isQQ && !isKugou && requestedQuality === 'jymaster' && !hasProviderSvip('netease', loginStatus)) requestedQuality = 'hires';
  ...
  var data = isQQ
    ...
    : (isKugou ? await apiJson(provider === 'kugouMusic' ? kugouMusicSongUrlApiPath(song, qualityParam) : kugouSongUrlApiPath(song, qualityParam)) : await apiJson('/api/song/url?id=' + encodeURIComponent(song.id) + qualityParam));
...
// 2.5c: 3D 歌单架模块已移至 src/music/04b-shelf-3d.js（classic script，加载于 04a 之后、08 之前）。
...
// 2.5c: the blur/mouseleave/mouseout listeners that call clearShelfPreviewOnPointerExit have
// been MOVED into 04b-shelf-3d.js (immediately after that function's definition). clearShelfPreviewOnPointerExit
// is now 04b-owned (loaded AFTER legacy), so registering the listeners here at legacy top-level
// would capture a non-existent/stale value. Do NOT re-add them here.
...
function makeContentListManager() {
  ...
  return {
    ...
    open: async function(playlistId, title, fromCard) {
      ...
      var kugouMusicPlaylistId = String(playlistId || '').indexOf('kugouMusic:') === 0 ? String(playlistId).slice(11) : '';
      ...
      try {
        r = podcastCollectionKey
          ...
          : (qqPlaylistId
            ...
            : (kugouMusicPlaylistId
              ? await apiJson('/api/kugou-music/playlist/tracks?id=' + encodeURIComponent(kugouMusicPlaylistId))
              : (kugouPlaylistId
      ...
      try {
        ...
        var detailProvider = qqPlaylistId ? 'qq' : (kugouMusicPlaylistId ? 'kugouMusic' : (kugouPlaylistId ? 'kugou' : 'netease'));
        allTracks = podcastCollectionKey
          ? tracks
          : sortPlaylistTracksForDisplay(tracks.map(normalizePlaylistPanelTrack), detailProvider);
...
function isKugouQualityProvider(provider) {
  return provider === 'kugou' || provider === 'kugouMusic';
}
function providerPlaybackQualityValue(provider, value) {
  value = normalizePlaybackQuality(value);
  if (isKugouQualityProvider(provider) && value === 'jymaster') return 'hires';
  return value;
}
function providerAvailablePlaybackQualities(provider) {
  return isKugouQualityProvider(provider)
    ? ['hires', 'lossless', 'exhigh', 'standard']
    : ['jymaster', 'hires', 'lossless', 'exhigh', 'standard'];
}
function providerQualityOptionMeta(provider, value) {
  provider = provider || 'netease';
  value = normalizePlaybackQuality(value);
  var kugouLike = isKugouQualityProvider(provider);
  if (kugouLike) {
    if (value === 'jymaster' || value === 'hires') return { label: 'Hi-Res音质', short: 'Hi-Res', sub: '超越CD品质，重塑发烧经典' };
    if (value === 'lossless') return { label: '无损音质', short: '无损', sub: '高保真无损品质' };
    if (value === 'exhigh') return { label: '高品音质', short: '高品', sub: '高频细节还原清晰' };
    if (value === 'standard') return { label: '标准音质', short: '标准', sub: '快捷清晰的音质' };
  }
  if (provider === 'qq') {
    if (value === 'jymaster') return { label: '臻品母带', short: '母带', sub: 'VIP / 最高规格' };
    if (value === 'hires') return { label: 'Hi-Res', short: 'Hi-Res', sub: '高解析优先' };
    if (value === 'lossless') return { label: '无损 SQ', short: 'SQ', sub: 'FLAC 优先' };
    if (value === 'exhigh') return { label: '极高 HQ', short: 'HQ', sub: '320kbps' };
    if (value === 'standard') return { label: '标准', short: 'STD', sub: '128kbps' };
  }
  if (value === 'jymaster') return { label: '超清母带', short: '母带', sub: 'SVIP / 最高规格' };
  if (value === 'hires') return { label: '高清臻音', short: '臻音', sub: '默认 / 细节优先' };
  if (value === 'lossless') return { label: '无损 SQ', short: 'SQ', sub: 'FLAC 优先' };
  if (value === 'exhigh') return { label: '极高 HQ', short: 'HQ', sub: '320kbps' };
  return { label: '标准', short: 'STD', sub: '128kbps' };
}
function playbackQualityLabelForProvider(provider, value) {
  return providerQualityOptionMeta(provider, value).label;
}
function playbackQualityShortLabelForProvider(provider, value) {
  return providerQualityOptionMeta(provider, value).short;
}
...
function playbackResolvedQualityText(data, provider) {
  data = data || {};
  var label = playbackQualityLabelForProvider(provider || data.provider || 'netease', data.level || data.quality || playbackQuality);
  var br = playbackBitrateLabel(data.br);
  return br ? (label + ' · ' + br) : label;
}
function qualityProviderForCurrentTrack() {
  var song = typeof currentCoverSong === 'function' ? currentCoverSong() : null;
  if (currentPlaybackProvider === 'netease' || currentPlaybackProvider === 'qq' || currentPlaybackProvider === 'kugou' || currentPlaybackProvider === 'kugouMusic') return currentPlaybackProvider;
  var provider = songProviderKey(song);
  if (provider === 'local' || provider === 'podcast') return '';
  if (provider === 'qishui') return alternatePlaybackProvider(song);
  if (provider === 'netease' || provider === 'qq' || provider === 'kugou' || provider === 'kugouMusic') return provider;
  if (hasPlatformLogin(activeAccountProvider)) return activeAccountProvider;
  return firstLoggedProvider();
}
function providerQualityLock(provider, quality) {
  provider = provider || 'netease';
  quality = normalizePlaybackQuality(quality);
  if (quality !== 'jymaster') return { locked: false, requirement: '' };
  if (provider === 'netease') {
    return {
      locked: !hasProviderSvip('netease', loginStatus),
      requirement: '网易云 SVIP',
      loginProvider: 'netease'
    };
  }
  if (provider === 'qq' || provider === 'kugou' || provider === 'kugouMusic') {
    var meta = platformMeta(provider);
    var ready = providerPlaybackReady(provider);
    return {
      locked: !ready,
      requirement: meta.label + (hasPlatformLogin(provider) ? '播放授权' : '账号'),
...
function updatePlaybackQualityUi() {
  ...
  var qualityProvider = qualityProviderForCurrentTrack();
  var effectiveQuality = providerPlaybackQualityValue(qualityProvider, playbackQuality);
  var gate = providerQualityLock(qualityProvider, effectiveQuality);
  var displayQuality = effectiveQuality === 'jymaster' && gate.locked ? 'hires' : effectiveQuality;
  var displayMeta = providerQualityOptionMeta(qualityProvider, displayQuality);
  var availableQualities = providerAvailablePlaybackQualities(qualityProvider);
  if (label) label.textContent = displayMeta.short;
  if (btn) btn.title = effectiveQuality === 'jymaster' && gate.locked
    ? '音质: ' + displayMeta.label + ' · ' + providerQualityOptionMeta(qualityProvider, effectiveQuality).label + '需' + gate.requirement
    : '音质: ' + displayMeta.label + ' · ' + playbackProviderLabel({ provider: qualityProvider || 'netease' });
  document.querySelectorAll('.quality-option').forEach(function(option){
    var q = normalizePlaybackQuality(option.dataset.quality);
    var available = availableQualities.indexOf(q) >= 0;
    ...
    var optionMeta = providerQualityOptionMeta(qualityProvider, q);
    var optionGate = providerQualityLock(qualityProvider, q);
    var locked = option.dataset.svip === '1' && optionGate.locked;
    ...
    option.title = locked ? (optionMeta.label + '需要' + optionGate.requirement) : optionMeta.label;
    if (span) span.textContent = optionMeta.label;
    if (small) small.textContent = optionMeta.sub;
    if (small) small.setAttribute('data-lock-suffix', locked ? (' · 需' + optionGate.requirement) : '');
  });
}
function setPlaybackQuality(value) {
  var qualityProvider = qualityProviderForCurrentTrack();
  var next = providerPlaybackQualityValue(qualityProvider, value);
  var gate = providerQualityLock(qualityProvider, next);
  if (gate.locked) {
    var optionMeta = providerQualityOptionMeta(qualityProvider, next);
    showToast(hasPlatformLogin(gate.loginProvider) ? (optionMeta.label + '需要' + gate.requirement) : ('登录' + gate.requirement + '后可用' + optionMeta.label));
    if (!hasPlatformLogin(gate.loginProvider)) openProviderLogin(gate.loginProvider);
    return;
  }
...
function canReloadCurrentTrackForQuality() {
  ...
  var provider = songProviderKey(song);
  return provider === 'netease' || provider === 'qq' || provider === 'kugou' || provider === 'kugouMusic';
}
function applyPlaybackQualityToCurrentTrack(nextQuality) {
  var qualityProvider = qualityProviderForCurrentTrack();
  var effectiveQuality = providerPlaybackQualityValue(qualityProvider, nextQuality || playbackQuality);
  var label = playbackQualityLabelForProvider(qualityProvider, effectiveQuality);
...
function songCustomCoverKey(song) {
  ...
  if (song.provider === 'kugouMusic' || song.source === 'kugouMusic' || song.type === 'kugouMusic') return 'kugouMusic:' + (song.hash || song.albumAudioId || song.album_audio_id || song.id || (song.name + '|' + song.artist));
  if (song.localKey) return 'local:' + song.localKey;
...
function defaultHomePosterState() {
  return {
    image: '',
    quote: '把喜欢的画面和一句话留在这里。',
  };
}

function saveHomePosterState() {
  try {
    localStorage.setItem(HOME_POSTER_STORE_KEY, JSON.stringify(homePosterState || defaultHomePosterState()));
    return true;
  } catch (e) {
    console.warn('home poster save failed:', e);
    return false;
  }
}
function homePosterFallbackImage() {
  var song = typeof currentCoverSong === 'function' ? currentCoverSong() : null;
  var src = songCoverSrc(song, 720);
  if (src) return src;
  var summary = typeof homeListenSummary === 'function' ? homeListenSummary() : null;
  if (summary && summary.recent) return songCoverSrc(summary.recent, 720) || summary.recent.cover || '';
  if (homeDiscoverState && homeDiscoverState.songs && homeDiscoverState.songs[0]) return songCoverSrc(homeDiscoverState.songs[0], 720);
  return '';
}
function renderHomePersonalPoster() {
  // T134-f 守卫：影视态下禁止音乐态渲染覆盖共享 DOM（#home-poster-media / #home-poster-quote）
  // safePlaybackStep('home-poster', ...) 在每次播放状态变化时调用此函数，无空间检查会抹掉影视态海报/文案
  if (document.body && document.body.classList.contains('video-space-active')) return;
  var media = document.getElementById('home-poster-media');
  var quote = document.getElementById('home-poster-quote');
  var now = document.getElementById('home-poster-now');
  var src = homePosterState && homePosterState.image || homePosterFallbackImage();
  if (media) {
    media.style.setProperty('--home-poster-image', src ? 'url("' + cssImageUrl(src) + '")' : '');
    media.classList.toggle('has-image', !!src);
  }
  if (quote) quote.textContent = (homePosterState && homePosterState.quote) || defaultHomePosterState().quote;
  if (now) {
    var song = typeof currentCoverSong === 'function' ? currentCoverSong() : null;
    if (song && (song.name || song.artist)) {
      now.textContent = '正在听 · ' + (song.name || '当前歌曲') + (song.artist ? ' / ' + song.artist : '');
    } else {
      var summary = typeof homeListenSummary === 'function' ? homeListenSummary() : null;
      now.textContent = summary && summary.recent ? ('最近播放 · ' + (summary.recent.name || '未命名歌曲')) : '播放一首歌后，这里会显示当前声音。';
    }
  }
}
function setHomePosterImage(src, toastText) {
  // T134-f Layer3：影视态下禁止音乐侧修改海报图（"用当前封面"等按钮触发）
  if (document.body && document.body.classList.contains('video-space-active')) return;
  homePosterState.image = String(src || '');
  var saved = saveHomePosterState();
  renderHomePersonalPoster();
  showToast(saved ? (toastText || '音乐海报已更新') : '音乐海报已应用，但本地存储空间不足');
}
function readHomePosterImageFile(file) {
  ...
  reader.onload = function(e) {
    ...
    img.onload = function() {
      ...
      var out = '';
      try { out = cv.toDataURL('image/webp', 0.82); } catch (err) {}
      if (!/^data:image\/webp/i.test(out)) {
        try { out = cv.toDataURL('image/jpeg', 0.84); } catch (err2) { out = String(e.target.result || ''); }
      }
      setHomePosterImage(out, '音乐海报图片已保存');
    };
    img.onerror = function(){ showToast('音乐海报图片读取失败'); };
    img.src = e.target.result;
  };
  reader.onerror = function(){ showToast('音乐海报图片读取失败'); };
  reader.readAsDataURL(file);
}
function applyHomePosterCurrentCover() {
  var song = typeof currentCoverSong === 'function' ? currentCoverSong() : null;
  var src = songCoverSrc(song, 900);
  if (!src) {
    showToast('当前没有可用封面');
    return;
  }
  setHomePosterImage(src, '已使用当前封面');
}
function editHomePosterQuote() {
  var current = homePosterState && homePosterState.quote || defaultHomePosterState().quote;
  var wrap = document.querySelector('.home-poster-copy');
  var input = document.getElementById('home-poster-quote-input');
...
function cancelHomePosterQuoteEdit() {
  var wrap = document.querySelector('.home-poster-copy');
  if (wrap) wrap.classList.remove('editing');
}
function saveHomePosterQuote() {
  // T134-f Layer5：影视态下禁止音乐态文案保存（防止遗留的保存按钮引用覆盖视频文案）
  if (document.body && document.body.classList.contains('video-space-active')) return;
  var input = document.getElementById('home-poster-quote-input');
  ...
  homePosterState.quote = next;
  var saved = saveHomePosterState();
  cancelHomePosterQuoteEdit();
  renderHomePersonalPoster();
  showToast(saved ? '音乐海报文案已保存' : '文案已应用，但本地存储空间不足');
}
function handleHomePosterQuoteKey(e) {
  if (!e) return;
  if (e.key === 'Escape') {
    e.preventDefault();
    cancelHomePosterQuoteEdit();
...
function resetHomePoster() {
  // T134-f Layer6：影视态下禁止音乐态重置海报
  if (document.body && document.body.classList.contains('video-space-active')) return;
  homePosterState = defaultHomePosterState();
  try { localStorage.removeItem(HOME_POSTER_STORE_KEY); } catch (e) {}
  cancelHomePosterQuoteEdit();
  renderHomePersonalPoster();
  showToast('音乐海报已重置');
}
...
function recordListenStart(snap) {
  if (!snap || !snap.key) return;
  var now = Date.now();
  listenStatsState.songs = listenStatsState.songs || {};
  var songStat = listenStatsState.songs[snap.key] || { key: snap.key, name: snap.name || '', artist: snap.artist || '', cover: snap.cover || '', source: snap.source || '', plays: 0, listenMs: 0, completed: 0, lastPlayedAt: 0 };
  songStat.name = snap.name || songStat.name || '';
  songStat.artist = snap.artist || songStat.artist || '';
  songStat.cover = snap.cover || songStat.cover || '';
  songStat.source = snap.source || songStat.source || '';
  songStat.starts = (Number(songStat.starts) || 0) + 1;
  songStat.lastStartedAt = now;
  listenStatsState.songs[snap.key] = songStat;
...
function finalizeListenSession(completed) {
  ...
  if (currentSong && queueItemKey(currentSong) === session.key) {
...
function pickHomeLibraryPlaylist(playlists) {
  var rows = (Array.isArray(playlists) ? playlists : []).filter(function(pl){ return pl && pl.name; });
  if (!rows.length) return null;
  var nonEmptyRows = rows.filter(function(pl){ return Math.max(0, Number(pl.trackCount) || 0) > 0; });
  var candidates = nonEmptyRows.length ? nonEmptyRows : rows;
  var likedRows = candidates.filter(function(pl){ return /我喜欢|我的喜欢|喜欢的音乐|默认收藏/i.test(String(pl.name || '')); });
  var pool = likedRows.length ? likedRows : candidates;
  return pool.slice().sort(function(a, b){
    return (Math.max(0, Number(b.trackCount) || 0) - Math.max(0, Number(a.trackCount) || 0));
  })[0] || null;
}
// 跨态连线（2026-08-05）：音乐态「影视空间」卡展示影视态左侧个人海报。
//   来源：① SFV.home.getUserVideoPoster().url（运行时 posterStore，可能尚未异步加载）
//        ② localStorage['stellaflix-video-poster'].url（兜底，与 home.js 持久键一致）
//   与音乐态 homePosterState 完全独立，互不影响。
function getVideoLeftPoster() {
  try {
    if (window.StellaflixVideo && window.StellaflixVideo.home && typeof window.StellaflixVideo.home.getUserVideoPoster === 'function') {
      var p = window.StellaflixVideo.home.getUserVideoPoster();
      if (p && p.url) return p.url;
    }
    if (window.localStorage) {
      var raw = window.localStorage.getItem('stellaflix-video-poster');
      if (raw) {
        var parsed = JSON.parse(raw);
        if (parsed && (parsed.url || parsed.poster)) return parsed.url || parsed.poster;
      }
    }
  } catch (e) { /* ignore */ }
  return '';
...
function renderHomeDiscover() {
  // 影视态下首页由 public/video/home.js 接管；双重守卫（state + body class）防止任何路径绕过
  if (window.StellaflixVideo && window.StellaflixVideo.state && window.StellaflixVideo.state.isVideo()) return;
  if (document.body && document.body.classList.contains('video-space-active')) return;
  ...
  if (weatherKicker) weatherKicker.textContent = 'Stellaflix · Your Library';
  if (sub) {
  ...
  var libraryPlaylist = pickHomeLibraryPlaylist(userPlaylists);
  ...
  // 影视空间卡（第5张）：保持静态文案，不被音乐数据覆写
  var videoTitle = document.getElementById('home-video-title');
  var videoSub = document.getElementById('home-video-sub');
  ...
  if (weatherCardSub) {
    weatherCardSub.textContent = libraryPlaylist
      ? (((Math.max(0, Number(libraryPlaylist.trackCount) || 0)) ? Math.max(0, Number(libraryPlaylist.trackCount) || 0) + ' 首 · ' : '') + (libraryPlaylist.name || '打开左侧歌单库'))
      : '打开左侧歌单库';
  }
  ...
  if (videoTitle) videoTitle.textContent = document.body.classList.contains('video-space-active') ? '音乐空间' : '影视空间';
  if (videoSub) videoSub.textContent = document.body.classList.contains('video-space-active') ? '返回音乐空间' : '搜索 / 播放影片';
  if (loggedOutHome) {
    ...
    setHomeArt('home-video-art', getVideoLeftPoster(), 280);
    ...
    setHomeArt('home-weather-art', (libraryPlaylist && libraryPlaylist.cover) || (playlistItem && playlistItem.cover) || daily && daily.cover, 280);
...
function prewarmHomeWallpaperPreview() {
  ...
  // 不在 splash 期间提前激活壁纸预览——那会触发 setPreset / 2D 粒子 / 潜在壁纸引擎
  // 初始化，在 splash 显示期占 GPU。home-revealed 时 updateEmptyHomeVisibility 会按需
  // 重新激活，因此此处提前预热无收益，只保留标志位避免重复触发。
}
...
function songSourceLabel(song) {
  ...
  if (song.provider === 'kugou' || song.source === 'kugou' || song.type === 'kugou') return '酷狗概念版';
  if (song.provider === 'kugouMusic' || song.source === 'kugouMusic' || song.type === 'kugouMusic') return '酷狗音乐';
...
function openTrackDetailModal(type, songOverride) {
  ...
  if (type === 'artist') {
    ...
    var detailIsKugou = songProviderKey(song) === 'kugou';
    var detailIsKugouMusic = songProviderKey(song) === 'kugouMusic';
    var detailKugouMixSongId = song.albumAudioId || song.album_audio_id || song.mixSongId || song.MixSongID || song.id || '';
    var detailCanLoadComments = isCloudSong(song) || detailIsQQ || ((detailIsKugou || detailIsKugouMusic) && !!detailKugouMixSongId);
    var detailCommentTitle = detailIsQQ ? 'QQ 音乐评论' : (detailIsKugouMusic ? '酷狗音乐评论' : (detailIsKugou ? '酷狗评论' : '网易云评论'));
    var detailEmptyText = detailIsQQ ? '当前 QQ 歌曲暂无评论' : (detailIsKugouMusic ? '当前酷狗音乐歌曲缺少评论 ID' : (detailIsKugou ? '当前酷狗歌曲缺少评论 ID' : '本地文件暂无网易云评论'));
    body.innerHTML =
    ...
    if (detailCanLoadComments) {
      var commentUrl = detailIsQQ
        ...
        : (detailIsKugouMusic
          ? ('/api/kugou-music/song/comments?mixsongid=' + encodeURIComponent(detailKugouMixSongId) + '&limit=18')
          : (detailIsKugou
          ? ('/api/kugou/song/comments?mixsongid=' + encodeURIComponent(detailKugouMixSongId) + '&limit=18')
...
function isCloudSong(song) {
  ...
  if (song.provider === 'kugouMusic' || song.source === 'kugouMusic' || song.type === 'kugouMusic') return false;
...
function songLikeKey(song) {
  ...
  var id = provider === 'kugou'
    ...
    : (song.id || '');
  return id ? (provider + ':' + String(id).toLowerCase()) : '';
}
...
function syncLikeStatusForSongs(songs) {
  ...
  var tasks = [];
  var neteaseSongs = songs.filter(function(s){ return isCloudSong(s) && songProviderKey(s) === 'netease'; });
  var kugouSongs = songs.filter(function(s){ return songProviderKey(s) === 'kugou' && songLikeKey(s); });
  var kugouMusicSongs = songs.filter(function(s){ return songProviderKey(s) === 'kugouMusic' && songLikeKey(s); });
  if (loginStatus.loggedIn && neteaseSongs.length) {
    var ids = neteaseSongs.map(function(s){ return String(s.id); });
    tasks.push(apiJson('/api/song/like/check?ids=' + encodeURIComponent(ids.join(','))).then(function(r){
      if (!r || !r.liked) return;
      neteaseSongs.forEach(function(song){
        // 并集语义:平台已红心或本地已红心都算喜欢,平台状态不得清除本地红心
        var key = songLikeKey(song);
        likedSongMap[key] = !!likedSongMap[key] || !!r.liked[String(song.id)];
      });
    }));
  }
  if (hasPlatformLogin('kugou') && kugouSongs.length) {
    var kugouIds = kugouSongs.map(function(s){ return String(s.hash || s.albumAudioId || s.album_audio_id || s.id || '').toLowerCase(); });
    tasks.push(apiJson('/api/kugou/song/like/check?ids=' + encodeURIComponent(kugouIds.join(','))).then(function(r){
      if (!r || !r.liked) return;
      kugouSongs.forEach(function(song, index){
        var key = songLikeKey(song);
        likedSongMap[key] = !!likedSongMap[key] || !!r.liked[kugouIds[index]];
      });
    }));
  }
  if (hasPlatformLogin('kugouMusic') && kugouMusicSongs.length) {
    var kugouMusicIds = kugouMusicSongs.map(function(s){ return String(s.hash || s.albumAudioId || s.album_audio_id || s.id || '').toLowerCase(); });
    tasks.push(apiJson('/api/kugou-music/song/like/check?ids=' + encodeURIComponent(kugouMusicIds.join(','))).then(function(r){
      if (!r || !r.liked) return;
      kugouMusicSongs.forEach(function(song, index){
        var key = songLikeKey(song);
        likedSongMap[key] = !!likedSongMap[key] || !!r.liked[kugouMusicIds[index]];
      });
    }));
  }
  ...
  Promise.all(tasks).then(function(){
    ...
    if (playlistPanelDetailState && playlistPanelDetailState.tracks && playlistPanelDetailState.tracks.length) renderPlaylistPanelDetailState();
    updateLikeButtons();
  }).catch(function(err){ console.warn('like check failed:', err); });
}
function syncLikeStatusForSong(song) {
  var provider = songProviderKey(song);
  if (!isCloudSong(song) && provider !== 'kugou' && provider !== 'kugouMusic') { updateLikeButtons(song); return; }
...
function markSongsLiked(songs, liked) {
  (songs || []).forEach(function(song){
    var provider = songProviderKey(song);
    if (isCloudSong(song) || provider === 'kugou' || provider === 'kugouMusic') {
      var key = songLikeKey(song);
...
async function toggleLikeSong(song) {
  var provider = songProviderKey(song);
  var isKugouProvider = provider === 'kugou' || provider === 'kugouMusic';
  var id = String(isKugouProvider ? (song.hash || song.albumAudioId || song.album_audio_id || song.id || '') : song.id || '');
  var likeKey = songLikeKey(song);
  if (!id || !likeKey || likeBusyMap[likeKey]) return;
  var next = !likedSongMap[likeKey];
  // 本地红心先行:未登录/本地文件/QQ 都立即生效并持久化;平台同步仅在对应平台已登录时进行
  likedSongMap[likeKey] = next;
  if (typeof SFLocalPlaylists !== 'undefined') SFLocalPlaylists.toggleLike(song, next);
  likeBusyMap[likeKey] = true;
  ...
  var canPlatformSync = isKugouProvider ? hasPlatformLogin(provider) : (provider === 'netease' && isCloudSong(song) && loginStatus.loggedIn);
  if (!canPlatformSync) {
    delete likeBusyMap[likeKey];
    updateLikeButtons(song);
    safeRenderQueuePanel('like-toggle-final', { scrollCurrent: miniQueueOpen });
    refreshSearchResultActionStates();
    showToast(next
      ? (provider === 'qq' ? '已加入红心喜欢 · QQ 同步待接入' : (provider === 'local' ? '已加入红心喜欢' : '已加入红心喜欢 · 未登录仅本地'))
      : '已取消红心');
    return;
  }
  try {
    var r = isKugouProvider
      ? await apiJson(provider === 'kugouMusic' ? '/api/kugou-music/song/like' : '/api/kugou/song/like', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            like: next,
    ...
    // 平台同步失败保留本地红心状态(本地为主数据源)
    showToast('红心已保存到本地,平台同步失败');
  } finally {
    delete likeBusyMap[likeKey];
    updateLikeButtons(song);
    safeRenderQueuePanel('like-toggle-final', { scrollCurrent: miniQueueOpen });
...
function openCollectModal(song) {
  var provider = songProviderKey(song);
  var isKugouProvider = provider === 'kugou' || provider === 'kugouMusic';
  var providerReady = isKugouProvider ? hasPlatformLogin(provider) : (provider === 'netease' && isCloudSong(song) && loginStatus.loggedIn);
  if (provider === 'qq') {
    showToast('QQ 音乐收藏到歌单待登录接口接入');
    return;
  }
  if (isKugouProvider && !providerReady) {
    showToast('登录' + playlistProviderLabel(provider) + '后可收藏到' + playlistProviderLabel(provider) + '歌单');
    openProviderLogin(provider);
    return;
  }
  if (!isKugouProvider && !isCloudSong(song)) {
    showToast('本地文件暂不支持收藏到网易云歌单');
    return;
  }
  if (provider === 'netease' && !providerReady && !ensureLoggedInForAction()) return;
  collectTargetSong = song;
  collectTargetProvider = provider;
  renderCollectModal();
...
function renderCollectModal() {
  ...
  var provider = normalizePlaylistProvider(collectTargetProvider || songProviderKey(song));
  var createRow = document.querySelector('.collect-create');
  if (createRow) createRow.style.display = provider === 'netease' || provider === 'kugou' || provider === 'kugouMusic' ? '' : 'none';
  ...
  if ((provider === 'kugou' || provider === 'kugouMusic') ? !hasPlatformLogin(provider) : !loginStatus.loggedIn) {
    list.innerHTML = '<div class="collect-empty">登录后显示你的' + playlistProviderLabel(provider) + '歌单</div>';
    return;
  }
  ...
  var mine = userPlaylists.filter(function(pl){ return normalizePlaylistProvider(pl.provider) === provider && !pl.subscribed; });
  if (!mine.length) {
    list.innerHTML = '<div class="collect-empty">还没有可写入的' + playlistProviderLabel(provider) + '歌单</div>';
    return;
  }
...
function createPlaylistResultMessage(r, provider) {
  if (!r) return '创建歌单失败';
  var msg = r.error || r.message || r.msg || '';
  if (msg === 'LOGIN_REQUIRED') return provider === 'kugou' || provider === 'kugouMusic' ? ('登录' + playlistProviderLabel(provider) + '后可新建歌单') : '登录后可新建歌单';
  if (msg === 'KUGOU_PLAYLIST_CREATE_FAILED') return playlistProviderLabel(provider) + '歌单创建失败';
  return msg ? ('创建歌单失败: ' + msg) : '创建歌单失败';
}
async function createPlaylistFromCollect() {
  var provider = normalizePlaylistProvider(collectTargetProvider || songProviderKey(collectTargetSong));
  var isKugouProvider = provider === 'kugou' || provider === 'kugouMusic';
  if (isKugouProvider) {
    if (!hasPlatformLogin(provider)) {
      showToast('登录' + playlistProviderLabel(provider) + '后可新建歌单');
      openProviderLogin(provider);
      return;
    }
  ...
  try {
    var r = await apiJson(isKugouProvider ? (provider === 'kugouMusic' ? '/api/kugou-music/playlist/create' : '/api/kugou/playlist/create') : '/api/playlist/create?name=' + encodeURIComponent(name), isKugouProvider ? {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: name })
    } : undefined);
    if (!(r && r.success)) throw new Error(createPlaylistResultMessage(r, provider));
    if (input) input.value = '';
    showToast('歌单已创建');
    await refreshUserPlaylists(true);
    renderCollectModal();
    var created = r && r.playlist;
    var pid = created && (created.id || created.listid);
    if (pid && collectTargetSong) addCollectTargetToPlaylist(pid);
  } catch (err) {
    showToast(err && err.message ? err.message : '创建歌单失败');
  }
}
function collectResultMessage(r) {
  if (!r) return '收藏失败';
  var msg = r.error || r.message || r.msg || '';
  var provider = normalizePlaylistProvider(collectTargetProvider || (collectTargetSong && songProviderKey(collectTargetSong)));
  if (msg === 'LOGIN_REQUIRED') return provider === 'kugou' || provider === 'kugouMusic' ? ('登录' + playlistProviderLabel(provider) + '后可同步到' + playlistProviderLabel(provider) + '歌单') : '登录后可同步到网易云';
  if (msg === 'KUGOU_PLAYLIST_ADD_FAILED') return playlistProviderLabel(provider) + '歌单写入失败';
...
async function verifySongInPlaylist(pid, songId, provider) {
  ...
  for (var attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      var isKugouProvider = provider === 'kugou' || provider === 'kugouMusic';
      var detail = await apiJson((isKugouProvider ? (provider === 'kugouMusic' ? '/api/kugou-music/playlist/tracks?id=' : '/api/kugou/playlist/tracks?id=') : '/api/playlist/tracks?id=') + encodeURIComponent(pid));
      var tracks = (detail && detail.tracks) || [];
      for (var i = 0; i < tracks.length; i++) {
        if (isKugouProvider) {
          var values = [tracks[i].id, tracks[i].hash, tracks[i].albumAudioId, tracks[i].album_audio_id].map(function(value){ return String(value || '').toLowerCase(); });
          if (values.indexOf(songId.toLowerCase()) >= 0) return true;
        } else if (String(tracks[i].id) === songId) {
          return true;
        }
      }
    } catch (e) {
...
async function addCollectTargetToPlaylist(pid) {
  ...
  try {
    var isKugouProvider = provider === 'kugou' || provider === 'kugouMusic';
    var songId = String(isKugouProvider
      ? (collectTargetSong.hash || collectTargetSong.albumAudioId || collectTargetSong.album_audio_id || collectTargetSong.id || '')
      : (collectTargetSong.id || ''));
    var payload = isKugouProvider
      ? {
          pid: pid,
          hash: collectTargetSong.hash || '',
          id: collectTargetSong.id || '',
          name: collectTargetSong.name || '',
          albumId: collectTargetSong.albumId || collectTargetSong.album_id || '',
          albumAudioId: collectTargetSong.albumAudioId || collectTargetSong.album_audio_id || ''
        }
      : { pid: pid, id: songId };
    var r = await apiJson(isKugouProvider ? (provider === 'kugouMusic' ? '/api/kugou-music/playlist/add-song' : '/api/kugou/playlist/add-song') : '/api/playlist/add-song', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    refreshUserPlaylists(true);
    setTimeout(function(){
      verifySongInPlaylist(pid, songId, provider).then(function(ok){
...
var SEARCH_HISTORY_STORE_KEY = 'stellaflix-search-history';
...
function isMusicSearchMode(mode) {
  return mode !== 'podcast' && mode !== 'qishui';
}
...
function updateSearchModeTabs() {
  ...
  var kugouMusicBtn = document.getElementById('search-mode-kugou-music');
  ...
  if (kugouMusicBtn) {
    kugouMusicBtn.classList.toggle('active', searchMode === 'kugouMusic');
    kugouMusicBtn.setAttribute('aria-selected', searchMode === 'kugouMusic' ? 'true' : 'false');
  }
  if (qishuiBtn) {
  ...
  if ($input) {
    if (searchMode === 'qishui') $input.placeholder = '粘贴汽水音乐分享链接或 track_id...';
    else $input.placeholder = searchMode === 'podcast'
      ? '搜索播客、电台...'
      : (searchMode === 'qq' ? '搜索 QQ 音乐...' : (searchMode === 'kugou' ? '搜索酷狗概念版...' : (searchMode === 'kugouMusic' ? '搜索酷狗音乐...' : (searchMode === 'netease' ? '搜索网易云音乐...' : '搜索歌曲、歌手...'))));
  }
  requestAnimationFrame(updateSearchPillGlassDisplacementMap);
}
function setSearchMode(mode) {
  mode = (mode === 'podcast' || mode === 'netease' || mode === 'qq' || mode === 'kugou' || mode === 'kugouMusic' || mode === 'qishui') ? mode : 'song';
  ...
  } else if (searchMode === 'qishui') {
    if (q) importQishuiShare(q);
    else renderQishuiSharePrompt();
  } else if (q) {
    doSearch(q);
  } else {
    renderSearchHistory();
...
function looksLikeQishuiShareInput(text) {
  text = String(text || '').trim();
  return /^(\d{16,24})$/.test(text) || /汽水|qishui|luna|soda|track[_-]?id|trackId/i.test(text);
}
function renderQishuiSharePrompt(text) {
  playlist = [];
  podcastResults = [];
  podcastPrograms = [];
  podcastCurrentRadio = null;
  ...
  $results.innerHTML =
    '<div class="search-empty">' +
      '<div style="font-weight:750;color:rgba(255,255,255,.82);margin-bottom:6px">汽水音乐实验音源</div>' +
      '<div style="line-height:1.65;color:rgba(255,255,255,.48)">粘贴汽水音乐歌曲或歌单分享链接，然后按 Enter 导入。第一阶段只做分享链接导入和尝试播放，不支持汽水账号登录、收藏和个人歌单。</div>' +
      (sample ? '<div style="margin-top:8px;color:rgba(244,210,138,.72)">按 Enter 导入当前链接</div>' : '') +
    '</div>';
  $results.classList.add('show');
}
function qishuiCredentialHint(data) {
  var missing = data && Array.isArray(data.missing) ? data.missing : [];
  if (!missing.length) return '';
  return '需要配置汽水电脑版凭证: ' + missing.join(', ');
}
function qishuiImportedPlaylistId(text, songs) {
  var raw = String(text || '');
  if (!raw && Array.isArray(songs)) raw = songs.map(function(song){ return song && (song.qishuiTrackId || song.id || ''); }).join('|');
  var hash = 2166136261;
  for (var i = 0; i < raw.length; i++) {
    hash ^= raw.charCodeAt(i);
    hash += (hash << 1) + (hash << 4) + (hash << 7) + (hash << 8) + (hash << 24);
  }
  return 'share-' + (hash >>> 0).toString(36);
}
function normalizeQishuiImportedSong(song, index) {
  song = song || {};
  var id = String(song.qishuiTrackId || song.id || '').trim();
  if (!id) return null;
  return {
    provider: 'qishui',
    source: 'qishui',
    type: 'qishui',
    mediaType: song.mediaType || song.qishuiMediaType || 'track',
    qishuiMediaType: song.qishuiMediaType || song.mediaType || 'track',
    id: id,
    qishuiTrackId: id,
    name: song.name || ('汽水音乐 ' + id),
    artist: song.artist || '汽水音乐',
    artists: Array.isArray(song.artists) ? song.artists : [],
    album: song.album || '',
    cover: song.cover || '',
    duration: song.duration || 0,
    playable: !!song.playable,
    restriction: song.restriction || null,
    experimental: true,
    sourceUrl: song.sourceUrl || '',
    savedIndex: Number.isFinite(Number(song.savedIndex)) ? Number(song.savedIndex) : index,
    savedAt: song.savedAt || Date.now(),
  };
}
function normalizeQishuiImportedPlaylist(pl, index) {
  pl = pl || {};
  var tracks = Array.isArray(pl.tracks) ? pl.tracks.map(normalizeQishuiImportedSong).filter(Boolean) : [];
  var id = String(pl.id || qishuiImportedPlaylistId(pl.sourceUrl || '', tracks) || ('share-' + index));
  return {
    id: id,
    provider: 'qishui',
    source: 'qishui',
    name: pl.name || '汽水音乐歌单',
    cover: pl.cover || (tracks[0] && tracks[0].cover) || '',
    trackCount: tracks.length,
    creator: pl.creator || '本地导入',
    playCount: 0,
    importedAt: pl.importedAt || Date.now(),
    sourceUrl: pl.sourceUrl || '',
    tracks: tracks,
  };
}
function readQishuiImportedPlaylists() {
  try {
    var raw = JSON.parse(localStorage.getItem(QISHUI_IMPORTED_PLAYLIST_STORE_KEY) || '[]') || [];
    return Array.isArray(raw) ? raw.map(normalizeQishuiImportedPlaylist).filter(function(pl){ return pl.tracks.length; }) : [];
  } catch (e) {
    return [];
  }
}
function writeQishuiImportedPlaylists() {
  try {
    localStorage.setItem(QISHUI_IMPORTED_PLAYLIST_STORE_KEY, JSON.stringify((qishuiImportedPlaylists || []).slice(0, 12)));
    return true;
  } catch (e) {
    console.warn('Qishui imported playlist save failed:', e);
    return false;
  }
}
function saveQishuiImportedPlaylist(text, data, songs) {
  songs = Array.isArray(songs) ? songs : [];
  var tracks = songs.map(normalizeQishuiImportedSong).filter(Boolean);
  if (!tracks.length) return null;
  var id = qishuiImportedPlaylistId(text, tracks);
  var existing = (qishuiImportedPlaylists || []).filter(function(pl){ return String(pl.id || '') !== id; });
  var sourceName = data && data.playlist && data.playlist.name;
  var pl = normalizeQishuiImportedPlaylist({
    id: id,
    name: sourceName ? ('汽水音乐歌单 · ' + sourceName) : '汽水音乐歌单',
    cover: (data && data.playlist && data.playlist.cover) || tracks[0].cover || '',
    creator: '本地导入',
    importedAt: Date.now(),
    sourceUrl: text,
    tracks: tracks,
  }, 0);
  qishuiImportedPlaylists = [pl].concat(existing).slice(0, 12);
  writeQishuiImportedPlaylists();
  return pl;
}
function getQishuiImportedPlaylist(pid) {
  pid = String(pid || '');
  return (qishuiImportedPlaylists || []).find(function(pl){ return String(pl.id || '') === pid; }) || null;
}
function getQishuiImportedPlaylistTracks(pid) {
  qishuiImportedPlaylists = readQishuiImportedPlaylists();
  var pl = getQishuiImportedPlaylist(pid);
  return { provider: 'qishui', loggedIn: true, playlist: pl || { id: pid, provider: 'qishui', name: '汽水音乐歌单', trackCount: 0 }, tracks: pl ? (pl.tracks || []) : [] };
}
function hasQishuiImportedPlaylists() {
  return !!(qishuiImportedPlaylists && qishuiImportedPlaylists.length);
}
qishuiImportedPlaylists = readQishuiImportedPlaylists();
async function importQishuiShare(text, opts) {
  opts = opts || {};
  text = String(text || '').trim();
  if (!text) {
    renderQishuiSharePrompt();
    return [];
  }
  var requestSeq = ++searchRequestSeq;
  playlist = [];
  searchLastResultQuery = '';
  $results.innerHTML = '<div class="search-empty">正在解析汽水音乐分享链接...</div>';
  $results.classList.add('show');
  try {
    var data = await apiJson('/api/qishui/share', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    playlist = songs;
    if (!songs.length) {
      $results.innerHTML = '<div class="search-empty">没有识别到汽水歌曲 ID。可以试试复制 App 分享出来的完整链接。</div>';
      return [];
    }
    var savedQishuiPlaylist = saveQishuiImportedPlaylist(text, data, songs);
    renderSongSearchResults(songs);
    if (savedQishuiPlaylist) {
      refreshUserPlaylists(true);
      openPlaylistPanelTab('playlists', true);
    }
    var hint = qishuiCredentialHint(data);
    if (hint) showToast(hint);
    else showToast('已导入汽水音乐 ' + songs.length + ' 首，并保存到我的歌单');
    if (opts.autoPlayFirst) playSearchResult(0);
    return songs;
  } catch (err) {
    console.error('Qishui import:', err);
    if (requestSeq === searchRequestSeq) $results.innerHTML = '<div class="search-empty">汽水分享解析失败: ' + escHtml(err && err.message || err) + '</div>';
    return [];
  }
}
...
$input.addEventListener('input', function(){
  ...
  if (!q) {
    ...
    else if (searchMode === 'qishui') renderQishuiSharePrompt();
    else renderSearchHistory();
    return;
  }
  if (searchMode === 'qishui') {
    renderQishuiSharePrompt(q);
    return;
  }
...
$input.addEventListener('focus', function(){
  var searchArea = document.getElementById('search-area');
  if (searchArea) setPeek(searchArea, true, 'search');
  if (searchMode === 'qishui' && !$input.value.trim()) renderQishuiSharePrompt();
...
function songProviderKey(song) {
  ...
  if (song && (song.provider === 'kugouMusic' || song.source === 'kugouMusic' || song.type === 'kugouMusic')) return 'kugouMusic';
...
function songSourceTagHtml(song) {
  ...
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'kugouMusic' ? 'KGM' : (key === 'qishui' ? 'QS' : (key === 'local' ? 'LOCAL' : 'NE'))));
...
function searchResultMetaText(song) {
  ...
  if (songProviderKey(song) === 'kugouMusic' && !song.playable) bits.push('酷狗音乐播放需会话/授权');
  if (songProviderKey(song) === 'qishui' && !song.playable) bits.push(song.needsCredentials ? '汽水需配置电脑版凭证' : '汽水播放地址待解析');
...
function scoreSongSearchResult(song, q, sourceIndex) {
  ...
  if (songProviderKey(song) === 'kugou') score += hasPlatformLogin('kugou') ? 36 : 5;
  if (songProviderKey(song) === 'kugouMusic') score += hasPlatformLogin('kugouMusic') ? 34 : 5;
...
function mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, kugouMusicSongs, limit, q) {
  var out = [];
  var seen = {};
  function push(song, sourceIndex) {
    if (!song || !song.name) return;
    var key = songProviderKey(song) + ':' + (song.hash || song.albumAudioId || song.album_audio_id || song.mid || song.id || (song.name + '|' + song.artist));
    if (seen[key]) return;
    seen[key] = true;
  ...
  (kugouMusicSongs || []).forEach(function(song, i){ push(song, i); });
...
async function fetchMusicSearchResults(q, mode) {
  ...
  if (mode === 'kugou') {
    var kgOnly = await apiJson('/api/kugou/search?keywords=' + encodeURIComponent(q) + '&limit=14');
    return mergeSongSearchResults([], [], kgOnly.songs || [], [], 18, q);
  }
  if (mode === 'kugouMusic') {
    var kgmOnly = await apiJson('/api/kugou-music/search?keywords=' + encodeURIComponent(q) + '&limit=14');
    return mergeSongSearchResults([], [], [], kgmOnly.songs || [], 18, q);
  }
  if (mode === 'netease') {
  ...
  var result = await Promise.allSettled([
    ...
    apiJson('/api/kugou/search?keywords=' + encodeURIComponent(q) + '&limit=12'),
    apiJson('/api/kugou-music/search?keywords=' + encodeURIComponent(q) + '&limit=12')
  ]);
  ...
  var kugouSongs = result[2].status === 'fulfilled' ? ((result[2].value && result[2].value.songs) || []) : [];
  var kugouMusicSongs = result[3].status === 'fulfilled' ? ((result[3].value && result[3].value.songs) || []) : [];
  if (result[1].status === 'rejected') console.warn('QQ search failed:', result[1].reason);
  if (result[2].status === 'rejected') console.warn('KuGou search failed:', result[2].reason);
  if (result[3].status === 'rejected') console.warn('KuGou Music search failed:', result[3].reason);
  return mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, kugouMusicSongs, 18, q);
}
function renderSongSearchResults(songs) {
  ...
  $results.innerHTML = playlist.map(function(s, i){
    ...
    var libraryActionHtml = songProviderKey(s) === 'qishui'
      ? ''
      : '<button class="song-action-btn' + (isSongLiked(s) ? ' liked' : '') + '" data-like-index="' + i + '" title="红心喜欢" onclick="event.stopPropagation();toggleLikeSearchResult(' + i + ')">' + heartIconSvg() + '</button>' +
...
async function doSearch(q, opts) {
  ...
  if (searchMode !== 'netease' && looksLikeQishuiShareInput(q)) {
    importQishuiShare(q, opts);
    return;
  }
  if (searchMode === 'qishui') {
    importQishuiShare(q, opts);
...
// ============================================================
//  [04e-audio-engine · 音频引擎核心 + 增益 ramp + 音量控制]
//  01-part2: 以下单元已迁至 src/music/04e-audio-engine.js（classic script，04d 之后、08 之前加载）。
//  仅保留标记；运行时符号经全局作用域由 legacy 闭包引用。
// ============================================================
...
function queueItemKey(song) {
  if (!song) return '';
  if (song.provider === 'qq' || song.source === 'qq' || song.type === 'qq') return 'qq:' + (song.mid || song.songmid || song.id || (song.name + '|' + song.artist));
  if (song.provider === 'kugou' || song.source === 'kugou' || song.type === 'kugou') return 'kugou:' + (song.hash || song.albumAudioId || song.album_audio_id || song.id || (song.name + '|' + song.artist));
  if (song.provider === 'kugouMusic' || song.source === 'kugouMusic' || song.type === 'kugouMusic') return 'kugouMusic:' + (song.hash || song.albumAudioId || song.album_audio_id || song.id || (song.name + '|' + song.artist));
  if (song.provider === 'qishui' || song.source === 'qishui' || song.type === 'qishui') return 'qishui:' + (song.qishuiTrackId || song.id || (song.name + '|' + song.artist));
  if (song.type === 'podcast' && song.programId) return 'podcast:' + song.programId;
  if (song.localKey) return 'local:' + song.localKey;
  if (song.id != null && song.id !== '') return 'song:' + song.id;
...
function playbackProviderLabel(song) {
  if (song && (song.provider === 'lx-custom-source' || song.source === 'lx-custom-source')) return '第三方音源';
  var provider = songProviderKey(song);
  if (provider === 'qq') return 'QQ 音乐';
  if (provider === 'kugou') return '酷狗概念版';
  if (provider === 'kugouMusic') return '酷狗音乐';
...
function playbackLoginProvider(song) {
  ...
  if (provider === 'kugouMusic') return 'kugouMusic';
  if (provider === 'qishui') return 'qishui';
  return 'netease';
}
async function resolveOfficialPlaybackData(song, requestedQuality) {
  var playbackProvider = songProviderKey(song);
  var isQQ = playbackProvider === 'qq';
  var isKugou = playbackProvider === 'kugou' || playbackProvider === 'kugouMusic';
  var isQishui = playbackProvider === 'qishui';
  requestedQuality = providerPlaybackQualityValue(playbackProvider, normalizePlaybackQuality(requestedQuality || playbackQuality));
  if (!isQQ && !isKugou && !isQishui && requestedQuality === 'jymaster' && !hasProviderSvip('netease', loginStatus)) requestedQuality = 'hires';
  if (isQQ && qqPlaybackQualityCeiling && (requestedQuality === 'jymaster' || requestedQuality === 'hires' || requestedQuality === 'lossless')) requestedQuality = qqPlaybackQualityCeiling;
  var qualityParam = '&quality=' + encodeURIComponent(requestedQuality);
  var result = isQQ
    ? await apiJson('/api/qq/song/url?mid=' + encodeURIComponent(song.mid || song.songmid || song.id || '') + '&mediaMid=' + encodeURIComponent(song.mediaMid || song.media_mid || '') + qualityParam)
    : (isQishui
      ? await apiJson('/api/qishui/song/url?id=' + encodeURIComponent(song.qishuiTrackId || song.id || '') + '&mediaType=' + encodeURIComponent(song.qishuiMediaType || song.mediaType || 'track'))
      : (isKugou
        ? await apiJson(playbackProvider === 'kugouMusic' ? kugouMusicSongUrlApiPath(song, qualityParam) : kugouSongUrlApiPath(song, qualityParam))
        : await apiJson('/api/song/url?id=' + encodeURIComponent(song.id) + qualityParam)));
  if (!result || typeof result !== 'object') result = { url: '', reason: 'url_unavailable' };
  result.requestedQuality = requestedQuality;
  return result;
}
var SOURCE_MODE_STORE_KEY = 'stellaflix-source-mode-v1';
var PLAYBACK_SOURCE_MODES = ['custom-first', 'official-first', 'custom-only'];
var PLAYBACK_SOURCE_MODE_HINTS = {
  'custom-first': '默认:先由已启用的第三方音源解析整曲;失败时回退官方链路(含试听),最后自动换源。',
  'official-first': '旧行为:官方链路优先,仅技术性失败时尝试第三方音源,权限类限制不触发第三方解析。',
  'custom-only': '只走第三方音源:未启用或解析失败时该首不可播,自动换源后仍按此模式重试。'
};
function readPlaybackSourceMode() {
  try {
    var stored = window.localStorage.getItem(SOURCE_MODE_STORE_KEY) || '';
    return PLAYBACK_SOURCE_MODES.indexOf(stored) >= 0 ? stored : 'custom-first';
  } catch (error) { return 'custom-first'; }
}
function writePlaybackSourceMode(mode) {
  if (PLAYBACK_SOURCE_MODES.indexOf(mode) < 0) return;
  if (mode === 'custom-first') window.localStorage.removeItem(SOURCE_MODE_STORE_KEY);
  else window.localStorage.setItem(SOURCE_MODE_STORE_KEY, mode);
}
function setPlaybackSourceMode(mode) {
  writePlaybackSourceMode(mode);
  renderPlaybackSourceModeRow();
}
function renderPlaybackSourceModeRow() {
  var row = document.getElementById('custom-source-mode-row');
  var hint = document.getElementById('custom-source-mode-hint');
  var mode = readPlaybackSourceMode();
  if (row) {
    var buttons = row.querySelectorAll('button[data-mode]');
    for (var i = 0; i < buttons.length; i++) buttons[i].classList.toggle('primary', buttons[i].getAttribute('data-mode') === mode);
  }
  if (hint) hint.textContent = PLAYBACK_SOURCE_MODE_HINTS[mode] || '';
}
async function tryCustomSourceResolve(song, requestedQuality, mode, officialFailure) {
  try {
    var custom = await apiJson('/api/custom-source/resolve', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ song: song || {}, quality: requestedQuality || 'hires', mode: mode })
    });
    if (custom && custom.url && custom.thirdParty === true) {
      custom.requestedQuality = normalizePlaybackQuality(requestedQuality);
      if (officialFailure) custom.officialFailure = officialFailure;
      return custom;
    }
  } catch (error) { console.warn('[CustomSourceResolve]', error); }
  return null;
}
async function resolveOnlinePlaybackData(song, requestedQuality) {
  // 本地音乐库短路:直接用 /api/local-file 代理地址(Range 可拖进度条),不进在线取源链。
  if (song && (song.provider === 'local' || song.type === 'local')) {
    var localUrl = typeof SFLocalLibrary !== 'undefined' ? SFLocalLibrary.ensureLocalSongUrl(song) : '';
    if (localUrl) {
      return { url: localUrl, local: true, provider: 'local', playbackProvider: 'local', requestedQuality: normalizePlaybackQuality(requestedQuality) };
    }
    return {
      url: '',
      trial: false,
      ...
      message: '本地文件不可用,可能已被移动或删除',
      requestedQuality: normalizePlaybackQuality(requestedQuality)
    };
  }
  var mode = readPlaybackSourceMode();
  if (mode !== 'official-first') {
    var customFirst = await tryCustomSourceResolve(song, requestedQuality, mode);
    if (customFirst && customFirst.url) return customFirst;
    if (mode === 'custom-only') {
      return customFirst || {
        url: '',
        trial: false,
        playable: false,
        reason: 'custom_source_unavailable',
        message: '仅自定义音源模式:未启用第三方音源或解析失败',
        requestedQuality: normalizePlaybackQuality(requestedQuality)
      };
    }
  }
  var officialResult;
  try { officialResult = await resolveOfficialPlaybackData(song, requestedQuality); }
  catch (error) { officialResult = { url: '', reason: 'request_failed', error: error && error.message ? error.message : String(error), requestedQuality: normalizePlaybackQuality(requestedQuality) }; }
  if (officialResult.url) return officialResult;
  // custom-first 已在入口尝试过第三方解析,直接返回官方失败进入换源流程。
  if (mode === 'custom-first') return officialResult;
  try {
    var custom = await apiJson('/api/custom-source/resolve', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        song: song || {},
        quality: officialResult.requestedQuality || requestedQuality || 'hires',
        officialResult: {
          url: '',
          reason: officialResult.reason || '',
          error: officialResult.error || '',
          restriction: officialResult.restriction || null
        }
      })
    });
    if (custom && custom.url && custom.thirdParty === true) {
      custom.requestedQuality = officialResult.requestedQuality || requestedQuality;
      custom.officialFailure = officialResult;
      return custom;
    }
  } catch (error) { console.warn('[CustomSourceResolve]', error); }
...
async function retryQQPlaybackWithCompatibleQuality(song, idx, token, opts, data, requestedQuality) {
  ...
  showSourceFallbackNotice('QQ 音质自动兼容', '当前音质启动失败，正在切到 ' + playbackQualityLabelForProvider('qq', nextQuality) + '。');
...
function providerPlaybackReady(provider) {
  var st = platformStatus(provider) || {};
  if (provider === 'netease') return true;
  if (!st.loggedIn) return false;
  if (provider === 'qq' && Object.prototype.hasOwnProperty.call(st, 'playbackKeyReady')) return !!st.playbackKeyReady;
  if ((provider === 'kugou' || provider === 'kugouMusic') && Object.prototype.hasOwnProperty.call(st, 'tokenReady')) return !!st.tokenReady;
  return true;
}
function fallbackProviderRank(provider) {
  var vip = hasProviderVip(provider, platformStatus(provider));
  var logged = hasPlatformLogin(provider);
  var ready = providerPlaybackReady(provider);
  var rank = provider === 'netease' ? 62 : 96;
  if (vip && ready) rank = 0;
  else if (ready && logged) rank = 18;
  else if (provider === 'netease' && logged) rank = 34;
  else if (logged) rank = 70;
  if (provider === activeAccountProvider) rank -= 24;
  if (provider === 'kugouMusic') rank -= 7;
  if (provider === 'kugou') rank -= 6;
  if (provider === 'qq') rank -= 3;
  if (provider === 'netease') rank -= 1;
  return rank;
}
function alternatePlaybackProviders(song) {
  var current = songProviderKey(song);
  var providers = ['kugouMusic', 'kugou', 'qq', 'netease'].filter(function(provider){ return provider !== current; });
  providers.sort(function(a, b){ return fallbackProviderRank(a) - fallbackProviderRank(b); });
  return providers;
}
...
function providerSearchUrl(provider, query) {
  ...
  if (provider === 'kugou') return '/api/kugou/search?keywords=' + encodeURIComponent(query) + '&limit=10';
  if (provider === 'kugouMusic') return '/api/kugou-music/search?keywords=' + encodeURIComponent(query) + '&limit=10';
...
async function searchAlternatePlatformSong(song) {
  ...
  for (var p = 0; p < providers.length; p++) {
    var target = providers[p];
    try {
      var data = await apiJson(providerSearchUrl(target, query));
      var list = data && (data.songs || data.result || []);
      for (var i = 0; i < list.length; i++) {
        if (isSameTitleArtist(song, list[i])) {
          var alternate = cloneSong(list[i]);
          alternate.autoFallbackTarget = target;
          return alternate;
        }
      }
    } catch (e) {
      console.warn('[AutoFallbackSearch]', target, e);
    }
  }
  return null;
}
...
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  ...
  var targetLabel = alternatePlaybackProviders(song).map(function(provider){ return playbackProviderLabel({ provider: provider }); }).join(' / ');
  showSourceFallbackNotice('正在自动换源', fromLabel + ' 当前不可播，正在按账号权限查找 ' + targetLabel + ' 的同名同歌手版本。');
  try {
    ...
    if (!alternate) {
      ...
      skipFailedQueueItem(idx, token, '没有找到同名同歌手的其它音源版本，正在播放下一首。');
      return true;
    }
    ...
    showSourceFallbackNotice('已自动切换音源', (song.name || '当前歌曲') + ' 已从 ' + fromLabel + ' 切到 ' + playbackProviderLabel(alternate) + '。');
    await playQueueAt(idx, { fallbackDepth: 1, manual: !!opts.manual });
    return true;
  } catch (e) {
    if (token !== trackSwitchToken) return true;
...
function pauseCurrentAudioForTrackSwitch() {
  ...
  if (!audio) return;
  if (manualAudioUnlockActive && audio.src === MANUAL_AUDIO_UNLOCK_SRC) {
    playing = false;
    setPlayIcon(false);
    return;
  }
...
async function playQueueAt(idx, opts) {
  opts = opts || {};
  if (opts.manual) primeManualAudioPlayback();
  ...
  // T134-f Layer1：包装回调——影视态下彻底拦截音乐态海报/文案覆盖（双重保险：回调入口 + 函数内部守卫）
  safePlaybackStep('home-poster', function(){
    if (document.body && document.body.classList.contains('video-space-active')) return;
    renderHomePersonalPoster();
  });
  ...
  try {
    ...
    var isKugouPlayback = playbackProvider === 'kugou' || playbackProvider === 'kugouMusic';
    ...
    var data = await resolveOnlinePlaybackData(song, requestedQuality);
    requestedQuality = normalizePlaybackQuality(data.requestedQuality || requestedQuality);
    if (token !== trackSwitchToken) return;
    if (!data.url) {
    ...
    var resolvedPlaybackProvider = data.playbackProvider || data.fallbackProvider || data.provider || playbackProvider;
    if (data.thirdParty) currentPlaybackProvider = 'lx-custom-source';
    else currentPlaybackProvider = normalizeAccountProvider(resolvedPlaybackProvider);
    safePlaybackStep('quality-ui-resolved', updatePlaybackQualityUi);
    var resolvedQualityText = playbackResolvedQualityText(data, data.thirdParty ? playbackProvider : currentPlaybackProvider);
    if (data.thirdParty) {
      showSourceFallbackNotice('已使用第三方音源', '官方地址因技术原因不可用，当前由已启用脚本解析 · ' + resolvedQualityText + '。');
    } else if (!isQQPlayback && !isKugouPlayback && !isQishuiPlayback && playbackQualityWasDowngraded(requestedQuality, data.level)) {
      showSourceFallbackNotice('网易云音质自动降级', '请求 ' + playbackQualityLabelForProvider('netease', requestedQuality) + '，实际播放 ' + resolvedQualityText + '。');
    ...
    releaseManualAudioPlaybackPrime();
    applyVolumeToAudio();
    var proxyAudioUrl = (data.thirdParty || data.local) ? data.url : '/api/audio?url=' + encodeURIComponent(data.url);
    ...
    var playbackStarted = await playAudio({ manual: !!opts.manual, silent: isQQPlayback || data.thirdParty });
    if (!playbackStarted) {
      if (data.thirdParty && await tryAutoPlaybackFallback(song, data.officialFailure || { reason: 'url_unavailable' }, idx, token, opts)) return;
      if (isQQPlayback && await retryQQPlaybackWithCompatibleQuality(song, idx, token, opts, data, requestedQuality)) return;
      forcePlaybackControlsInteractive();
      if (opts.manual) {
...
async function fetchLyric(songOrId, token) {
  try {
    ...
    var provider = songProviderKey(song);
    // 本地音乐:优先同名 .lrc/.txt(IPC 范围读取并缓存在歌曲对象上),无则走兜底行
    if (provider === 'local' && typeof SFLocalLibrary !== 'undefined') {
      var localText = await SFLocalLibrary.readLocalLyricText(song);
      if (token !== trackSwitchToken) return;
      var localLrcLines = parseLyricText(localText || '');
      var localTiming = localLrcLines.length ? 'local-lrc' : 'fallback';
      var localLines = withLyricFallback(localLrcLines);
      if (localLines.length && localLines[0].fallback) localTiming = 'fallback';
      setOriginalLyricsState(localLines, false, localTiming);
      applyPreferredLyricsForCurrent(true);
      return;
    }
    var endpoint;
    if (provider === 'qq') {
      ...
      var kugouHash = song.hash || '';
      var kugouAlbumAudioId = song.albumAudioId || song.album_audio_id || '';
      var kugouKeywords = ((song.name || '') + ' ' + (song.artist || '')).trim();
      endpoint = '/api/kugou/lyric?hash=' + encodeURIComponent(kugouHash) +
        '&albumAudioId=' + encodeURIComponent(kugouAlbumAudioId) +
        '&duration=' + encodeURIComponent(song.duration || 0) +
        '&keywords=' + encodeURIComponent(kugouKeywords) +
        '&name=' + encodeURIComponent(song.name || '') +
        '&artist=' + encodeURIComponent(song.artist || '');
    } else if (provider === 'kugouMusic') {
      var kugouMusicHash = song.hash || '';
      var kugouMusicAlbumAudioId = song.albumAudioId || song.album_audio_id || '';
      var kugouMusicKeywords = ((song.name || '') + ' ' + (song.artist || '')).trim();
      endpoint = '/api/kugou-music/lyric?hash=' + encodeURIComponent(kugouMusicHash) +
        '&albumAudioId=' + encodeURIComponent(kugouMusicAlbumAudioId) +
        '&duration=' + encodeURIComponent(song.duration || 0) +
        '&keywords=' + encodeURIComponent(kugouMusicKeywords) +
        '&name=' + encodeURIComponent(song.name || '') +
        '&artist=' + encodeURIComponent(song.artist || '');
    } else {
...
// ---------- 本地歌单动作:平台歌单快照导入/更新/删除(阶段二:登录只用于导入) ----------
function localPlaylistFetchTracks(provider, pid) {
  var url = provider === 'qq' ? '/api/qq/playlist/tracks?id=' + encodeURIComponent(pid)
    : provider === 'kugouMusic' ? '/api/kugou-music/playlist/tracks?id=' + encodeURIComponent(pid)
    : provider === 'kugou' ? '/api/kugou/playlist/tracks?id=' + encodeURIComponent(pid)
    : '/api/playlist/tracks?id=' + encodeURIComponent(pid);
  return apiJson(url);
}
async function importRemotePlaylistSnapshot(provider, pid, name) {
  if (typeof SFLocalPlaylists === 'undefined' || !pid) return;
  provider = normalizePlaylistProvider(provider);
  if (provider === 'local' || provider === 'qishui') return;
  var needsLogin = provider === 'netease' ? !loginStatus.loggedIn
    : provider === 'qq' ? !qqLoginStatus.loggedIn
    : provider === 'kugou' ? !kugouLoginStatus.loggedIn
    : !kugouMusicLoginStatus.loggedIn;
  if (needsLogin) {
    showToast('登录' + playlistProviderLabel(provider) + '后可导入歌单快照');
    openProviderLogin(provider);
    return;
  }
  showToast('正在导入「' + (name || pid) + '」…');
  try {
    var result = await SFLocalPlaylists.importRemotePlaylist(provider, pid, { name: name }, localPlaylistFetchTracks);
    showToast((result.updated ? '已更新本地歌单「' : '已导入本地歌单「') + result.pl.name + '」· ' + result.count + ' 首');
    resetPlaylistPanelRenderLimit();
    renderUserPlaylistsList({ reset: true });
    scheduleShelfRebuild('local-playlist-import', true);
  } catch (e) {
    console.warn('[LocalPlaylistImport]', provider, pid, e);
    showToast(e && e.message === 'EMPTY_PLAYLIST' ? '歌单为空或没有可导入曲目' : '导入失败:' + (e && e.message || ''));
  }
}
async function updateLocalPlaylistSnapshot(pid) {
  if (typeof SFLocalPlaylists === 'undefined' || !pid) return;
  var pl = SFLocalPlaylists.list().find(function(item){ return item.id === pid; });
  if (!pl || !pl.remoteProvider || !pl.remoteId) return;
  var provider = pl.remoteProvider;
  var needsLogin = provider === 'netease' ? !loginStatus.loggedIn
    : provider === 'qq' ? !qqLoginStatus.loggedIn
    : provider === 'kugou' ? !kugouLoginStatus.loggedIn
    : !kugouMusicLoginStatus.loggedIn;
  if (needsLogin) {
    showToast('登录' + playlistProviderLabel(provider) + '后可更新此快照');
    openProviderLogin(provider);
    return;
  }
  showToast('正在更新「' + pl.name + '」…');
  try {
    var result = await SFLocalPlaylists.importRemotePlaylist(provider, pl.remoteId, { name: pl.name, cover: pl.cover }, localPlaylistFetchTracks);
    showToast('已更新「' + result.pl.name + '」· ' + result.count + ' 首');
    resetPlaylistPanelRenderLimit();
    renderUserPlaylistsList({ reset: true });
  } catch (e) {
    console.warn('[LocalPlaylistUpdate]', pid, e);
    showToast('更新失败:' + (e && e.message || ''));
  }
}
function removeLocalPlaylistSnapshot(pid) {
  if (typeof SFLocalPlaylists === 'undefined' || !pid) return;
  var pl = SFLocalPlaylists.list().find(function(item){ return item.id === pid; });
  if (!pl) return;
  if (!window.confirm('删除本地歌单「' + (pl.name || '') + '」?不影响平台原歌单。')) return;
  if (playlistPanelDetailState.key === playlistPanelKey('local', pid)) {
    playlistPanelDetailState = { key: '', loading: false, playlist: null, tracks: [], token: playlistPanelDetailState.token, renderLimit: PLAYLIST_DETAIL_INITIAL_RENDER };
  }
  SFLocalPlaylists.removePlaylist(pid);
  resetPlaylistPanelRenderLimit();
  renderUserPlaylistsList({ reset: true });
...
async function refreshUserPlaylists(force) {
  qishuiImportedPlaylists = readQishuiImportedPlaylists();
  var hasLocalPanelContent = typeof SFLocalPlaylists !== 'undefined' && SFLocalPlaylists.hasContent();
  if (!loginStatus.loggedIn && !qqLoginStatus.loggedIn && !kugouLoginStatus.loggedIn && !kugouMusicLoginStatus.loggedIn && !hasQishuiImportedPlaylists() && !hasLocalPanelContent) {
    resetPlaylistPanelRenderLimit();
    document.getElementById('pl-list').innerHTML = '<div style="text-align:center;padding:24px 0;color:rgba(255,255,255,.32);font-size:11.5px">登录后可导入平台歌单 · 本地红心无需登录</div>';
  ...
  if (!loginStatus.loggedIn && !qqLoginStatus.loggedIn && !kugouLoginStatus.loggedIn && !kugouMusicLoginStatus.loggedIn && hasLocalPanelContent) {
    // 未登录但有本地歌单/红心:只渲染本地分组,不发起平台请求
    resetPlaylistPanelRenderLimit();
    userPlaylists = hasQishuiImportedPlaylists() ? qishuiImportedPlaylists : [];
    renderUserPlaylistsList({ reset: true });
    var podcastListLocalOnly = document.getElementById('podcast-list');
    if (podcastListLocalOnly) podcastListLocalOnly.innerHTML = '<div style="text-align:center;padding:14px 0;color:rgba(255,255,255,.28);font-size:11.5px">登录后显示我的播客</div>';
    return;
  }
  ...
  var hasCachedKugouMusicPlaylists = userPlaylists.some(function(pl){ return pl && pl.provider === 'kugouMusic'; });
  ...
  var needsKugouMusicRefresh = kugouMusicLoginStatus.loggedIn && !hasCachedKugouMusicPlaylists;
  if (!force && !needsQQRefresh && !needsKugouRefresh && !needsKugouMusicRefresh && (userPlaylists.length || myPodcastCollections.length)) {
  ...
  try {
    var result = await Promise.all([
      ...
      kugouMusicLoginStatus.loggedIn ? apiJson('/api/kugou-music/user/playlists') : Promise.resolve({ playlists: [] })
    ]);
    ...
    kugouMusicPlaylists = (result[4].playlists || []).map(function(pl){ pl.provider = 'kugouMusic'; pl.source = 'kugouMusic'; return pl; });
    qishuiImportedPlaylists = readQishuiImportedPlaylists();
    userPlaylists = neteaseLists.concat(qqPlaylists, kugouPlaylists, kugouMusicPlaylists, qishuiImportedPlaylists);
...
function isPlaylistDetailSortMode(mode) {
  return PLAYLIST_DETAIL_SORT_OPTIONS.some(function(option){ return option.key === mode; });
}

function playlistDetailSortLabel(mode) {
  var option = PLAYLIST_DETAIL_SORT_OPTIONS.find(function(item){ return item.key === mode; });
  return option ? option.label : '收藏时间';
}
function playlistDetailCurrentProvider() {
  return normalizePlaylistProvider(playlistPanelDetailState && playlistPanelDetailState.playlist && playlistPanelDetailState.playlist.provider);
}
function playlistDetailSupportsPlatformPlayCounts(provider) {
  provider = normalizePlaylistProvider(provider);
  return provider === 'kugou' || provider === 'kugouMusic';
}
function effectivePlaylistDetailSortMode(provider) {
  provider = normalizePlaylistProvider(provider || playlistDetailCurrentProvider());
  if (playlistDetailSortMode === 'platformPlays' && !playlistDetailSupportsPlatformPlayCounts(provider)) return 'saved';
  return playlistDetailSortMode;
}
function playlistPlatformPlayCountEndpoint(provider) {
  provider = normalizePlaylistProvider(provider);
  if (provider === 'kugouMusic') return '/api/kugou-music/listen-counts';
  if (provider === 'kugou') return '/api/kugou/listen-counts';
  return '';
}
function playlistPlatformPlayCountProviderState(provider) {
  provider = normalizePlaylistProvider(provider);
  return playlistPlatformPlayCountState[provider] || null;
}
async function loadPlaylistPlatformPlayCounts(provider, force) {
  provider = normalizePlaylistProvider(provider);
  if (!playlistDetailSupportsPlatformPlayCounts(provider)) return null;
  var state = playlistPlatformPlayCountProviderState(provider);
  if (!state) return null;
  if (state.loaded && !force) return state;
  if (state.loading && state.promise) return state.promise;
  var endpoint = playlistPlatformPlayCountEndpoint(provider);
  if (!endpoint) return state;
  state.loading = true;
  state.error = '';
  state.promise = apiJson(endpoint + '?type=1&t=' + Date.now()).then(function(result){
    state.counts = (result && result.counts) || {};
    state.loaded = true;
    state.error = '';
    return state;
  }).catch(function(err){
    state.error = err && err.message || '平台播放次数同步失败';
...
function playlistDetailCleanTitle(value) {
  return String(value || '').replace(/\.(mp3|flac|wav|m4a|aac|ogg)$/i, '').trim();
}
function playlistDetailSplitFileTitle(value) {
  var title = playlistDetailCleanTitle(value);
  var match = title.match(/^(.+?)\s+[-－—–]\s+(.+)$/);
  if (!match) return null;
  var artist = String(match[1] || '').trim();
  var name = String(match[2] || '').trim();
  return artist && name ? { artist: artist, name: name } : null;
}
function setPlaylistDetailSortMode(mode) {
  if (!isPlaylistDetailSortMode(mode)) mode = 'saved';
  if (mode === 'platformPlays' && !playlistDetailSupportsPlatformPlayCounts(playlistDetailCurrentProvider())) {
    showToast('当前音源暂无平台播放次数');
    return;
  }
  if (playlistDetailSortMode === mode) return;
  playlistDetailSortMode = mode;
  try { localStorage.setItem(PLAYLIST_DETAIL_SORT_STORE_KEY, mode); } catch (e) {}
  var st = playlistPanelDetailState;
  if (st && st.key) {
    var total = st.tracks ? st.tracks.length : 0;
    st.renderLimit = Math.min(total, Math.max(PLAYLIST_DETAIL_INITIAL_RENDER, st.renderLimit || PLAYLIST_DETAIL_INITIAL_RENDER));
    renderPlaylistPanelDetailState();
  }
  showToast('歌单排序：' + playlistDetailSortLabel(mode));
}
function playlistDetailSongArtist(song) {
  if (!song) return '';
  if (song.artist) return String(song.artist).trim();
  if (Array.isArray(song.artists)) {
    return song.artists.map(function(artist){ return artist && (artist.name || artist.title || artist); }).filter(Boolean).join(' / ');
  }
  var split = playlistDetailSplitFileTitle(song.name || song.title || song.filename);
  if (split) return split.artist;
  return String(song.singername || song.singer || song.author_name || '').trim();
}
function playlistDetailSongName(song) {
  var raw = String(song && (song.name || song.title || song.filename) || '').trim();
  var split = playlistDetailSplitFileTitle(raw);
  return split ? split.name : playlistDetailCleanTitle(raw);
}
function playlistDetailCompareText(a, b) {
  return String(a || '').localeCompare(String(b || ''), 'zh-CN', { numeric: true, sensitivity: 'base' });
}
function playlistDetailMatchText(value) {
  return playlistDetailCleanTitle(value).toLowerCase().replace(/\s+/g, '').replace(/[·・]/g, '').trim();
}
function playlistDetailSongStatKeys(song) {
  if (!song) return [];
  var ids = [
    song.hash,
    song.albumAudioId,
    ...
    song.songmid,
    song.id
  ].filter(function(value){ return value != null && value !== ''; }).map(function(value){ return String(value); });
  var keys = [queueItemKey(song)];
  ids.forEach(function(id){
    keys.push('kugou:' + id, 'kugouMusic:' + id, 'qq:' + id, 'song:' + id);
  });
  return keys.filter(function(key, index){ return key && keys.indexOf(key) === index; });
}
function normalizePlaylistPanelTrack(song) {
  var cloned = cloneSong(song || {});
  var parsed = playlistDetailSplitFileTitle(cloned.name || cloned.title || cloned.filename);
  if (parsed) {
    if (!cloned.artist) cloned.artist = parsed.artist;
    if (!cloned.artists || !cloned.artists.length) cloned.artists = [{ id: cloned.artistId || '', name: cloned.artist }];
    cloned.name = parsed.name;
  } else {
    cloned.name = playlistDetailCleanTitle(cloned.name || cloned.title || cloned.filename);
  }
  return cloned;
}
function playlistDetailSongStats(song) {
  var songs = listenStatsState && listenStatsState.songs;
  if (!songs || !song) return null;
  var keys = playlistDetailSongStatKeys(song);
  for (var i = 0; i < keys.length; i++) {
    if (songs[keys[i]]) return songs[keys[i]];
  }
  var targetName = playlistDetailMatchText(playlistDetailSongName(song));
  var targetArtist = playlistDetailMatchText(playlistDetailSongArtist(song));
  if (!targetName) return null;
  var found = null;
  Object.keys(songs).some(function(key){
    var stat = songs[key] || {};
    var statSplit = playlistDetailSplitFileTitle(stat.name);
    var statName = playlistDetailMatchText(statSplit ? statSplit.name : stat.name);
    var statArtist = playlistDetailMatchText(stat.artist || (statSplit && statSplit.artist));
    if (statName !== targetName) return false;
    if (targetArtist && statArtist && targetArtist !== statArtist) return false;
    found = stat;
    return true;
  });
  return found;
}
function playlistDetailSongPlayCount(song) {
  var stat = playlistDetailSongStats(song) || {};
  return Number(stat.starts) || Number(stat.plays) || 0;
}
function playlistDetailPlatformPlayCountKeys(song, provider) {
  provider = normalizePlaylistProvider(provider || playlistDetailCurrentProvider());
  if (!song || !playlistDetailSupportsPlatformPlayCounts(provider)) return [];
  var ids = [
    song.hash,
    song.albumAudioId,
    song.album_audio_id,
  ...
  var keys = [];
  ids.forEach(function(id){
    keys.push(provider + ':' + id, 'song:' + id, 'hash:' + id.toLowerCase());
  });
  var nameKey = playlistDetailMatchText(playlistDetailSongName(song));
  var artistKey = playlistDetailMatchText(playlistDetailSongArtist(song));
  if (nameKey) keys.push('text:' + nameKey + '|' + artistKey);
  return keys.filter(function(key, index){ return key && keys.indexOf(key) === index; });
}
function playlistDetailSongPlatformPlayCount(song, provider) {
  provider = normalizePlaylistProvider(provider || playlistDetailCurrentProvider());
  var state = playlistPlatformPlayCountProviderState(provider);
  if (!state || !state.loaded || !state.counts) return null;
  var keys = playlistDetailPlatformPlayCountKeys(song, provider);
  for (var i = 0; i < keys.length; i++) {
    if (state.counts[keys[i]] != null) return Number(state.counts[keys[i]]) || 0;
  }
  return null;
}
function playlistDetailSongPlatformPlayCountLabel(song, provider) {
  provider = normalizePlaylistProvider(provider || playlistDetailCurrentProvider());
  var state = playlistPlatformPlayCountProviderState(provider);
  if (state && state.loading && !state.loaded) return '同步中';
  var count = playlistDetailSongPlatformPlayCount(song, provider);
  return count == null ? '暂无' : (count + ' 次');
}
function sortPlaylistTracksForDisplay(tracks, provider) {
  tracks = Array.isArray(tracks) ? tracks : [];
  provider = normalizePlaylistProvider(provider);
  var sortMode = effectivePlaylistDetailSortMode(provider);
  if (sortMode === 'saved') return tracks.slice().reverse();
  var rows = tracks.map(function(song, index){
    var stat = playlistDetailSongStats(song) || {};
    var platformPlays = playlistDetailSongPlatformPlayCount(song, provider);
    return {
      song: song,
      index: index,
      artist: playlistDetailSongArtist(song),
      name: playlistDetailSongName(song),
      plays: Number(stat.starts) || Number(stat.plays) || 0,
      platformPlays: Number(platformPlays) || 0,
      listenMs: Number(stat.listenMs) || 0,
      lastPlayedAt: Number(stat.lastPlayedAt || stat.lastStartedAt) || 0,
    };
  });
  if (sortMode === 'artist') {
    rows.sort(function(a, b){
      return playlistDetailCompareText(a.artist, b.artist)
        || playlistDetailCompareText(a.name, b.name)
        || (a.index - b.index);
    });
  } else if (sortMode === 'plays') {
    rows.sort(function(a, b){
      return (b.plays - a.plays)
        || (b.listenMs - a.listenMs)
        || (b.lastPlayedAt - a.lastPlayedAt)
        || (a.index - b.index);
    });
  } else if (sortMode === 'platformPlays') {
    rows.sort(function(a, b){
      return (b.platformPlays - a.platformPlays)
        || playlistDetailCompareText(a.artist, b.artist)
        || playlistDetailCompareText(a.name, b.name)
        || (a.index - b.index);
    });
  }
  return rows.map(function(row){ return row.song; });
}
function getPlaylistPanelSortedTracks() {
  var tracks = playlistPanelDetailState && playlistPanelDetailState.tracks ? playlistPanelDetailState.tracks : [];
  return sortPlaylistTracksForDisplay(tracks, playlistDetailCurrentProvider());
}
function playlistDetailSortButtonsHtml(provider) {
  provider = normalizePlaylistProvider(provider || playlistDetailCurrentProvider());
  var sortMode = effectivePlaylistDetailSortMode(provider);
  var options = PLAYLIST_DETAIL_SORT_OPTIONS.filter(function(option){
    return option.key !== 'platformPlays' || playlistDetailSupportsPlatformPlayCounts(provider);
  });
  return '<div class="pl-detail-sort" role="group" aria-label="歌单排序">' + options.map(function(option){
    var active = option.key === sortMode ? ' active' : '';
    return '<button class="fx-mini-btn ghost' + active + '" type="button" data-pl-detail-sort="' + escHtml(option.key) + '">' + escHtml(option.label) + '</button>';
  }).join('') + '</div>';
}
function normalizePlaylistProvider(provider) {
  return provider === 'qq' || provider === 'kugou' || provider === 'kugouMusic' || provider === 'qishui' || provider === 'local' ? provider : 'netease';
}
function playlistProviderShortLabel(provider) {
  provider = normalizePlaylistProvider(provider);
  return provider === 'qq' ? 'QQ' : (provider === 'kugou' ? 'KG' : (provider === 'kugouMusic' ? 'KGM' : (provider === 'qishui' ? 'QS' : (provider === 'local' ? 'LOCAL' : 'NE'))));
}
function playlistProviderLabel(provider) {
  provider = normalizePlaylistProvider(provider);
  return provider === 'qq' ? 'QQ 音乐' : (provider === 'kugou' ? '酷狗概念版' : (provider === 'kugouMusic' ? '酷狗音乐' : (provider === 'qishui' ? '汽水音乐' : (provider === 'local' ? '本地音乐库' : '网易云音乐'))));
}
function playlistCoverForProvider(pl, provider, size) {
  return pl && pl.cover ? coverUrlWithSize(pl.cover, size || 88) : '';
}
...
function playlistPanelProviderId(provider, id) {
  ...
  if (provider === 'kugouMusic') return 'kugouMusic:' + id;
...
function playlistPanelDetailHtml(pl, provider) {
  ...
  var sortMode = effectivePlaylistDetailSortMode(provider);
  var sortedTracks = loading ? [] : getPlaylistPanelSortedTracks();
  var cover = playlistCoverForProvider(pl, provider, 96);
  ...
  renderLimit = Math.min(sortedTracks.length, renderLimit);
  var visibleTracks = loading ? [] : sortedTracks.slice(0, renderLimit);
  var rows = loading
    ...
    : visibleTracks.map(function(song, i){
        ...
        var rowName = playlistDetailSongName(song) || song.name || '';
        var rowArtist = playlistDetailSongArtist(song) || '未知歌手';
        var playBadge = sortMode === 'plays'
          ? '<div class="pl-detail-count">' + playlistDetailSongPlayCount(song) + ' 次</div>'
          : (sortMode === 'platformPlays' ? '<div class="pl-detail-count">' + escHtml(playlistDetailSongPlatformPlayCountLabel(song, provider)) + '</div>' : '');
        return '<div class="pl-detail-row" data-pl-detail-row="' + i + '">' +
          imgTag +
          '<div style="flex:1;min-width:0"><div class="pl-detail-row-title">' + escHtml(rowName) + '</div>' +
          '<button type="button" class="pl-detail-row-artist" data-pl-detail-artist="' + i + '">' + escHtml(rowArtist) + '</button></div>' +
          playBadge +
        '</div>';
      }).join('');
  ...
  return '<div class="pl-inline-detail" data-pl-detail="' + escHtml(key) + '">' +
    '<div class="pl-detail-sticky">' +
      '<div class="pl-detail-head">' + img + '<div style="flex:1;min-width:0"><div class="pl-detail-title">' + escHtml(pl.name || '歌单详情') + '</div><div class="pl-detail-sub">' + escHtml((pl.trackCount || tracks.length || 0) + ' 首 · ' + (pl.creator || playlistProviderLabel(provider))) + '</div></div><div class="pl-detail-count">' + (loading ? '载入中' : (renderLimit + '/' + tracks.length)) + '</div></div>' +
      '<div class="pl-detail-actions"><button class="pl-detail-play" type="button" data-pl-detail-play="' + escHtml(key) + '"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>播放歌单</button><button class="fx-mini-btn ghost pl-detail-top-btn" type="button" data-pl-detail-top="1">回到顶部</button>' + playlistDetailSortButtonsHtml(provider) + '</div>' +
    '</div>' +
...
async function openPlaylistPanelDetail(provider, pid, title) {
  ...
  try {
    var r = provider === 'qishui'
      ? getQishuiImportedPlaylistTracks(pid)
      : (provider === 'local' && typeof SFLocalPlaylists !== 'undefined'
        ? SFLocalPlaylists.tracksFor(pid)
        : (provider === 'qq'
        ? await apiJson('/api/qq/playlist/tracks?id=' + encodeURIComponent(pid))
        : (provider === 'kugouMusic'
          ? await apiJson('/api/kugou-music/playlist/tracks?id=' + encodeURIComponent(pid))
          : (provider === 'kugou'
    ...
    playlistPanelDetailState.tracks = (r && r.tracks || []).map(normalizePlaylistPanelTrack);
    if (provider === 'kugou' && isLikedPlaylistContext(pid, title, r && r.playlist)) markSongsLiked(playlistPanelDetailState.tracks, true);
    playlistPanelDetailState.renderLimit = Math.min(playlistPanelDetailState.tracks.length, PLAYLIST_DETAIL_INITIAL_RENDER);
    syncLikeStatusForSongs(playlistPanelDetailState.tracks);
    renderPlaylistPanelDetailState();
    if (playlistDetailSupportsPlatformPlayCounts(provider)) {
      loadPlaylistPlatformPlayCounts(provider).then(function(){
        if (playlistPanelDetailState.token !== token || playlistPanelDetailState.key !== key) return;
        renderPlaylistPanelDetailState();
      });
    }
  } catch (e) {
...
function playPlaylistPanelDetail() {
  var st = playlistPanelDetailState;
  if (!st || !st.key) return;
  var tracks = getPlaylistPanelSortedTracks();
  if (tracks.length) {
    ...
    forcePlaybackControlsInteractive();
    playQueueAt(0, { manual: true }).catch(function(e){ console.warn('[PlaylistPanelDetailPlay]', e); });
    return;
  }
...
function playPlaylistPanelDetailTrack(index) {
  ...
  safeShelfRebuild('playlist-panel-detail', true);
  forcePlaybackControlsInteractive();
  playQueueAt(index, { manual: true }).catch(function(e){ console.warn('[PlaylistPanelDetailPlay]', e); });
}
function openPlaylistPanelDetailArtist(index) {
  var tracks = getPlaylistPanelSortedTracks();
  var song = tracks && tracks[index];
  if (song) openArtistDetailForSong(song);
}
...
function playlistPanelTotalCount() {
  return userPlaylists.length + (typeof SFLocalPlaylists !== 'undefined' ? SFLocalPlaylists.panelLists().length : 0);
}
function growPlaylistPanelRenderLimit() {
  var total = playlistPanelTotalCount();
  if (!total) return;
  var next = Math.min(total, (playlistPanelRenderLimit || PLAYLIST_PANEL_BATCH_SIZE) + PLAYLIST_PANEL_BATCH_SIZE);
...
function bindPlaylistPanelLazyRender() {
  ...
  panel.addEventListener('scroll', function(){
    ...
    if (queueViewTab !== 'playlists' || playlistPanelRenderLimit >= playlistPanelTotalCount()) return;
...
function renderUserPlaylistsList(opts) {
  ...
  var localPanelPlaylists = typeof SFLocalPlaylists !== 'undefined' ? SFLocalPlaylists.panelLists() : [];
  if (!userPlaylists.length && !localPanelPlaylists.length) {
  ...
  function playlistCardHtml(pl) {
    ...
    var providerLabel = playlistProviderShortLabel(provider);
    var thumb = playlistCoverForProvider(pl, provider, 88);
    ...
    // 平台歌单提供「导入」快照到本地;本地歌单提供「更新(重拉远端)/删除」
    var cardActions = '';
    if (provider !== 'local' && provider !== 'qishui' && pl.id) {
      cardActions = '<button type="button" class="pl-card-action" data-pl-import="' + escHtml(provider) + '|' + escHtml(String(pl.id)) + '" title="导入为本地歌单快照(登录状态拉全量曲目)">导入</button>';
    } else if (provider === 'local' && pl.remoteProvider && pl.remoteId) {
      cardActions = '<button type="button" class="pl-card-action" data-pl-update="' + escHtml(String(pl.id)) + '" title="按远端歌单重新拉取(需对应平台已登录)">更新</button>' +
        '<button type="button" class="pl-card-action danger" data-pl-remove="' + escHtml(String(pl.id)) + '" title="删除本地歌单快照">删</button>';
    } else if (provider === 'local' && pl.id && String(pl.id) !== 'local:liked') {
      cardActions = '<button type="button" class="pl-card-action danger" data-pl-remove="' + escHtml(String(pl.id)) + '" title="删除本地歌单快照">删</button>';
    }
  ...
  var groups = [
    { key:'local', label:'本地歌单', items:localPanelPlaylists },
    { key:'netease', label:'网易云歌单', items:userPlaylists.filter(function(pl){ return normalizePlaylistProvider(pl.provider) === 'netease'; }) },
    { key:'qq', label:'QQ 音乐歌单', items:userPlaylists.filter(function(pl){ return normalizePlaylistProvider(pl.provider) === 'qq'; }) },
    { key:'kugou', label:'酷狗概念版歌单', items:userPlaylists.filter(function(pl){ return normalizePlaylistProvider(pl.provider) === 'kugou'; }) },
    { key:'kugouMusic', label:'酷狗音乐歌单', items:userPlaylists.filter(function(pl){ return normalizePlaylistProvider(pl.provider) === 'kugouMusic'; }) },
    { key:'qishui', label:'汽水音乐歌单', items:userPlaylists.filter(function(pl){ return normalizePlaylistProvider(pl.provider) === 'qishui'; }) }
  ];
  if (opts.reset) resetPlaylistPanelRenderLimit();
  var playlistPanelTotal = userPlaylists.length + localPanelPlaylists.length;
  playlistPanelRenderLimit = Math.max(PLAYLIST_PANEL_BATCH_SIZE, Math.min(playlistPanelTotal, playlistPanelRenderLimit || PLAYLIST_PANEL_BATCH_SIZE));
  var renderedCount = 0;
  ...
  if (playlistPanelTotal > renderedCount) {
    $pl.insertAdjacentHTML('beforeend', '<button type="button" class="fx-mini-btn ghost pl-load-more" data-pl-load-more="1">加载更多 ' + renderedCount + '/' + playlistPanelTotal + '</button>');
  }
...
document.getElementById('pl-list').addEventListener('click', function(e){
  ...
  var detailSort = e.target && e.target.closest ? e.target.closest('[data-pl-detail-sort]') : null;
  if (detailSort) {
    e.preventDefault();
    e.stopPropagation();
    setPlaylistDetailSortMode(detailSort.getAttribute('data-pl-detail-sort'));
    return;
  }
  ...
  // 本地歌单:平台歌单「导入」快照 / 本地快照「更新」「删除」
  var importAction = e.target && e.target.closest ? e.target.closest('[data-pl-import]') : null;
  if (importAction) {
    e.preventDefault();
    e.stopPropagation();
    var importParts = String(importAction.getAttribute('data-pl-import') || '').split('|');
    var importCard = importAction.closest ? importAction.closest('.pl-card') : null;
    importRemotePlaylistSnapshot(importParts[0], importParts[1], importCard ? (importCard.getAttribute('data-playlist-title') || '') : '');
    return;
  }
  var updateAction = e.target && e.target.closest ? e.target.closest('[data-pl-update]') : null;
  if (updateAction) {
    e.preventDefault();
    e.stopPropagation();
    updateLocalPlaylistSnapshot(updateAction.getAttribute('data-pl-update') || '');
    return;
  }
  var removeAction = e.target && e.target.closest ? e.target.closest('[data-pl-remove]') : null;
  if (removeAction) {
    e.preventDefault();
    e.stopPropagation();
    removeLocalPlaylistSnapshot(removeAction.getAttribute('data-pl-remove') || '');
    return;
  }
...
async function loadPlaylistIntoQueueById(id, autoplay, title) {
  ...
  var kugouMusicPlaylistId = String(id || '').indexOf('kugouMusic:') === 0 ? String(id).slice(11) : '';
  var qishuiPlaylistId = String(id || '').indexOf('qishui:') === 0 ? String(id).slice(7) : '';
  var r = null;
  try {
    r = qishuiPlaylistId
      ? getQishuiImportedPlaylistTracks(qishuiPlaylistId)
      : (qqPlaylistId
        ? await apiJson('/api/qq/playlist/tracks?id=' + encodeURIComponent(qqPlaylistId))
        : (kugouMusicPlaylistId
          ? await apiJson('/api/kugou-music/playlist/tracks?id=' + encodeURIComponent(kugouMusicPlaylistId))
  ...
  try {
    ...
    if (!qqPlaylistId && !kugouPlaylistId && !kugouMusicPlaylistId && isLikedPlaylistContext(id, title, r.playlist)) markSongsLiked(playQueue, true);
    if (kugouPlaylistId && isLikedPlaylistContext(kugouPlaylistId, title, r.playlist)) markSongsLiked(playQueue, true);
    if (!qqPlaylistId && !qishuiPlaylistId) syncLikeStatusForSongs(playQueue);
    currentIdx = 0;
...
function updatePlaybackProgressUi() {
  if (window.StellaflixVideo && StellaflixVideo.player && StellaflixVideo.player.isOpen && StellaflixVideo.player.isOpen()) return;
...
function seekFromProgressPointer(e, emitParticles) {
  ...
  if (window.StellaflixVideo && StellaflixVideo.player && StellaflixVideo.player.isOpen && StellaflixVideo.player.isOpen()) {
    StellaflixVideo.player.seek(ratio);
...
var USER_SHAPE_PRESET_STORE_KEY = 'stellaflix-shape-presets-v1';
var ACTIVE_SHAPE_PRESET_STORE_KEY = 'stellaflix-active-shape-preset-v1';
var USER_SHAPE_PRESET_EXPORT_TYPE = 'stellaflix-shape-preset';
var USER_SHAPE_PRESET_SCHEMA = 1;
var SHAPE_PRIMITIVE_LIMIT = 12;
var SHAPE_TOTAL_PARTICLE_LIMIT = 6000;
var shapePrimitiveDefs = {
  point: { name: '点', desc: '粒子簇', count: 180, size: 0.90, depth: 0.22, rotation: 0.12, audioFollow: 0.42, opacity: 0.96, material: 'cover' },
  line: { name: '线', desc: '直线排布', count: 240, size: 0.86, depth: 0.12, rotation: 0.20, audioFollow: 0.30, opacity: 0.90, material: 'cover' },
  ring: { name: '环', desc: '圆形轨道', count: 280, size: 1.05, depth: 0.18, rotation: 0.26, audioFollow: 0.42, opacity: 0.94, material: 'cover' },
  curve: { name: '曲线环', desc: '参数曲线轨迹', count: 420, size: 1.08, depth: 0.42, rotation: 0.28, audioFollow: 0.46, opacity: 0.90, material: 'cover' },
  plane: { name: '面', desc: '平面网格', count: 400, size: 1.02, depth: 0.20, rotation: 0.10, audioFollow: 0.30, opacity: 0.88, material: 'cover' },
  spiral: { name: '螺旋', desc: '旋转排布', count: 460, size: 1.10, depth: 0.46, rotation: 0.36, audioFollow: 0.58, opacity: 0.90, material: 'cover' },
  wave: { name: '波', desc: '曲面起伏', count: 400, size: 1.02, depth: 0.34, rotation: 0.18, audioFollow: 0.64, opacity: 0.86, material: 'cover' },
  tunnel: { name: '隧道', desc: '深度环阵', count: 520, size: 1.14, depth: 0.66, rotation: 0.32, audioFollow: 0.48, opacity: 0.82, material: 'cover' },
  sphere: { name: '球面', desc: '球形粒子', count: 460, size: 1.08, depth: 0.46, rotation: 0.26, audioFollow: 0.40, opacity: 0.88, material: 'cover' },
  dust: { name: '星尘', desc: '深度星点', count: 560, size: 1.24, depth: 0.78, rotation: 0.20, audioFollow: 0.46, opacity: 0.84, material: 'cover' },
  card: { name: '卡片', desc: '底片平面', count: 120, size: 0.96, depth: 0.28, rotation: 0.14, audioFollow: 0.22, opacity: 0.90, material: 'cover' }
};
var shapePrimitiveOrder = ['point','line','ring','curve','plane','spiral','wave','tunnel','sphere','dust','card'];
var shapeStructureTemplates = {
  disc: {
    name: '唱片结构',
    desc: '环 + 中心面',
    primitives: [
      { type: 'ring', count: 300, size: 1.12, depth: 0.12, rotation: 0.28, audioFollow: 0.32 },
      { type: 'plane', count: 180, size: 0.72, depth: 0.06, rotation: 0.10, audioFollow: 0.18 }
    ]
  },
  galaxy: {
    name: '星河结构',
    desc: '螺旋 + 发光点',
    primitives: [
      { type: 'spiral', count: 520, size: 1.12, depth: 0.54, rotation: 0.36, audioFollow: 0.62 },
      { type: 'point', count: 180, size: 0.55, depth: 0.40, rotation: 0.18, audioFollow: 0.42 }
    ]
  },
  tunnel: {
    name: '隧道结构',
    desc: '环阵列推进',
    primitives: [
      { type: 'tunnel', count: 520, size: 1.08, depth: 0.72, rotation: 0.38, audioFollow: 0.48 },
      { type: 'ring', count: 180, size: 0.82, depth: 0.32, rotation: 0.20, audioFollow: 0.30 }
    ]
  },
  sphere: {
    name: '球面结构',
    desc: '球面 + 星尘',
    primitives: [
      { type: 'sphere', count: 520, size: 1.08, depth: 0.46, rotation: 0.26, audioFollow: 0.40 },
      { type: 'dust', count: 220, size: 0.88, depth: 0.62, rotation: 0.16, audioFollow: 0.34 }
    ]
  },
  wall: {
    name: '照片墙结构',
    desc: '卡片 + 平面',
    primitives: [
      { type: 'card', count: 96, size: 0.90, depth: 0.32, rotation: 0.16, audioFollow: 0.22 },
      { type: 'plane', count: 280, size: 1.05, depth: 0.22, rotation: 0.10, audioFollow: 0.26 }
    ]
  },
  curtain: {
    name: '粒子帘结构',
    desc: '面 + 波',
    primitives: [
      { type: 'plane', count: 360, size: 1.02, depth: 0.24, rotation: 0.08, audioFollow: 0.34 },
      { type: 'wave', count: 360, size: 1.00, depth: 0.36, rotation: 0.18, audioFollow: 0.64 }
    ]
  }
};
function shapePrimitiveDef(type) {
  return shapePrimitiveDefs[type] || shapePrimitiveDefs.point;
}
function shapePrimitiveSpawnPosition(index) {
  index = Math.max(0, Math.floor(Number(index) || 0));
  if (index === 0) return { x: 0, y: 0 };
  if (index === 1) return { x: 0.24, y: 0.08 };
  var angle = index * 2.399963229728653;
  var radius = Math.min(0.62, 0.18 + index * 0.055);
  return {
    x: clampRange(Math.cos(angle) * radius, -0.76, 0.76),
    y: clampRange(Math.sin(angle) * radius, -0.62, 0.62)
  };
}
function createShapePrimitive(type, raw) {
  var def = shapePrimitiveDef(type);
  raw = raw && typeof raw === 'object' ? raw : {};
  var currentCount = shapeWorkshopState && shapeWorkshopState.recipe && Array.isArray(shapeWorkshopState.recipe.primitives)
    ? shapeWorkshopState.recipe.primitives.length
    : 0;
  var spawn = shapePrimitiveSpawnPosition(raw.index != null ? raw.index : currentCount);
  if (raw.x == null) raw.x = spawn.x;
  if (raw.y == null) raw.y = spawn.y;
  return normalizeShapePrimitive(Object.assign({}, def, raw, {
    id: raw.id || (type + '-' + Date.now().toString(36) + '-' + Math.floor(Math.random() * 999).toString(36)),
    type: type
  }), currentCount);
}
function shapeRecipeNumber(raw, key, fallback, min, max) {
  var value = raw && raw[key] != null ? Number(raw[key]) : fallback;
  if (!isFinite(value)) value = fallback;
  return clampRange(value, min, max);
}
function shapePrimitiveCountMax(raw) {
  return 1200;
}
function normalizeShapePrimitive(raw, index) {
  raw = raw && typeof raw === 'object' ? raw : {};
  var type = shapePrimitiveDefs[raw.type] ? raw.type : 'point';
  var def = shapePrimitiveDef(type);
  var spawn = shapePrimitiveSpawnPosition(index);
  var countMax = shapePrimitiveCountMax(raw);
  return {
    id: String(raw.id || (type + '-' + index)).replace(/[^a-z0-9_-]/gi, '').slice(0, 36) || (type + '-' + index),
    type: type,
    x: shapeRecipeNumber(raw, 'x', spawn.x, -0.95, 0.95),
    y: shapeRecipeNumber(raw, 'y', spawn.y, -0.95, 0.95),
    z: shapeRecipeNumber(raw, 'z', def.z || 0, -0.95, 0.95),
    count: Math.round(shapeRecipeNumber(raw, 'count', def.count, 8, countMax)),
    size: shapeRecipeNumber(raw, 'size', def.size, 0.2, 1.8),
    depth: shapeRecipeNumber(raw, 'depth', def.depth, 0, 1.2),
    rotation: shapeRecipeNumber(raw, 'rotation', def.rotation, 0, 1.2),
    audioFollow: shapeRecipeNumber(raw, 'audioFollow', def.audioFollow, 0, 1),
    opacity: shapeRecipeNumber(raw, 'opacity', def.opacity == null ? 0.86 : def.opacity, 0.08, 1),
    material: /^(cover|accent|mono)$/.test(String(raw.material || def.material || 'cover')) ? String(raw.material || def.material || 'cover') : 'cover'
  };
}
function normalizeShapeRecipeName(name) {
  name = String(name || '').replace(/\s+/g, ' ').trim();
  return (name || '我的形态').slice(0, 28);
}
function normalizeShapeRecipe(raw) {
  raw = raw && typeof raw === 'object' ? raw : {};
  var primitives = Array.isArray(raw.primitives) ? raw.primitives.slice(0, SHAPE_PRIMITIVE_LIMIT) : [];
  primitives = primitives.map(normalizeShapePrimitive);
  var total = primitives.reduce(function(sum, p){ return sum + p.count; }, 0);
  if (total > SHAPE_TOTAL_PARTICLE_LIMIT) {
    var ratio = SHAPE_TOTAL_PARTICLE_LIMIT / total;
    primitives = primitives.map(function(p){
      p.count = Math.max(8, Math.round(p.count * ratio));
      return p;
    });
  }
  var stability = raw.stability && typeof raw.stability === 'object' ? raw.stability : {};
  return {
    type: USER_SHAPE_PRESET_EXPORT_TYPE,
    version: USER_SHAPE_PRESET_SCHEMA,
    name: normalizeShapeRecipeName(raw.name),
    primitives: primitives,
    materialBinding: {
      mode: /^(cover-texture|cover-color|text-fill)$/.test(String(raw.materialBinding && raw.materialBinding.mode || 'cover-texture')) ? String(raw.materialBinding && raw.materialBinding.mode || 'cover-texture') : 'cover-texture',
      colorSource: /^(cover|manual)$/.test(String(raw.materialBinding && raw.materialBinding.colorSource || 'cover')) ? String(raw.materialBinding && raw.materialBinding.colorSource || 'cover') : 'cover'
    },
    camera: {
      mode: /^(static|slow-orbit|stage)$/.test(String(raw.camera && raw.camera.mode || 'slow-orbit')) ? String(raw.camera && raw.camera.mode || 'slow-orbit') : 'slow-orbit',
      distance: clampRange(Number(raw.camera && raw.camera.distance) || 3.2, 1.8, 6.5)
    },
    stability: {
      driftLock: stability.driftLock === true
    }
  };
}
function defaultShapeRecipe() {
  return normalizeShapeRecipe({
    name: '我的形态',
    primitives: []
  });
}
function cloneShapeRecipe(recipe) {
  return normalizeShapeRecipe(JSON.parse(JSON.stringify(recipe || defaultShapeRecipe())));
}
function shapeRecipeTotalParticles(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  return recipe.primitives.reduce(function(sum, p){ return sum + p.count; }, 0);
}
function shapePrimitiveLabel(primitive) {
  var def = shapePrimitiveDef(primitive && primitive.type);
  return def.name;
}
function shapeRecipeSummary(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  return recipe.primitives.length + ' 层 · ' + shapeRecipeTotalParticles(recipe) + ' 粒子';
}
function shapePrimitiveIsCoverSurface(primitive) {
  primitive = normalizeShapePrimitive(primitive || {}, 0);
  return primitive.material === 'cover' && (primitive.type === 'plane' || primitive.type === 'card');
}
function shapePrimitiveMaterialLabel(primitive) {
  primitive = normalizeShapePrimitive(primitive || {}, 0);
  if (shapePrimitiveIsCoverSurface(primitive)) return '承载封面';
  if (primitive.material === 'cover') return '取封面色';
  if (primitive.material === 'accent') return '主题色';
  return '单色粒子';
}
function shapePrimitiveMaterialTagClass(primitive) {
  primitive = normalizeShapePrimitive(primitive || {}, 0);
  if (shapePrimitiveIsCoverSurface(primitive)) return 'cover-surface';
  if (primitive.material === 'cover') return 'cover-color';
  return 'neutral';
}
function shapeRecipeCoverSurfaceLayers(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  return recipe.primitives.map(function(p, index){
    return { primitive: p, index: index };
  }).filter(function(entry){
    return shapePrimitiveIsCoverSurface(entry.primitive);
  });
}
function shapeRecipeCoverColorLayerCount(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  return recipe.primitives.filter(function(p){ return p.material === 'cover'; }).length;
}
function shapeRecipeCoverLandingLabel(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  if (!recipe.primitives.length) return '尚未添加层级';
  if (!customShapeUsesCoverTexture(recipe)) return '当前形态未使用封面底片';
  var surfaceLayers = shapeRecipeCoverSurfaceLayers(recipe);
  if (surfaceLayers.length) {
    return surfaceLayers.map(function(entry){
      return '第' + (entry.index + 1) + '层 ' + shapePrimitiveLabel(entry.primitive);
    }).join(' / ');
  }
  return '自动后置封面粒子底板';
}
function shapeRecipeMaterialSourceLabel() {
  return shapeMaterialMediaLabel(currentShapeMaterialMedia());
}
function renderShapeMaterialMap(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  var coverLayers = shapeRecipeCoverColorLayerCount(recipe);
  var modeLabel = recipe.materialBinding.mode === 'cover-texture' ? '封面粒子 + 封面取色' : (recipe.materialBinding.mode === 'cover-color' ? '仅封面取色' : '文字/底片填充');
  var note = shapeRecipeCoverSurfaceLayers(recipe).length
    ? '面/卡片层会生成额外封面粒子，让封面或上传图片更容易被看出来。'
    : '没有面/卡片层时，系统会在形态后方自动补一个封面粒子底板；环、线、星河等层主要负责取色。';
  return '<div class="shape-material-map">' +
    '<div class="shape-material-map-row"><b>底片来源</b><span>' + escHtml(shapeRecipeMaterialSourceLabel()) + '</span></div>' +
    '<div class="shape-material-map-row"><b>封面落点</b><span>' + escHtml(shapeRecipeCoverLandingLabel(recipe)) + '</span></div>' +
    '<div class="shape-material-map-row"><b>绑定方式</b><span>' + escHtml(modeLabel) + '</span></div>' +
    '<div class="shape-material-map-row"><b>取色层数</b><span>' + coverLayers + ' 层</span></div>' +
    '<div class="shape-material-map-note">' + escHtml(note) + '</div>' +
  '</div>';
}
function isExcludedGeneratedShapePreset(slot) {
  slot = slot && typeof slot === 'object' ? slot : {};
  var recipe = normalizeShapeRecipe(slot.recipe || slot);
  var blockedName = '\u661f\u6cb3' + '\u83ab\u6bd4' + '\u4e4c\u65af';
  if (normalizeShapeRecipeName(slot.name || recipe.name) === blockedName) return true;
  var blockedIdParts = ['mo' + 'bius-cover-nebula', 'mo' + 'bius-infinity-ring', 'mo' + 'bius-galaxy'];
  return recipe.primitives.some(function(p){
    var id = String(p && p.id || '');
    return blockedIdParts.some(function(part){ return id.indexOf(part) >= 0; });
  });
}
function readUserShapePresets() {
  var raw = [];
  try {
    raw = JSON.parse(localStorage.getItem(USER_SHAPE_PRESET_STORE_KEY) || '[]') || [];
  } catch (e) {
    raw = [];
  }
  ...
  return raw.map(function(slot, index){
    ...
    var recipe = normalizeShapeRecipe(slot.recipe || slot);
    return {
      name: normalizeShapeRecipeName(slot.name || recipe.name || ('我的形态 ' + (index + 1))),
      savedAt: Number(slot.savedAt) || Date.now(),
      recipe: recipe
    };
  }).filter(function(slot){ return !!slot.recipe && !isExcludedGeneratedShapePreset(slot); });
}
function saveUserShapePresets() {
  try {
    localStorage.setItem(USER_SHAPE_PRESET_STORE_KEY, JSON.stringify(userShapePresets));
  } catch (e) {
    showToast('形态保存失败，本地存储空间可能不足');
  }
}
function shapePresetExportPayload(slot) {
  var recipe = normalizeShapeRecipe(slot && slot.recipe);
  return {
    type: USER_SHAPE_PRESET_EXPORT_TYPE,
    schema: USER_SHAPE_PRESET_SCHEMA,
    exportedAt: Date.now(),
    name: normalizeShapeRecipeName(slot && slot.name || recipe.name),
    savedAt: Number(slot && slot.savedAt) || Date.now(),
    recipe: recipe
  };
}
function safeShapePresetFileName(name) {
  return String(name || 'Stellaflix 形态预设').replace(/[\\/:*?"<>|]+/g, '-').slice(0, 48) + '.stellaflix-shape.json';
}
function exportUserShapePreset(index) {
  index = Math.floor(Number(index) || 0);
  var slot = userShapePresets[index];
  if (!slot || !slot.recipe || !slot.recipe.primitives || !slot.recipe.primitives.length) {
    showToast('空白形态不能导出');
    return;
  }
  var payload = shapePresetExportPayload(slot);
  var text = JSON.stringify(payload, null, 2);
  var api = getDesktopWindowApi && getDesktopWindowApi();
  if (api && typeof api.exportJsonFile === 'function') {
    api.exportJsonFile({ defaultName: safeShapePresetFileName(slot.name), text: text }).then(function(res){
      if (res && res.ok) showToast('形态预设已导出');
      else if (!res || !res.canceled) showToast('形态预设导出失败');
    }).catch(function(){ showToast('形态预设导出失败'); });
    return;
  }
  ...
  a.href = url;
  a.download = safeShapePresetFileName(slot.name);
  a.click();
  setTimeout(function(){ URL.revokeObjectURL(url); }, 1000);
}
function normalizeImportedShapePresetPayload(payload, fileName) {
  if (!payload || typeof payload !== 'object') return null;
  var rawRecipe = payload.recipe || payload.shapeRecipe || payload;
  var recipe = normalizeShapeRecipe(rawRecipe);
  if (!recipe.primitives.length) return null;
  var baseName = String(fileName || '').split(/[\\/]/).pop().replace(/\.stellaflix-shape\.json$/i, '').replace(/\.json$/i, '');
  var name = normalizeShapeRecipeName(payload.name || (rawRecipe && rawRecipe.name) || baseName || recipe.name);
  recipe.name = name;
  return {
    name: name,
...
function importUserShapePresetText(text, fileName) {
  var payload = null;
  try { payload = JSON.parse(String(text || '')); } catch (e) {}
  var slot = normalizeImportedShapePresetPayload(payload, fileName);
  if (!slot) {
    showToast('导入失败，文件不是有效的形态预设');
    return false;
  }
  userShapePresets.push(slot);
  shapeWorkshopState.open = true;
  shapeWorkshopState.recipe = cloneShapeRecipe(slot.recipe);
  shapeWorkshopState.selectedId = shapeWorkshopState.recipe.primitives[0] ? shapeWorkshopState.recipe.primitives[0].id : '';
  shapeWorkshopState.editingIndex = userShapePresets.length - 1;
  shapeWorkshopState.deleteConfirmIndex = -1;
  shapeWorkshopState.motionPreview = false;
  shapeWorkshopResetHistory();
  shapeWorkshopClearPreviewFocus();
  saveUserShapePresets();
  enterShapeWorkshopRenderMode();
  buildPresetGrid();
  renderShapeWorkshop();
  showToast('已导入形态: ' + slot.name);
  return true;
}
function importUserShapePresetFromDialog() {
  var api = getDesktopWindowApi && getDesktopWindowApi();
  if (api && typeof api.importJsonFile === 'function') {
    api.importJsonFile().then(function(res){
      if (res && res.ok) importUserShapePresetText(res.text, res.filePath || '形态预设.json');
  ...
  input.type = 'file';
  input.accept = '.stellaflix-shape.json,.json,application/json';
  input.onchange = function(){
    var file = input.files && input.files[0];
    if (file) readUserShapePresetImportFile(file);
  };
  input.click();
}
function readUserShapePresetImportFile(file) {
  if (!file || !/\.json$/i.test(file.name || '')) {
    showToast('请导入 JSON 形态预设');
    return;
  }
  var reader = new FileReader();
  reader.onload = function(e){ importUserShapePresetText(e.target && e.target.result, file.name); };
  reader.onerror = function(){ showToast('导入失败'); };
  reader.readAsText(file, 'utf-8');
}
var userShapePresets = readUserShapePresets();
var shapeWorkshopState = {
  open: false,
  recipe: defaultShapeRecipe(),
  selectedId: '',
  previewPrevPreset: null,
  motionPreview: false,
  soloId: '',
  hiddenIds: {},
  editingIndex: -1,
...
shapeWorkshopState.selectedId = shapeWorkshopState.recipe.primitives[0] ? shapeWorkshopState.recipe.primitives[0].id : '';
var activeUserShapeRecipe = readActiveUserShapeRecipe();
var customShapeGroup = null;
var customShapeRecipeKey = '';
var customShapeRenderRecipe = null;

function readActiveUserShapeRecipe() {
  var raw = null;
  try {
    raw = JSON.parse(localStorage.getItem(ACTIVE_SHAPE_PRESET_STORE_KEY) || 'null');
  } catch (e) {
    raw = null;
  }
  var recipe = raw && raw.recipe ? raw.recipe : raw;
  recipe = recipe ? normalizeShapeRecipe(recipe) : null;
  if (recipe && isExcludedGeneratedShapePreset(recipe)) {
    try { localStorage.removeItem(ACTIVE_SHAPE_PRESET_STORE_KEY); } catch (e) {}
    return null;
  }
  return recipe && recipe.primitives && recipe.primitives.length ? recipe : null;
}
function saveActiveUserShapeRecipe(recipe) {
  try {
    if (!recipe || !recipe.primitives || !recipe.primitives.length) {
      localStorage.removeItem(ACTIVE_SHAPE_PRESET_STORE_KEY);
      return;
    }
    localStorage.setItem(ACTIVE_SHAPE_PRESET_STORE_KEY, JSON.stringify({
      appliedAt: Date.now(),
      recipe: cloneShapeRecipe(recipe)
    }));
  } catch (e) {
    showToast('当前形态应用状态保存失败，本地存储空间可能不足');
  }
}
function shapeRecipeRenderKey(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  return JSON.stringify({
    version: recipe.version,
    name: recipe.name,
    primitives: recipe.primitives,
    materialBinding: recipe.materialBinding,
    camera: recipe.camera,
    stability: recipe.stability
  });
}
function shapeRecipeDriftLockEnabled(recipe) {
  recipe = recipe ? normalizeShapeRecipe(recipe) : null;
  return !!(recipe && recipe.stability && recipe.stability.driftLock);
}
function isCustomShapeLayerDriftLocked(recipe) {
  return shapeRecipeDriftLockEnabled(recipe);
}
function shapeWorkshopHiddenIds() {
  if (!shapeWorkshopState.hiddenIds || typeof shapeWorkshopState.hiddenIds !== 'object') shapeWorkshopState.hiddenIds = {};
  return shapeWorkshopState.hiddenIds;
}
function shapeWorkshopClearPreviewFocus() {
  shapeWorkshopState.soloId = '';
  shapeWorkshopState.hiddenIds = {};
}
function shapeWorkshopCleanPreviewFocus(recipe) {
  recipe = normalizeShapeRecipe(recipe || shapeWorkshopState.recipe);
  var valid = {};
  recipe.primitives.forEach(function(p){ valid[p.id] = true; });
  var nextHidden = {};
  Object.keys(shapeWorkshopHiddenIds()).forEach(function(id){
    if (valid[id]) nextHidden[id] = true;
  });
  shapeWorkshopState.hiddenIds = nextHidden;
  if (shapeWorkshopState.soloId && !valid[shapeWorkshopState.soloId]) shapeWorkshopState.soloId = '';
  return recipe;
}
function shapeWorkshopResetHistory() {
  shapeWorkshopState.undoStack = [];
  shapeWorkshopState.redoStack = [];
  shapeWorkshopState.activeParamUndoKey = '';
}
function shapeWorkshopHistoryEntry() {
  var recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  return {
    key: shapeRecipeRenderKey(recipe),
    recipe: cloneShapeRecipe(recipe),
    selectedId: shapeWorkshopState.selectedId || ''
  };
}
function shapeWorkshopPushUndo() {
  shapeWorkshopState.undoStack = Array.isArray(shapeWorkshopState.undoStack) ? shapeWorkshopState.undoStack : [];
  shapeWorkshopState.redoStack = Array.isArray(shapeWorkshopState.redoStack) ? shapeWorkshopState.redoStack : [];
  var entry = shapeWorkshopHistoryEntry();
  var last = shapeWorkshopState.undoStack[shapeWorkshopState.undoStack.length - 1];
  if (!last || last.key !== entry.key) {
    shapeWorkshopState.undoStack.push(entry);
    if (shapeWorkshopState.undoStack.length > 40) shapeWorkshopState.undoStack.shift();
  }
  shapeWorkshopState.activeParamUndoKey = '';
  shapeWorkshopState.redoStack = [];
}
function shapeWorkshopRestoreHistoryEntry(entry) {
  if (!entry || !entry.recipe) return;
  var recipe = shapeWorkshopCleanPreviewFocus(cloneShapeRecipe(entry.recipe));
  var selectedExists = recipe.primitives.some(function(p){ return p.id === entry.selectedId; });
  shapeWorkshopState.recipe = recipe;
  shapeWorkshopState.selectedId = selectedExists ? entry.selectedId : (recipe.primitives[0] ? recipe.primitives[0].id : '');
  shapeWorkshopState.deleteConfirmIndex = -1;
  shapeWorkshopState.activeParamUndoKey = '';
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
}
function shapeWorkshopUndo() {
  shapeWorkshopState.undoStack = Array.isArray(shapeWorkshopState.undoStack) ? shapeWorkshopState.undoStack : [];
  shapeWorkshopState.redoStack = Array.isArray(shapeWorkshopState.redoStack) ? shapeWorkshopState.redoStack : [];
  var entry = shapeWorkshopState.undoStack.pop();
  if (!entry) {
    showToast('没有可撤销的操作');
    return;
  }
  shapeWorkshopState.redoStack.push(shapeWorkshopHistoryEntry());
  shapeWorkshopRestoreHistoryEntry(entry);
  showToast('已撤销');
}
function shapeWorkshopRedo() {
  shapeWorkshopState.undoStack = Array.isArray(shapeWorkshopState.undoStack) ? shapeWorkshopState.undoStack : [];
  shapeWorkshopState.redoStack = Array.isArray(shapeWorkshopState.redoStack) ? shapeWorkshopState.redoStack : [];
  var entry = shapeWorkshopState.redoStack.pop();
  if (!entry) {
    showToast('没有可重做的操作');
    return;
  }
  shapeWorkshopState.undoStack.push(shapeWorkshopHistoryEntry());
  shapeWorkshopRestoreHistoryEntry(entry);
  showToast('已重做');
}
function shapeWorkshopEnsureParamUndo(id, key) {
  var editKey = String(id || '') + ':' + String(key || '');
  if (shapeWorkshopState.activeParamUndoKey === editKey) return;
  shapeWorkshopPushUndo();
  shapeWorkshopState.activeParamUndoKey = editKey;
}
function shapeWorkshopFinishParamEdit() {
  shapeWorkshopState.activeParamUndoKey = '';
  renderShapeWorkshop();
}
function shapeWorkshopIsPrimitiveHidden(id) {
  return !!shapeWorkshopHiddenIds()[id];
}
function shapeWorkshopPreviewPrimitives(recipe) {
  recipe = shapeWorkshopCleanPreviewFocus(recipe);
  var soloId = String(shapeWorkshopState.soloId || '');
  if (soloId) {
    var solo = recipe.primitives.filter(function(p){ return p.id === soloId; });
    if (solo.length) return solo;
  }
  var hidden = shapeWorkshopHiddenIds();
  return recipe.primitives.filter(function(p){ return !hidden[p.id]; });
}
function shapeWorkshopPreviewRecipeForRender() {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  return normalizeShapeRecipe({
    name: recipe.name,
    primitives: shapeWorkshopPreviewPrimitives(recipe),
    materialBinding: recipe.materialBinding,
    camera: recipe.camera,
    stability: recipe.stability
  });
}
function shapeWorkshopPreviewStatus(recipe) {
  recipe = shapeWorkshopCleanPreviewFocus(recipe);
  if (!recipe.primitives.length) return '空白形态';
  if (shapeWorkshopState.soloId) {
    var soloIndex = recipe.primitives.findIndex(function(p){ return p.id === shapeWorkshopState.soloId; });
    if (soloIndex >= 0) return '正在单看第 ' + (soloIndex + 1) + ' 层';
  }
  var hiddenCount = Object.keys(shapeWorkshopHiddenIds()).length;
  if (hiddenCount) return '已隐藏 ' + hiddenCount + ' 层';
  return '全部层可见';
}
// [moved -> 08-fx-visual.js · region D]
function refreshCustomShapeColorsFromCover() {
  if (!isCustomShapeRenderActive()) return;
  syncCustomShapeLayer(true);
}
function enterShapeWorkshopRenderMode() {
  if (shapeWorkshopState.previewPrevPreset == null && fx) shapeWorkshopState.previewPrevPreset = Number(fx.preset) || 0;
  if (typeof setPreset === 'function') setPreset(3, { silent: true, preserveCamera: false, skipTransition: true, noSave: true, keepCustomShape: true });
  syncCustomShapeLayer(true);
}
function exitShapeWorkshopRenderMode() {
  var previous = shapeWorkshopState.previewPrevPreset;
  shapeWorkshopState.previewPrevPreset = null;
  if (!activeUserShapeRecipe && previous != null && typeof setPreset === 'function' && fx && Number(fx.preset) === 3) {
    setPreset(previous, { silent: true, preserveCamera: false, skipTransition: true, noSave: true, keepCustomShape: true });
  }
  syncCustomShapeLayer(true);
}
function applyShapeRecipeToPlayer(recipe, silent) {
  recipe = normalizeShapeRecipe(recipe);
  if (!recipe.primitives.length) {
    showToast('先添加基础粒子，再应用到播放器');
    return false;
  }
  activeUserShapeRecipe = cloneShapeRecipe(recipe);
  saveActiveUserShapeRecipe(activeUserShapeRecipe);
  if (typeof setPreset === 'function') setPreset(3, { silent: true, preserveCamera: false, skipTransition: true, keepCustomShape: true, commitPlaybackPreset: true });
  syncCustomShapeLayer(true);
  refreshPresetGrid();
  if (!silent) showToast('已应用 3D 形态: ' + activeUserShapeRecipe.name);
  return true;
}
function clearActiveUserShapePreset(opts) {
  opts = opts || {};
  activeUserShapeRecipe = null;
  saveActiveUserShapeRecipe(null);
  if (!opts.skipRender) syncCustomShapeLayer(true);
}
var USER_FX_ARCHIVE_STORE_KEY = 'stellaflix-user-fx-archives-v1';
var USER_FX_ARCHIVE_EXPORT_TYPE = 'stellaflix-user-fx-archive';
var USER_FX_WORK_EXPORT_TYPE = 'stellaflix-visual-work';
...
function normalizeUserFxArchiveDescription(text) {
...
function normalizeFxArchiveSnapshot(raw) {
  ...
  return {
    ...
    shapeMaterialMedia: normalizeShapeMaterialMedia(raw.shapeMaterialMedia),
    ...
    lyricNextLine: raw.lyricNextLine !== false,
...
function readUserFxArchives() {
  ...
  return raw.map(function(slot, index){
    ...
    return {
      ...
      description: normalizeUserFxArchiveDescription(slot.description || slot.desc || ''),
...
function createPackagedDefaultUserFxArchiveSlot() {
  return {
    ...
    description: normalizeUserFxArchiveDescription('内置示例作品，可作为 DIY 沙盒的起点。'),
...
function applyFxArchiveSnapshot(snapshot) {
  ...
  loadShapeMaterialCanvas(fx.shapeMaterialMedia, true);
...
// [load-order fix] readUserFxArchives() -> normalizeFxArchiveSnapshot() calls a cluster of helpers
// (normalizeHexColor / normalizeLyricFontKey / normalizeShapeMaterialMedia / normalizeDesktopLyricsFps /
// normalizePerformanceBackgroundMode / normalizePerformanceQuality) that live in 04c-lyrics.js, which
// loads AFTER music.js. Running it at top-level threw `X is not defined` and aborted the ENTIRE boot
// (no home page, all functions dead). Defer the read to DOMContentLoaded — by then every classic script
// (incl. 04c/08) has executed and all helpers are global. The packaged-default bootstrap below also needs
// 08-owned globals, so it shares this same deferred path.
var userFxArchives = [];
function loadUserFxArchives() {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', loadUserFxArchives);
} else {
  loadUserFxArchives();
}
...
function userFxArchivePresetName(snapshot) {
  var preset = snapshot && presetMeta[Math.max(0, Math.min(presetMeta.length - 1, Number(snapshot.preset) || 0))];
  return preset && preset.name ? preset.name : '当前形态';
}
function userFxArchiveMaterialLabel(snapshot) {
  var media = snapshot && normalizeShapeMaterialMedia(snapshot.shapeMaterialMedia);
  if (media) return '上传图片底片';
  return '当前歌曲封面';
}
function userFxArchiveAutoDescription(slot) {
  var snapshot = slot && slot.snapshot;
  if (!snapshot) return '空白作品，保存后会记录当前视觉积木。';
  return userFxArchiveMaterialLabel(snapshot) + ' · ' + userFxArchivePresetName(snapshot) + ' · ' +
    (snapshot.floatLayer ? '浮空粒子' : '基础粒子') + (snapshot.bloom ? ' · 光晕' : '');
}
function userFxArchiveDescription(slot) {
  return normalizeUserFxArchiveDescription(slot && slot.description) || userFxArchiveAutoDescription(slot);
}
function fxArchiveCreativeRecipe(snapshot) {
  snapshot = normalizeFxArchiveSnapshot(snapshot);
  if (!snapshot) return null;
  return {
    type: 'stellaflix-visual-recipe',
    version: 1,
    material: {
      source: normalizeShapeMaterialMedia(snapshot.shapeMaterialMedia) ? 'uploaded-shape-material' : 'current-cover'
    },
    shape: {
      preset: snapshot.preset,
      name: userFxArchivePresetName(snapshot)
    },
    layers: {
      floatLayer: !!snapshot.floatLayer,
      bloom: !!snapshot.bloom,
      edge: !!snapshot.edge,
      lyricGlowParticles: !!snapshot.lyricGlowParticles
    },
    motion: {
      intensity: snapshot.intensity,
      speed: snapshot.speed,
      camera: snapshot.cinema ? 'cinema' : 'static'
    }
  };
}
function renderUserFxArchives() {
  ...
  var toolbar =
    '<div class="user-archive-toolbar">' +
      '<div class="user-archive-note">我的作品会保存当前素材、形态、粒子、歌词和镜头参数；支持拖拽 JSON 导入，也可以导出分享。</div>' +
  ...
  var cards = userFxArchives.map(function(slot, index){
    ...
    var actionsHtml = editing
      ...
      : '<button type="button" onclick="applyUserFxArchive(' + index + ')"' + (hasSave ? '' : ' disabled') + '>应用</button>' +
        '<button type="button" onclick="saveUserFxArchive(' + index + ')">保存</button>' +
        '<button type="button" onclick="renameUserFxArchive(' + index + ')">命名</button>' +
        '<button type="button" onclick="duplicateUserFxArchive(' + index + ')"' + (hasSave ? '' : ' disabled') + '>复制</button>' +
    ...
    return '<div class="user-archive-slot' + (hasSave ? ' has-save' : '') + '" data-slot="' + index + '">' +
      '<div class="user-archive-preview"></div>' +
      nameHtml +
      '<div class="user-archive-description">' + escHtml(userFxArchiveDescription(slot)) + '</div>' +
      '<div class="user-archive-meta">' + (hasSave ? formatUserArchiveTime(slot.savedAt) : '空白作品，点击保存写入当前视觉') + '</div>' +
      '<div class="user-archive-actions">' + actionsHtml + '</div>' +
    '</div>';
  }).join('');
  var addCard = '<button class="user-archive-slot is-new" type="button" onclick="createUserFxArchive()"><strong>＋ 新建作品</strong><span class="user-archive-meta">先搭积木，再保存成作品</span></button>';
...
function saveCurrentAsUserFxArchive() {
  var index = userFxArchives.length;
  var now = Date.now();
  var slot = {
    name: normalizeUserFxArchiveName(creativeSandboxPresetName() + ' 作品', index),
    description: '',
    createdAt: now,
    savedAt: now,
...
function saveUserFxArchive(index) {
  ...
  slot.description = normalizeUserFxArchiveDescription(slot.description);
  saveUserFxArchives();
  renderUserFxArchives();
...
function duplicateUserFxArchive(index) {
  var slot = userFxArchiveAt(index);
  if (!slot || !slot.snapshot) {
    showToast('空白作品不能复制');
    return;
  }
  var nextIndex = userFxArchives.length;
  userFxArchives.push({
    name: normalizeUserFxArchiveName(slot.name + ' 副本', nextIndex),
    description: normalizeUserFxArchiveDescription(slot.description),
    createdAt: Date.now(),
    savedAt: slot.savedAt || Date.now(),
    snapshot: normalizeFxArchiveSnapshot(slot.snapshot)
  });
  saveUserFxArchives();
  renderUserFxArchives();
  showToast('已复制为新作品');
...
function userFxArchiveExportPayload(slot) {
  return {
    type: USER_FX_WORK_EXPORT_TYPE,
    legacyType: USER_FX_ARCHIVE_EXPORT_TYPE,
    ...
    creativeRecipe: fxArchiveCreativeRecipe(slot.snapshot)
  };
}
function safeArchiveFileName(name) {
  return String(name || 'Stellaflix 视觉作品').replace(/[\\/:*?"<>|]+/g, '-').slice(0, 48) + '.stellaflix-visual.json';
}
function exportUserFxArchive(index) {
  ...
  if (api && typeof api.exportJsonFile === 'function') {
    api.exportJsonFile({ defaultName: safeArchiveFileName(slot.name), text: text }).then(function(res){
      if (res && res.ok) showToast('视觉作品已导出');
      else if (!res || !res.canceled) showToast('视觉作品导出失败');
    }).catch(function(){ showToast('视觉作品导出失败'); });
    return;
  }
  var blob = new Blob([text], { type: 'application/json;charset=utf-8' });
  var url = URL.createObjectURL(blob);
...
function normalizeImportedFxArchivePayload(payload, fileName) {
  ...
  if (!snapshot) return null;
  var baseName = String(fileName || '').split(/[\\/]/).pop().replace(/\.stellaflix-visual\.json$/i, '').replace(/\.json$/i, '');
  return {
    name: normalizeUserFxArchiveName(payload.name || baseName, userFxArchives.length),
    description: normalizeUserFxArchiveDescription(payload.description || payload.desc || ''),
    createdAt: Date.now(),
    savedAt: Number(payload.savedAt) || Date.now(),
    snapshot: snapshot
  };
...
function importUserFxArchiveFromDialog() {
  ...
  if (api && typeof api.importJsonFile === 'function') {
    api.importJsonFile().then(function(res){
      if (res && res.ok) importUserFxArchiveText(res.text, res.filePath || '视觉作品.json');
      else if (!res || !res.canceled) showToast('导入失败');
    }).catch(function(){ showToast('导入失败'); });
    return;
  }
  var input = document.createElement('input');
  input.type = 'file';
  input.accept = '.stellaflix-visual.json,.json,application/json';
  input.onchange = function(){
    var file = input.files && input.files[0];
...
function shapeWorkshopSelectedPrimitive() {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  var selected = recipe.primitives.filter(function(p){ return p.id === shapeWorkshopState.selectedId; })[0];
  if (!selected && recipe.primitives[0]) {
    selected = recipe.primitives[0];
    shapeWorkshopState.selectedId = selected.id;
  }
  return selected || null;
}
var shapeStageDrag = null;
function shapePositionStyle(primitive, index, scale, xFactor, yFactor) {
  var x = clampRange(Number(primitive && primitive.x) || 0, -0.95, 0.95);
  var y = clampRange(Number(primitive && primitive.y) || 0, -0.95, 0.95);
  return 'left:' + (50 + x * xFactor).toFixed(2) + '%;top:' + (50 + y * yFactor).toFixed(2) + '%;--i:' + index + ';--scale:' + scale.toFixed(2);
}
function shapeStageItemHtml(primitive, index) {
  var scale = clampRange(0.78 + primitive.size * 0.22 + primitive.depth * 0.14, 0.62, 1.48);
  var active = primitive.id === shapeWorkshopState.selectedId;
  var dragging = shapeStageDrag && shapeStageDrag.id === primitive.id;
  return '<button class="shape-stage-item ' + escHtml(primitive.type) + (active ? ' active' : '') + (dragging ? ' dragging' : '') + '" type="button" ' +
    'data-shape-stage-id="' + escHtml(primitive.id) + '" style="' + shapePositionStyle(primitive, index, scale, 42, 38) + '" ' +
    'onpointerdown="beginShapeStageDrag(event,\'' + escHtml(primitive.id) + '\')" onclick="shapeWorkshopSelectPrimitive(\'' + escHtml(primitive.id) + '\')" ' +
    'title="' + escHtml(shapePrimitiveLabel(primitive)) + '"></button>';
}
function shapeStageGizmoHtml(primitive, index) {
  if (!primitive) return '';
  var dragging = shapeStageDrag && shapeStageDrag.id === primitive.id;
  var rotationDeg = clampRange(Number(primitive.rotation) || 0, 0, 1.2) * 360;
  return '<div class="shape-stage-gizmo' + (dragging ? ' dragging' : '') + '" style="' + shapePositionStyle(primitive, index, 1, 42, 38) + ';--shape-rot:' + rotationDeg.toFixed(1) + 'deg;--shape-label-rot:' + (-rotationDeg).toFixed(1) + 'deg">' +
    '<button class="shape-stage-gizmo-rotate" type="button" onpointerdown="beginShapeStageRotate(event,\'' + escHtml(primitive.id) + '\')" title="拖动旋转层角度"></button>' +
    '<button class="shape-stage-gizmo-move" type="button" onpointerdown="beginShapeStageDrag(event,\'' + escHtml(primitive.id) + '\')" title="拖动调整左右 / 上下"></button>' +
    '<div class="shape-stage-gizmo-label">' + escHtml(shapePrimitiveLabel(primitive)) + ' · ' + Math.round(rotationDeg) + '°</div>' +
  '</div>';
}
function renderShapeWorkshopStage() {
  var stage = document.getElementById('shape-workshop-stage');
  if (!stage) return;
  if (!shapeWorkshopState || !shapeWorkshopState.open) {
    stage.innerHTML = '';
    stage.setAttribute('aria-hidden', 'true');
    if (document.body) document.body.classList.remove('shape-workshop-mode');
    return;
  }
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  if (document.body) document.body.classList.add('shape-workshop-mode');
  stage.setAttribute('aria-hidden', 'false');
  var items = '';
  var empty = items ? '' : '<div class="shape-stage-empty">从右侧基础例子库点击一个例子，它会出现在这里；拖动它就能改变形态结构。</div>';
  empty = recipe.primitives.length ? '' : '<div class="shape-stage-empty">空白形态</div>';
  var motion = !!shapeWorkshopState.motionPreview;
  var previewRecipe = shapeWorkshopPreviewRecipeForRender();
  var previewStatus = shapeWorkshopPreviewStatus(recipe);
  var selected = shapeWorkshopSelectedPrimitive();
  var selectedIndex = selected ? recipe.primitives.findIndex(function(p){ return p.id === selected.id; }) : -1;
  var selectedVisible = !!(selected && (!shapeWorkshopIsPrimitiveHidden(selected.id) || shapeWorkshopState.soloId === selected.id));
  var gizmo = selectedVisible ? shapeStageGizmoHtml(selected, Math.max(0, selectedIndex)) : '';
  if (recipe.primitives.length && !previewRecipe.primitives.length) empty = '<div class="shape-stage-empty">当前没有可见层<br>点“全部预览”或在层级里点“显示”。</div>';
  var countText = previewRecipe.primitives.length === recipe.primitives.length
    ? shapeRecipeSummary(recipe)
    : (previewRecipe.primitives.length + ' / ' + recipe.primitives.length + ' 层 · ' + shapeRecipeTotalParticles(previewRecipe) + ' 粒子');
  stage.innerHTML =
    '<div class="shape-stage-head">' +
      '<div><div class="shape-stage-title">Shape Workshop</div><div class="shape-stage-sub">真实 3D 点云预览。静态编辑是正面投影操作；前后厚度、层角度和律动预览会展示 3D 深度。</div></div>' +
      '<div class="shape-stage-count">' + escHtml(countText) + '</div>' +
    '</div>' +
    empty +
    gizmo +
    '<div class="shape-stage-status"><strong>' + (motion ? '律动预览' : '静态编辑') + '</strong><span>' + escHtml(previewStatus) + ' · ' + (motion ? '自动旋转 / 随音乐起伏' : '停止自转 / 停止音乐抖动') + '</span></div>';
}
function shapeWorkshopPrimitiveById(id) {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  return recipe.primitives.filter(function(p){ return p.id === id; })[0] || null;
}
function shapeStagePointFromEvent(event) {
  var stage = document.getElementById('shape-workshop-stage');
  if (!stage || !event) return null;
  var rect = stage.getBoundingClientRect();
  if (!rect.width || !rect.height) return null;
  var left = clampRange((event.clientX - rect.left) / rect.width, 0.04, 0.96);
  var top = clampRange((event.clientY - rect.top) / rect.height, 0.06, 0.94);
  return {
    x: clampRange((left - 0.5) / 0.42, -0.95, 0.95),
    y: clampRange((top - 0.5) / 0.38, -0.95, 0.95)
  };
}
function shapeStageCenterForPrimitive(primitive) {
  var stage = document.getElementById('shape-workshop-stage');
  if (!stage || !primitive) return null;
  var rect = stage.getBoundingClientRect();
  if (!rect.width || !rect.height) return null;
  var x = clampRange(Number(primitive.x) || 0, -0.95, 0.95);
  var y = clampRange(Number(primitive.y) || 0, -0.95, 0.95);
  return {
    x: rect.left + rect.width * (0.5 + x * 0.42),
    y: rect.top + rect.height * (0.5 + y * 0.38)
  };
}
function shapeStageAngleFromEvent(event, center) {
  if (!event || !center) return 0;
  return Math.atan2(event.clientY - center.y, event.clientX - center.x);
}
function normalizeShapeStageAngleDelta(delta) {
  while (delta > Math.PI) delta -= Math.PI * 2;
  while (delta < -Math.PI) delta += Math.PI * 2;
  return delta;
}
function shapeWorkshopPatchPrimitive(id, patch, renderPanel) {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  recipe.primitives = recipe.primitives.map(function(p, index){
    if (p.id !== id) return p;
    Object.keys(patch || {}).forEach(function(key){ p[key] = patch[key]; });
    return normalizeShapePrimitive(p, index);
  });
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  syncCustomShapeLayer(false);
  if (renderPanel) renderShapeWorkshop();
  else renderShapeWorkshopStage();
}
function shapeWorkshopSetPrimitivePosition(id, x, y, renderPanel) {
  shapeWorkshopPatchPrimitive(id, {
    x: clampRange(Number(x) || 0, -0.95, 0.95),
    y: clampRange(Number(y) || 0, -0.95, 0.95)
  }, renderPanel);
}
function shapeWorkshopSetPrimitiveRotation(id, rotation, renderPanel) {
  shapeWorkshopPatchPrimitive(id, {
    rotation: clampRange(Number(rotation) || 0, 0, 1.2)
  }, renderPanel);
}
function beginShapeStageDrag(event, id) {
  if (!event || !shapeWorkshopState.open) return;
  event.preventDefault();
  event.stopPropagation();
  shapeWorkshopPushUndo();
  shapeWorkshopState.selectedId = id;
  shapeStageDrag = { id: id, mode: 'move' };
  document.removeEventListener('pointermove', moveShapeStageDrag);
  document.removeEventListener('pointerup', endShapeStageDrag);
  document.removeEventListener('pointercancel', endShapeStageDrag);
  document.addEventListener('pointermove', moveShapeStageDrag);
  document.addEventListener('pointerup', endShapeStageDrag);
  document.addEventListener('pointercancel', endShapeStageDrag);
  renderShapeWorkshopStage();
}
function beginShapeStageRotate(event, id) {
  if (!event || !shapeWorkshopState.open) return;
  var primitive = shapeWorkshopPrimitiveById(id);
  var center = shapeStageCenterForPrimitive(primitive);
  if (!primitive || !center) return;
  event.preventDefault();
  event.stopPropagation();
  shapeWorkshopPushUndo();
  shapeWorkshopState.selectedId = id;
  shapeStageDrag = {
    ...
    mode: 'rotate',
    center: center,
    startAngle: shapeStageAngleFromEvent(event, center),
    startRotation: Number(primitive.rotation) || 0
  };
  document.removeEventListener('pointermove', moveShapeStageDrag);
  document.removeEventListener('pointerup', endShapeStageDrag);
  document.removeEventListener('pointercancel', endShapeStageDrag);
...
function moveShapeStageDrag(event) {
  if (!shapeStageDrag) return;
  if (shapeStageDrag.mode === 'rotate') {
    var delta = normalizeShapeStageAngleDelta(shapeStageAngleFromEvent(event, shapeStageDrag.center) - shapeStageDrag.startAngle);
    shapeWorkshopSetPrimitiveRotation(shapeStageDrag.id, shapeStageDrag.startRotation + delta / (Math.PI * 2), false);
    return;
  }
  var point = shapeStagePointFromEvent(event);
  if (!point) return;
  shapeWorkshopSetPrimitivePosition(shapeStageDrag.id, point.x, point.y, false);
}
function endShapeStageDrag(event) {
  if (shapeStageDrag && event) {
    if (shapeStageDrag.mode === 'rotate') {
      var delta = normalizeShapeStageAngleDelta(shapeStageAngleFromEvent(event, shapeStageDrag.center) - shapeStageDrag.startAngle);
      shapeWorkshopSetPrimitiveRotation(shapeStageDrag.id, shapeStageDrag.startRotation + delta / (Math.PI * 2), false);
    } else {
      var point = shapeStagePointFromEvent(event);
      if (point) shapeWorkshopSetPrimitivePosition(shapeStageDrag.id, point.x, point.y, false);
    }
  }
  var shouldRender = !!shapeStageDrag;
  shapeStageDrag = null;
  document.removeEventListener('pointermove', moveShapeStageDrag);
  document.removeEventListener('pointerup', endShapeStageDrag);
  document.removeEventListener('pointercancel', endShapeStageDrag);
  if (shouldRender) renderShapeWorkshop();
}
function shapePreviewHtml(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  var material = recipe.materialBinding && recipe.materialBinding.mode === 'cover-color' ? '取色' : '封面';
  return '<div class="shape-3d-preview-note"><strong>真实 3D 预览</strong><span>左侧舞台实时显示</span></div>' +
    '<div class="shape-3d-stat-grid">' +
      '<div class="shape-3d-stat"><b>' + recipe.primitives.length + '</b><span>层级</span></div>' +
      '<div class="shape-3d-stat"><b>' + shapeRecipeTotalParticles(recipe) + '</b><span>粒子</span></div>' +
      '<div class="shape-3d-stat"><b>' + material + '</b><span>材质</span></div>' +
    '</div>';
}
function renderShapeWorkshopClosed() {
  var hasActiveShape = !!(activeUserShapeRecipe && activeUserShapeRecipe.primitives && activeUserShapeRecipe.primitives.length);
  return '<div class="shape-workshop-closed">' +
    '<div><strong>创建形态</strong><span>用点、线、环、面、螺旋等基础例子拼出自己的形态预设；底片只负责填充。</span></div>' +
    '<div class="shape-workshop-closed-actions">' +
      (hasActiveShape ? '<button class="fx-mini-btn ghost" type="button" onclick="openShapeWorkshopFromActiveShape()">编辑当前</button>' : '') +
      '<button class="fx-mini-btn" type="button" onclick="openShapeWorkshop()">进入</button>' +
    '</div>' +
  '</div>';
}
function renderShapePrimitiveButtons() {
  return shapePrimitiveOrder.map(function(type){
    var def = shapePrimitiveDef(type);
    return '<button type="button" onclick="shapeWorkshopAddPrimitive(\'' + type + '\')" title="' + escHtml(def.desc) + '">' + escHtml(def.name) + '</button>';
  }).join('');
}
function renderShapeStructureButtons() {
  return Object.keys(shapeStructureTemplates).map(function(key){
    var item = shapeStructureTemplates[key];
    return '<button type="button" onclick="shapeWorkshopApplyStructure(\'' + key + '\')" title="加入' + escHtml(item.desc) + '">' + escHtml(item.name.replace('结构', '')) + '</button>';
  }).join('');
}
function renderShapeLayerList(recipe) {
  recipe = normalizeShapeRecipe(recipe);
  return recipe.primitives.map(function(p, index){
    var active = p.id === shapeWorkshopState.selectedId;
    var solo = p.id === shapeWorkshopState.soloId;
    var hidden = shapeWorkshopIsPrimitiveHidden(p.id);
    var rowClass = 'shape-layer-row' + (active ? ' active' : '') + (solo ? ' solo' : '') + (hidden ? ' hidden' : '');
    var meta = p.count + ' 粒子 · 前后厚度 ' + p.depth.toFixed(2) + ' · 随音乐动 ' + Math.round(p.audioFollow * 100) + '%';
    if (solo) meta += ' · 单看中';
    if (hidden && !solo) meta += ' · 已隐藏';
    var materialLabel = shapePrimitiveMaterialLabel(p);
    var materialClass = shapePrimitiveMaterialTagClass(p);
    return '<div class="' + rowClass + '" onclick="shapeWorkshopSelectPrimitive(\'' + escHtml(p.id) + '\')">' +
      '<div><div class="shape-layer-name">' + (index + 1) + '. ' + escHtml(shapePrimitiveLabel(p)) + '</div>' +
      '<div class="shape-layer-meta">' + escHtml(meta) + '</div>' +
      '<div class="shape-layer-tags"><span class="shape-layer-tag ' + materialClass + '">' + escHtml(materialLabel) + '</span></div></div>' +
      '<div class="shape-layer-actions">' +
        '<button type="button" onclick="event.stopPropagation();shapeWorkshopSoloPrimitive(\'' + escHtml(p.id) + '\')">' + (solo ? '全部' : '单看') + '</button>' +
        '<button type="button" onclick="event.stopPropagation();shapeWorkshopTogglePrimitiveHidden(\'' + escHtml(p.id) + '\')">' + (hidden ? '显示' : '隐藏') + '</button>' +
        '<button type="button" onclick="event.stopPropagation();shapeWorkshopDuplicatePrimitive(\'' + escHtml(p.id) + '\')">复制</button>' +
        '<button type="button" onclick="event.stopPropagation();shapeWorkshopRemovePrimitive(\'' + escHtml(p.id) + '\')">删除</button>' +
      '</div>' +
    '</div>';
  }).join('') || '<div class="shape-layer-row"><div><div class="shape-layer-name">还没有例子</div><div class="shape-layer-meta">从基础例子库添加一个开始</div></div></div>';
}
function renderShapeVisibilityControl(recipe, selected) {
  recipe = shapeWorkshopCleanPreviewFocus(recipe);
  var hasFocus = !!(shapeWorkshopState.soloId || Object.keys(shapeWorkshopHiddenIds()).length);
  var selectedSolo = !!(selected && shapeWorkshopState.soloId === selected.id);
  var disabled = !selected ? ' disabled' : '';
  return '<div class="shape-layer-visibility" role="group" aria-label="层级预览范围">' +
    '<button type="button" class="' + (!hasFocus ? 'active' : '') + '" onclick="shapeWorkshopShowAllLayers()">全部预览</button>' +
    '<button type="button" class="' + (selectedSolo ? 'active' : '') + '"' + disabled + ' onclick="shapeWorkshopSoloSelectedPrimitive()">只看所选</button>' +
  '</div>' +
  '<div class="shape-preview-hint">' + escHtml(shapeWorkshopPreviewStatus(recipe)) + '。这只影响工坊预览，不会删除或改写保存的形态。</div>';
}
function renderShapeMotionControl() {
  var motion = !!(shapeWorkshopState && shapeWorkshopState.motionPreview);
  var recipe = shapeWorkshopState ? normalizeShapeRecipe(shapeWorkshopState.recipe) : defaultShapeRecipe();
  var driftLocked = shapeRecipeDriftLockEnabled(recipe);
  return '<div class="shape-motion-seg" role="group" aria-label="形态预览模式">' +
    '<button type="button" class="' + (!motion ? 'active' : '') + '" onclick="shapeWorkshopSetMotionPreview(false)">静态编辑</button>' +
    '<button type="button" class="' + (motion ? 'active' : '') + '" onclick="shapeWorkshopSetMotionPreview(true)">律动预览</button>' +
  '</div>' +
  '<div class="shape-stability-actions" role="group" aria-label="形态稳定底座">' +
    '<button type="button" class="' + (driftLocked ? 'active' : '') + '" onclick="shapeWorkshopToggleDriftLock()">' + (driftLocked ? '漂移已锁定' : '锁定层漂移') + '</button>' +
    '<button type="button" onclick="shapeWorkshopStabilizeRecipe()">一键稳定</button>' +
  '</div>' +
  '<div class="shape-motion-hint">' + (motion ? '有歌曲播放时跟随真实音频；未播放时使用演示节拍。' : '静态编辑已开启，粒子不会自动旋转或随音乐抖动。') + (driftLocked ? ' 层级漂移已锁定，多层律动时不会各自慢慢偏走。' : ' 如果多层律动后容易偏走，可以先打开漂移锁定。') + '</div>';
}
function renderShapeParamPanel(primitive) {
  if (!primitive) return '<div class="shape-layer-meta">先选择一个基础例子。</div>';
  var countMax = shapePrimitiveCountMax(primitive);
  var params = [
    ['x', '左右', -0.95, 0.95, 0.01, primitive.x],
    ['y', '上下', -0.95, 0.95, 0.01, primitive.y],
    ['z', '前后', -0.95, 0.95, 0.01, primitive.z],
    ['count', '数量', 8, countMax, 1, primitive.count],
    ['size', '大小', 0.2, 1.8, 0.01, primitive.size],
    ['depth', '前后厚度', 0, 1.2, 0.01, primitive.depth],
    ['rotation', '层角度', 0, 1.2, 0.01, primitive.rotation],
    ['audioFollow', '随音乐动', 0, 1, 0.01, primitive.audioFollow],
    ['opacity', '透明', 0.08, 1, 0.01, primitive.opacity]
  ];
  return '<div class="shape-param-grid">' + params.map(function(p){
    var value = p[5];
    var text = shapeParamDisplayValue(p[0], value);
    return '<div class="shape-param-row"><label>' + p[1] + '</label>' +
      '<input type="range" min="' + p[2] + '" max="' + p[3] + '" step="' + p[4] + '" value="' + value + '" oninput="shapeWorkshopUpdatePrimitive(\'' + primitive.id + '\',\'' + p[0] + '\',this.value,this)" onchange="shapeWorkshopFinishParamEdit()">' +
      '<output>' + text + '</output></div>';
  }).join('') + '</div>';
}
function shapeParamDisplayValue(key, value) {
  return key === 'count' ? String(Math.round(Number(value) || 0)) : Number(value || 0).toFixed(2);
}
function renderShapeSavedPresets() {
  if (!userShapePresets.length) {
    return '<div class="shape-layer-meta">还没有保存的形态。保存后会出现在这里，也可以导入别人分享的形态预设。</div>' +
      '<div class="shape-saved-actions"><button type="button" onclick="importUserShapePresetFromDialog()">导入形态</button></div>';
  }
  return '<div class="shape-saved-grid">' + userShapePresets.map(function(slot, index){
    var deleting = shapeWorkshopState.deleteConfirmIndex === index;
    return '<div class="shape-saved-card">' +
      '<strong>' + escHtml(slot.name) + '</strong>' +
      '<span>' + escHtml(shapeRecipeSummary(slot.recipe)) + '</span>' +
      '<div class="shape-saved-actions">' +
        '<button type="button" onclick="loadUserShapePreset(' + index + ')">编辑</button>' +
        '<button type="button" onclick="applyUserShapePreset(' + index + ')">应用</button>' +
        '<button type="button" onclick="duplicateUserShapePreset(' + index + ')">复制</button>' +
        '<button type="button" onclick="exportUserShapePreset(' + index + ')">导出</button>' +
        '<button type="button" onclick="deleteUserShapePreset(' + index + ')">' + (deleting ? '确认删除' : '删除') + '</button>' +
      '</div>' +
    '</div>';
  }).join('') + '</div>';
}
function renderShapeWorkshop() {
  var root = document.getElementById('shape-workshop');
  if (!root) return;
  if (!shapeWorkshopState.open) {
    root.innerHTML = renderShapeWorkshopClosed();
    renderShapeWorkshopStage();
    refreshPresetGrid();
    return;
  }
  ...
  var selected = shapeWorkshopSelectedPrimitive();
  syncCustomShapeLayer(false);
  var editingSlot = shapeWorkshopState.editingIndex >= 0 && userShapePresets[shapeWorkshopState.editingIndex] ? userShapePresets[shapeWorkshopState.editingIndex] : null;
  var saveText = editingSlot ? '保存更新' : '保存为我的形态';
  var saveApplyText = editingSlot ? '保存更新并应用' : '保存并应用';
  root.innerHTML =
    '<div class="shape-workshop-panel">' +
      '<div class="shape-workshop-head">' +
        '<div><div class="shape-workshop-title">形态工坊</div><div class="shape-workshop-sub">先创造形态规则，再让歌曲封面、图片、文字作为底片填充。</div></div>' +
        '<button class="fx-mini-btn ghost" type="button" onclick="closeShapeWorkshop()">收起</button>' +
      '</div>' +
      '<input class="shape-workshop-name" maxlength="28" value="' + escHtml(recipe.name) + '" oninput="shapeWorkshopUpdateName(this.value)" onchange="shapeWorkshopFinishParamEdit()" aria-label="形态名称">' +
      '<div class="shape-workshop-preview shape-workshop-preview-summary">' + shapePreviewHtml(recipe) + '</div>' +
      '<div class="shape-workshop-section"><div class="shape-workshop-section-title">底片落点</div>' + renderShapeMaterialMap(recipe) + '</div>' +
      '<div class="shape-workshop-section shape-workshop-params-section"><div class="shape-workshop-section-title">所选例子参数</div>' + renderShapeMotionControl() + renderShapeVisibilityControl(recipe, selected) + renderShapeParamPanel(selected) + '</div>' +
      '<div class="shape-workshop-section"><div class="shape-workshop-section-title">当前形态层级</div><div class="shape-layer-list">' + renderShapeLayerList(recipe) + '</div></div>' +
      '<div class="shape-workshop-section"><div class="shape-workshop-section-title">基础例子库</div><div class="shape-chip-grid">' + renderShapePrimitiveButtons() + '</div></div>' +
      '<div class="shape-workshop-section"><div class="shape-workshop-section-title">基础结构包</div><div class="shape-chip-grid">' + renderShapeStructureButtons() + '</div></div>' +
      '<div class="shape-workshop-actions">' +
        '<button type="button" onclick="shapeWorkshopUndo()">撤销</button>' +
        '<button type="button" onclick="shapeWorkshopRedo()">重做</button>' +
        '<button type="button" onclick="applyCurrentShapePreset()">应用到播放器</button>' +
        '<button type="button" onclick="saveCurrentShapePreset(false,false)">' + saveText + '</button>' +
        '<button type="button" onclick="saveCurrentShapePreset(true,false)">' + saveApplyText + '</button>' +
        '<button type="button" onclick="saveCurrentShapePreset(false,true)">另存为新形态</button>' +
        '<button type="button" onclick="importUserShapePresetFromDialog()">导入形态</button>' +
        '<button type="button" onclick="resetShapeWorkshopRecipe()">清空画布</button>' +
      '</div>' +
      '<div class="shape-workshop-section"><div class="shape-workshop-section-title">我的形态</div>' + renderShapeSavedPresets() + '</div>' +
    '</div>';
  renderShapeWorkshopStage();
  refreshPresetGrid();
}
function openShapeWorkshop() {
  shapeWorkshopState.open = true;
  shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  shapeWorkshopState.selectedId = shapeWorkshopState.selectedId || (shapeWorkshopState.recipe.primitives[0] && shapeWorkshopState.recipe.primitives[0].id) || '';
  shapeWorkshopState.deleteConfirmIndex = -1;
  shapeWorkshopState.motionPreview = false;
  shapeWorkshopClearPreviewFocus();
  enterShapeWorkshopRenderMode();
...
function loadRecipeIntoShapeWorkshop(recipe, editingIndex, toastText) {
  recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.open = true;
  shapeWorkshopState.recipe = recipe;
  shapeWorkshopState.selectedId = recipe.primitives[0] ? recipe.primitives[0].id : '';
  shapeWorkshopState.editingIndex = Math.floor(Number(editingIndex));
  if (!isFinite(shapeWorkshopState.editingIndex)) shapeWorkshopState.editingIndex = -1;
  shapeWorkshopState.deleteConfirmIndex = -1;
  shapeWorkshopState.motionPreview = false;
  shapeWorkshopResetHistory();
  shapeWorkshopClearPreviewFocus();
...
function openShapeWorkshopFromActiveShape() {
  if (!activeUserShapeRecipe || !activeUserShapeRecipe.primitives || !activeUserShapeRecipe.primitives.length) {
    openShapeWorkshop();
    return;
  }
  var recipe = cloneShapeRecipe(activeUserShapeRecipe);
  loadRecipeIntoShapeWorkshop(recipe, -1, '已载入当前形态，可另存为我的形态');
}
function closeShapeWorkshop() {
  shapeWorkshopState.open = false;
  shapeWorkshopState.motionPreview = false;
  exitShapeWorkshopRenderMode();
  renderShapeWorkshop();
}
function shapeWorkshopSetMotionPreview(enabled) {
  shapeWorkshopState.motionPreview = !!enabled;
  syncCustomShapeLayer(true);
  renderShapeWorkshopStage();
  renderShapeWorkshop();
}
function shapeWorkshopToggleDriftLock() {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  shapeWorkshopPushUndo();
  var next = !shapeRecipeDriftLockEnabled(recipe);
  recipe.stability = { driftLock: next };
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
  showToast(next ? '已锁定层级漂移' : '已允许层级自由漂移');
}
function shapeWorkshopStabilizeRecipe() {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  if (!recipe.primitives.length) {
    showToast('先添加基础粒子，再启用稳定底座');
    return;
  }
  shapeWorkshopPushUndo();
  recipe.stability = { driftLock: true };
  recipe.primitives = recipe.primitives.map(function(p, index){
    var next = Object.assign({}, p);
    next.audioFollow = clampRange(Math.min(Number(next.audioFollow) || 0, 0.55), 0, 1);
    return normalizeShapePrimitive(next, index);
  });
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
  showToast('已启用稳定底座：锁定漂移并收敛随音乐动');
}
function shapeWorkshopShowAllLayers() {
  shapeWorkshopClearPreviewFocus();
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
}
function shapeWorkshopSoloPrimitive(id) {
  var primitive = shapeWorkshopPrimitiveById(id);
  if (!primitive) return;
  shapeWorkshopState.selectedId = primitive.id;
  shapeWorkshopState.soloId = shapeWorkshopState.soloId === primitive.id ? '' : primitive.id;
  if (shapeWorkshopState.soloId) delete shapeWorkshopHiddenIds()[primitive.id];
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
}
function shapeWorkshopSoloSelectedPrimitive() {
  var selected = shapeWorkshopSelectedPrimitive();
  if (selected) shapeWorkshopSoloPrimitive(selected.id);
}
function shapeWorkshopTogglePrimitiveHidden(id) {
  var primitive = shapeWorkshopPrimitiveById(id);
  if (!primitive) return;
  var hidden = shapeWorkshopHiddenIds();
  if (hidden[primitive.id]) delete hidden[primitive.id];
  else hidden[primitive.id] = true;
  if (shapeWorkshopState.soloId === primitive.id) shapeWorkshopState.soloId = '';
  shapeWorkshopState.selectedId = primitive.id;
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
}
function shapeWorkshopUpdateName(value) {
  shapeWorkshopEnsureParamUndo('recipe', 'name');
  shapeWorkshopState.recipe.name = normalizeShapeRecipeName(value);
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(false);
  renderShapeWorkshopStage();
}
function shapeWorkshopAddPrimitive(type) {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  if (recipe.primitives.length >= SHAPE_PRIMITIVE_LIMIT) {
    showToast('基础例子最多 ' + SHAPE_PRIMITIVE_LIMIT + ' 层');
    return;
  }
  shapeWorkshopPushUndo();
  var primitive = createShapePrimitive(type);
  recipe.primitives.push(primitive);
  shapeWorkshopState.selectedId = primitive.id;
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(false);
...
function shapeWorkshopApplyStructure(key) {
  var tpl = shapeStructureTemplates[key];
  if (!tpl) return;
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  var available = SHAPE_PRIMITIVE_LIMIT - recipe.primitives.length;
  if (available <= 0) {
    showToast('基础例子最多 ' + SHAPE_PRIMITIVE_LIMIT + ' 层');
    return;
  }
  shapeWorkshopPushUndo();
  var groupCenter = recipe.primitives.length ? shapePrimitiveSpawnPosition(recipe.primitives.length) : { x: 0, y: 0 };
  var added = tpl.primitives.slice(0, available).map(function(p, index){
    return createShapePrimitive(p.type, Object.assign({}, p, {
      index: recipe.primitives.length + index,
      x: clampRange(groupCenter.x + (Number(p.x) || 0), -0.95, 0.95),
      y: clampRange(groupCenter.y + (Number(p.y) || 0), -0.95, 0.95)
    }));
  });
  recipe.primitives = recipe.primitives.concat(added);
  if (recipe.name === '我的形态' && recipe.primitives.length === added.length) recipe.name = tpl.name;
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.selectedId = added.length ? added[0].id : '';
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(false);
  renderShapeWorkshop();
  showToast(added.length < tpl.primitives.length ? '已加入部分' + tpl.name + '，层数已达上限' : '已加入 ' + tpl.name);
}
function shapeWorkshopSelectPrimitive(id) {
  shapeWorkshopState.selectedId = id;
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
}
function shapeWorkshopUpdatePrimitive(id, key, value, inputEl) {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  var updated = null;
  shapeWorkshopEnsureParamUndo(id, key);
  recipe.primitives = recipe.primitives.map(function(p, index){
    if (p.id !== id) return p;
    p[key] = key === 'count' ? Math.round(Number(value) || p[key]) : Number(value);
    updated = normalizeShapePrimitive(p, index);
    return updated;
  });
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.deleteConfirmIndex = -1;
  ...
  if (inputEl && inputEl.parentElement) {
    var output = inputEl.parentElement.querySelector('output');
    if (output && updated) output.textContent = shapeParamDisplayValue(key, updated[key]);
    return;
  }
  renderShapeWorkshop();
}
function shapeWorkshopRemovePrimitive(id) {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  if (!recipe.primitives.some(function(p){ return p.id === id; })) return;
  shapeWorkshopPushUndo();
  recipe.primitives = recipe.primitives.filter(function(p){ return p.id !== id; });
  shapeWorkshopState.selectedId = recipe.primitives[0] ? recipe.primitives[0].id : '';
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(false);
...
function shapeWorkshopDuplicatePrimitive(id) {
  var recipe = shapeWorkshopState.recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  if (recipe.primitives.length >= SHAPE_PRIMITIVE_LIMIT) {
    showToast('基础例子最多 ' + SHAPE_PRIMITIVE_LIMIT + ' 层');
    return;
  }
  var item = recipe.primitives.filter(function(p){ return p.id === id; })[0];
  if (!item) return;
  shapeWorkshopPushUndo();
  var copy = createShapePrimitive(item.type, Object.assign({}, item, {
    id: '',
    x: clampRange((Number(item.x) || 0) + 0.08, -0.95, 0.95),
    y: clampRange((Number(item.y) || 0) + 0.08, -0.95, 0.95)
  }));
  recipe.primitives.push(copy);
  shapeWorkshopState.selectedId = copy.id;
  shapeWorkshopState.recipe = normalizeShapeRecipe(recipe);
  shapeWorkshopState.deleteConfirmIndex = -1;
  syncCustomShapeLayer(false);
  renderShapeWorkshop();
}
function resetShapeWorkshopRecipe() {
  shapeWorkshopPushUndo();
  shapeWorkshopState.recipe = defaultShapeRecipe();
  shapeWorkshopState.selectedId = '';
  shapeWorkshopState.editingIndex = -1;
  shapeWorkshopState.deleteConfirmIndex = -1;
  shapeWorkshopClearPreviewFocus();
  syncCustomShapeLayer(true);
  renderShapeWorkshop();
...
function saveCurrentShapePreset(applyAfterSave, forceNew) {
  var recipe = normalizeShapeRecipe(shapeWorkshopState.recipe);
  if (!recipe.primitives.length) {
    showToast('先添加基础粒子，再保存形态');
    return;
  }
  var slot = {
    name: recipe.name,
  ...
  var editing = !forceNew && shapeWorkshopState.editingIndex >= 0 && userShapePresets[shapeWorkshopState.editingIndex];
  if (editing) {
    userShapePresets[shapeWorkshopState.editingIndex] = slot;
  } else {
    userShapePresets.push(slot);
    shapeWorkshopState.editingIndex = userShapePresets.length - 1;
  }
  shapeWorkshopState.deleteConfirmIndex = -1;
  saveUserShapePresets();
  if (applyAfterSave) applyShapeRecipeToPlayer(slot.recipe, true);
  buildPresetGrid();
  renderShapeWorkshop();
  var toastPrefix = editing ? (applyAfterSave ? '已更新并应用: ' : '已更新形态: ') : (applyAfterSave ? '已保存并应用: ' : '已保存为我的形态: ');
  showToast(toastPrefix + slot.name);
}
function applyCurrentShapePreset() {
  applyShapeRecipeToPlayer(shapeWorkshopState.recipe);
}
function loadUserShapePreset(index) {
  index = Math.floor(Number(index) || 0);
  var slot = userShapePresets[index];
  if (!slot || !slot.recipe) return;
  shapeWorkshopState.open = true;
  shapeWorkshopState.recipe = cloneShapeRecipe(slot.recipe);
  shapeWorkshopState.recipe.name = normalizeShapeRecipeName(slot.name || shapeWorkshopState.recipe.name);
  shapeWorkshopState.selectedId = shapeWorkshopState.recipe.primitives[0] ? shapeWorkshopState.recipe.primitives[0].id : '';
  shapeWorkshopState.editingIndex = index;
  shapeWorkshopState.deleteConfirmIndex = -1;
  shapeWorkshopState.motionPreview = false;
  shapeWorkshopResetHistory();
  shapeWorkshopClearPreviewFocus();
  enterShapeWorkshopRenderMode();
  renderShapeWorkshop();
  showToast('已载入形态: ' + shapeWorkshopState.recipe.name);
}
function applyUserShapePreset(index) {
  index = Math.floor(Number(index) || 0);
  var slot = userShapePresets[index];
  if (!slot || !slot.recipe) return;
  shapeWorkshopState.deleteConfirmIndex = -1;
  applyShapeRecipeToPlayer(slot.recipe);
  renderShapeWorkshop();
}
function duplicateUserShapePreset(index) {
  index = Math.floor(Number(index) || 0);
  var slot = userShapePresets[index];
  if (!slot || !slot.recipe) return;
  var copy = {
    name: normalizeShapeRecipeName((slot.name || '我的形态') + ' 副本'),
    savedAt: Date.now(),
    recipe: cloneShapeRecipe(slot.recipe)
  };
  copy.recipe.name = copy.name;
  userShapePresets.push(copy);
  shapeWorkshopState.editingIndex = userShapePresets.length - 1;
  shapeWorkshopState.deleteConfirmIndex = -1;
  saveUserShapePresets();
  buildPresetGrid();
  renderShapeWorkshop();
  showToast('已复制形态: ' + copy.name);
}
function deleteUserShapePreset(index) {
  index = Math.floor(Number(index) || 0);
  var slot = userShapePresets[index];
  if (!slot) return;
  if (shapeWorkshopState.deleteConfirmIndex !== index) {
    shapeWorkshopState.deleteConfirmIndex = index;
    renderShapeWorkshop();
    buildPresetGrid();
    showToast('再点一次确认删除: ' + slot.name);
    return;
  }
  var deletingActive = activeUserShapeRecipe && shapeRecipeRenderKey(activeUserShapeRecipe) === shapeRecipeRenderKey(slot.recipe);
  userShapePresets.splice(index, 1);
  if (shapeWorkshopState.editingIndex === index) shapeWorkshopState.editingIndex = -1;
  else if (shapeWorkshopState.editingIndex > index) shapeWorkshopState.editingIndex -= 1;
  shapeWorkshopState.deleteConfirmIndex = -1;
  if (deletingActive) clearActiveUserShapePreset();
  saveUserShapePresets();
  buildPresetGrid();
  renderShapeWorkshop();
  showToast('已删除形态: ' + slot.name);
}

function creativeSandboxPresetName() {
  var preset = presetMeta[Math.max(0, Math.min(presetMeta.length - 1, Number(fx.preset) || 0))];
  return preset && preset.name ? preset.name : '当前形态';
}
function creativeSandboxMaterialLabel() {
  return shapeMaterialMediaLabel(fx.shapeMaterialMedia);
}
function renderCreativeSandboxBuilder() {
  var grid = document.getElementById('sandbox-builder-grid');
  if (!grid) return;
  var material = creativeSandboxMaterialLabel();
  var preset = creativeSandboxPresetName();
  var layerText = [
    fx.floatLayer ? '浮空' : '',
    fx.bloom ? '光晕' : '',
    fx.edge ? '轮廓' : ''
  ].filter(Boolean).join(' / ') || '基础粒子';
  var motionText = '律动 ' + Math.round(clampRange(Number(fx.intensity) || 0, 0.2, 1.6) / 1.6 * 100) + '%';
  var cameraText = fx.cinema ? '电影镜头' : '静态镜头';
  grid.innerHTML =
    '<div class="sandbox-block primary">' +
      '<div class="sandbox-block-head"><div><div class="sandbox-block-kicker">material</div><div class="sandbox-block-title">形态素材</div></div><div class="sandbox-status">' + escHtml(material) + '</div></div>' +
      '<div class="sandbox-block-desc">这里只调整形态内部使用的底片素材，不会铺满整屏背景。</div>' +
      '<div class="sandbox-actions">' +
        '<button type="button" onclick="creativeSandboxUseCover()" class="' + (currentShapeMaterialMedia() ? '' : 'active') + '">当前封面</button>' +
        '<button type="button" onclick="creativeSandboxUploadMaterial()">上传素材</button>' +
      '</div>' +
    '</div>' +
    '<div class="sandbox-block">' +
      '<div class="sandbox-block-head"><div><div class="sandbox-block-kicker">shape</div><div class="sandbox-block-title">形态</div></div><div class="sandbox-status">' + escHtml(preset) + '</div></div>' +
      '<div class="sandbox-block-desc">这里复用现有稳定预设；真正自创形态请进入上方形态工坊。</div>' +
      '<div class="sandbox-actions">' +
        '<button type="button" onclick="creativeSandboxSetPreset(0)" class="' + (fx.preset === 0 ? 'active' : '') + '">封面粒子</button>' +
        '<button type="button" onclick="creativeSandboxSetPreset(4)" class="' + (fx.preset === 4 ? 'active' : '') + '">唱片</button>' +
        '<button type="button" onclick="creativeSandboxSetPreset(5)" class="' + (fx.preset === 5 ? 'active' : '') + '">星河</button>' +
      '</div>' +
    '</div>' +
    '<div class="sandbox-block">' +
      '<div class="sandbox-block-head"><div><div class="sandbox-block-kicker">layers</div><div class="sandbox-block-title">粒子层</div></div><div class="sandbox-status">' + escHtml(layerText) + '</div></div>' +
      '<div class="sandbox-block-desc">这里是作品层面的叠加开关，不等同于形态规则。</div>' +
      '<div class="sandbox-actions">' +
        '<button type="button" onclick="creativeSandboxToggleFx(\'floatLayer\')" class="' + (fx.floatLayer ? 'active' : '') + '">浮空</button>' +
        '<button type="button" onclick="creativeSandboxToggleFx(\'bloom\')" class="' + (fx.bloom ? 'active' : '') + '">光晕</button>' +
        '<button type="button" onclick="creativeSandboxToggleFx(\'edge\')" class="' + (fx.edge ? 'active' : '') + '">轮廓</button>' +
      '</div>' +
    '</div>' +
    '<div class="sandbox-block">' +
      '<div class="sandbox-block-head"><div><div class="sandbox-block-kicker">audio</div><div class="sandbox-block-title">律动</div></div><div class="sandbox-status">' + escHtml(motionText) + '</div></div>' +
      '<div class="sandbox-block-desc">先开放安全的律动强度和速度组合，保证不会拖垮性能。</div>' +
      '<div class="sandbox-actions">' +
        '<button type="button" onclick="creativeSandboxSetMotion(\'soft\')">柔和</button>' +
        '<button type="button" onclick="creativeSandboxSetMotion(\'beat\')" class="' + (fx.intensity >= 1.05 ? 'active' : '') + '">节拍</button>' +
        '<button type="button" onclick="creativeSandboxSetMotion(\'calm\')">静态</button>' +
      '</div>' +
    '</div>' +
    '<div class="sandbox-block">' +
      '<div class="sandbox-block-head"><div><div class="sandbox-block-kicker">camera</div><div class="sandbox-block-title">镜头</div></div><div class="sandbox-status">' + escHtml(cameraText) + '</div></div>' +
      '<div class="sandbox-block-desc">镜头作为积木独立控制，后续可以做更多转场。</div>' +
      '<div class="sandbox-actions">' +
        '<button type="button" onclick="creativeSandboxSetCamera(\'static\')" class="' + (!fx.cinema ? 'active' : '') + '">静态</button>' +
        '<button type="button" onclick="creativeSandboxSetCamera(\'cinema\')" class="' + (fx.cinema ? 'active' : '') + '">电影</button>' +
        '<button type="button" onclick="creativeSandboxSetCamera(\'stage\')" class="' + (fx.shelf === 'stage' ? 'active' : '') + '">舞台</button>' +
      '</div>' +
    '</div>' +
    '<div class="sandbox-block primary">' +
      '<div class="sandbox-block-head"><div><div class="sandbox-block-kicker">work</div><div class="sandbox-block-title">作品</div></div><div class="sandbox-status">保存 / 复制 / 导出</div></div>' +
      '<div class="sandbox-block-desc">搭好后保存到“我的作品”，可以复制出新版本继续二创。</div>' +
      '<div class="sandbox-actions">' +
        '<button type="button" onclick="createUserFxArchive()">新作品</button>' +
        '<button type="button" onclick="saveCurrentAsUserFxArchive()">保存当前</button>' +
      '</div>' +
    '</div>';
}
function refreshCreativeSandboxBuilder() {
  renderCreativeSandboxBuilder();
}
function creativeSandboxUseCover() {
  clearShapeMaterialMedia();
  refreshCreativeSandboxBuilder();
}
function creativeSandboxUploadMaterial() {
  var input = document.getElementById('shape-material-input');
  if (input) input.click();
}
function creativeSandboxSetPreset(p) {
  setPreset(p);
  updateFxInputs();
  refreshCreativeSandboxBuilder();
}
function creativeSandboxToggleFx(key) {
  toggleFx(key);
  refreshCreativeSandboxBuilder();
}
function creativeSandboxSetMotion(mode) {
  if (mode === 'beat') {
    fx.intensity = 1.22;
    fx.speed = 1.18;
    fx.cinemaShake = 0.72;
  ...
  updateFxInputs();
  saveLyricLayout();
  showToast(mode === 'beat' ? '律动积木: 节拍' : (mode === 'calm' ? '律动积木: 静态' : '律动积木: 柔和'));
}
function creativeSandboxSetCamera(mode) {
  if (mode === 'stage') {
    setShelfMode('stage');
  } else {
    fx.cinema = mode === 'cinema';
    if (mode === 'static') fx.cinemaShake = 0;
    else if (fx.cinemaShake < 0.35) fx.cinemaShake = 0.5;
    updateFxInputs();
    saveLyricLayout();
  }
  refreshCreativeSandboxBuilder();
  showToast(mode === 'stage' ? '镜头积木: 舞台' : (mode === 'cinema' ? '镜头积木: 电影' : '镜头积木: 静态'));
}

// === [03-lyrics · 颜色控件块] 区域横幅（2026-08-07 标订；2.3 仅标记边界，不搬迁） ===
...
function buildPresetGrid() {
  ...
  var builtinCards = order.map(function(i){
    var p = presetMeta[i];
  ...
  grid.innerHTML =
    '<button class="preset-card shape-workshop-entry" type="button" data-shape-workshop="entry" onclick="openShapeWorkshop()">' +
      '<div class="pc-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 18c4-8 8-8 16 0"/><path d="M6 14c4-5 8-5 12 0"/><circle cx="7" cy="17" r="1.5" fill="currentColor"/><circle cx="12" cy="10" r="1.5" fill="currentColor"/><circle cx="17" cy="17" r="1.5" fill="currentColor"/></svg></div>' +
      '<div class="pc-name">创建形态</div>' +
      '<div class="pc-desc">基础例子 · 自定义预设</div>' +
    '</button>' +
    renderPresetGridUserShapeCards() +
    builtinCards;
  refreshPresetGrid();
}
function renderPresetGridUserShapeCards() {
  if (!userShapePresets || !userShapePresets.length) return '';
  return userShapePresets.map(function(slot, index){
    var recipe = normalizeShapeRecipe(slot.recipe);
    var deleting = shapeWorkshopState && shapeWorkshopState.deleteConfirmIndex === index;
    return '<div class="preset-card user-shape-preset" role="button" tabindex="0" data-user-shape-index="' + index + '" onclick="applyUserShapePresetFromGrid(' + index + ')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();applyUserShapePresetFromGrid(' + index + ')}" title="' + escHtml(slot.name) + '">' +
      '<button class="user-shape-card-delete' + (deleting ? ' confirm' : '') + '" type="button" onclick="event.stopPropagation();deleteUserShapePreset(' + index + ')">' + (deleting ? '确认' : '删除') + '</button>' +
      '<div class="pc-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 17c2.5-5 5-5 10 0s7.5 5 10 0"/><path d="M6 12c2.1-2.8 4.2-2.8 8.4 0s6.3 2.8 8.4 0"/><circle cx="7" cy="17" r="1.45" fill="currentColor"/><circle cx="14" cy="10" r="1.45" fill="currentColor"/><circle cx="21" cy="17" r="1.45" fill="currentColor"/></svg></div>' +
      '<div class="pc-name">' + escHtml(slot.name) + '</div>' +
      '<div class="pc-desc">我的形态 · ' + escHtml(shapeRecipeSummary(recipe)) + '</div>' +
    '</div>';
  }).join('');
}
function refreshPresetGrid() {
  var customActive = !!(activeUserShapeRecipe && fx && Number(fx.preset) === 3);
  document.querySelectorAll('.preset-card').forEach(function(el){
    var isWorkshop = el.getAttribute('data-shape-workshop') === 'entry';
    var userShapeIndex = el.getAttribute('data-user-shape-index');
    var userShapeActive = false;
    if (userShapeIndex != null) {
      var slot = userShapePresets[Math.floor(Number(userShapeIndex) || 0)];
      userShapeActive = !!(customActive && slot && slot.recipe && shapeRecipeRenderKey(activeUserShapeRecipe) === shapeRecipeRenderKey(slot.recipe));
    }
    el.classList.toggle('active', userShapeIndex != null ? userShapeActive : (isWorkshop ? (!!(shapeWorkshopState && shapeWorkshopState.open) && !customActive) : (!customActive && Number(el.dataset.preset) === fx.preset)));
  });
}
function applyUserShapePresetFromGrid(index) {
  index = Math.floor(Number(index) || 0);
  var slot = userShapePresets[index];
  if (!slot || !slot.recipe) {
    showToast('这个形态预设已不存在');
    buildPresetGrid();
    return;
  }
  shapeWorkshopState.open = false;
...
// [moved -> 08-fx-visual.js · region E]
function setPreset(p, opts) {
  opts = opts || {};
  p = Math.max(0, Math.min(presetMeta.length - 1, Number(p) || 0));
  var prev = fx.preset;
  var changed = prev !== p;
  if (!opts.keepCustomShape && !opts.silent && activeUserShapeRecipe) clearActiveUserShapePreset();
  fx.preset = p;
...
// [moved -> 08-fx-visual.js · region F]
...
function updateFxInputs() {
  ...
  var lyricNextLineToggle = document.getElementById('t-lyricNextLine');
  if (lyricNextLineToggle) lyricNextLineToggle.classList.toggle('on', fx.lyricNextLine !== false);
...
/* === 状态切换器（启动默认空间）：点击切换音乐/影视空间；持久化到 stellaflix-start-space === */
...
function setStartSpacePreference(mode) {
  ...
  try { localStorage.setItem('stellaflix-start-space', mode); } catch (e) { /* 静默 */ }
}
function updateStartSpaceUI() {
  var card = document.getElementById('t-startSpace');
  if (!card) return;
  var cur = getStartSpacePreference();
  // mode 名称由 DOM data-mode 标识驱动——JS 永远不改 textContent，纯靠 CSS .start-space-video 切换显隐
  // 这样：改 mode 名字只改 DOM，文字与 data-mode 永远绑定，不存在 toggle 错位风险
  card.classList.toggle('start-space-video', cur === 'video');
  card.setAttribute('aria-checked', cur === 'video' ? 'true' : 'false');
  card.setAttribute('data-start-space', cur);
}
function toggleStartSpace() {
  ...
  updateStartSpaceUI();
  // 立即反馈：把运行时空间也切到目标，便于用户在设置面板中直接看到效果
  var SFV = window.StellaflixVideo;
  if (SFV && SFV.state && typeof SFV.state.setSpace === 'function') {
    try { SFV.state.setSpace(next); } catch (e) { /* 静默 */ }
  }
  if (typeof showToast === 'function') {
    showToast(next === 'video' ? '下次启动将进入影视空间' : '下次启动将进入音乐空间');
  }
}
// 启动后第一次同步：等待 DOM 就绪
if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', updateStartSpaceUI);
  } else {
    // 微任务延后，确保卡片已注入
    Promise.resolve().then(updateStartSpaceUI);
  }
}
...
function setFxPanelTab(tab) {
  ...
  var nextTab = allowed[tab] ? tab : 'presets';
  if (nextTab !== 'presets' && typeof shapeWorkshopState !== 'undefined' && shapeWorkshopState.open && typeof closeShapeWorkshop === 'function') {
    closeShapeWorkshop();
  }
  fxPanelTab = nextTab;
  var panel = document.getElementById('fx-panel');
...
function fxControlBlock(id) {
  var el = document.getElementById(id);
  if (!el) return null;
  return el.closest('.fx-slider,.lyric-color-row,.lyric-color-grid,.fx-seg,.preset-grid,.shape-workshop-shell,.sandbox-builder-grid,.user-archive-grid,.fx-font-grid') || el;
}
...
function applyBackgroundMediaHint() {
  ...
  if (value && !value.dataset.mediaHint) {
    ...
    value.title = '只影响整屏背景；形态素材请在作品组合里上传';
  }
...
function relabelFxPanelControls() {
  ...
  setFxSectionBefore('preset-grid', '形态预设');
  setFxSectionBefore('shape-workshop', '形态工坊');
  setFxSectionBefore('sandbox-builder-grid', '作品组合');
  setFxSectionBefore('user-archive-grid', '我的作品');
  ...
  modal.innerHTML =
    '<div class="hotkey-dialog" role="dialog" aria-modal="true" aria-label="热键设置">' +
      '<div class="hotkey-head">' +
        '<div><div class="hotkey-title">热键设置</div><div class="hotkey-sub">局内热键只在 Stellaflix 窗口内生效；全局热键会向系统注册，并检测是否被占用。</div></div>' +
...
function hotkeyStatusMarkup(scope, actionKey, binding, duplicate) {
  ...
  if (duplicate && duplicate[binding] > 1) return '<span class="hotkey-status conflict"><span class="source-icon">!</span>Stellaflix 内部重复</span>';
...
function bindFxPanel() {
  ...
  ids.forEach(function(pair){
    ...
    el.addEventListener('input', function(){
      ...
      saveLyricLayout();
      if (/^(intensity|speed|cinemaShake|backgroundOpacity)$/.test(pair[1])) refreshCreativeSandboxBuilder();
    });
  });
  ...
  var shapeMaterialInput = document.getElementById('shape-material-input');
  if (shapeMaterialInput) {
    shapeMaterialInput.addEventListener('change', function(e){
      var file = e.target.files && e.target.files[0];
      if (file) readShapeMaterialImageFile(file);
      e.target.value = '';
    });
  }
...
function toggleFx(key) {
  ...
  if (key === 'lyricNextLine') showToast(fx.lyricNextLine !== false ? '下一句歌词已显示' : '下一句歌词已隐藏');
...
function syncUpdatePreviewStateClass() {
  ...
  if (foot) {
    ...
    else if (isReady && isPatch) foot.textContent = updatePreviewState.restartRequired ? '快速补丁已应用，重启 Stellaflix 后生效。' : '快速补丁已应用。';
...
async function restartForAppliedPatch() {
  ...
  showToast('请手动重启 Stellaflix 让补丁生效');
}
...
function platformMeta(provider) {
  if (provider === 'kugouMusic') return { key: 'kugouMusic', short: 'KGM', label: '酷狗音乐', app: '酷狗音乐 App', dot: 'kugouMusic' };
...
function platformStatus(provider) {
  if (provider === 'kugouMusic') return kugouMusicLoginStatus;
...
function providerVipBadge(provider, status, idAttr) {
  ...
  if (provider === 'kugou') cls += ' kugou';
  if (provider === 'kugouMusic') cls += ' kugouMusic';
  var level = providerVipLevel(provider, status);
  var label = provider === 'qq' ? 'QQ VIP' : (level === 'svip' ? 'SVIP' : 'VIP');
  if (provider === 'kugou') label = 'KG VIP';
  if (provider === 'kugouMusic') label = 'KGM VIP';
...
function hasAnyPlatformLogin() {
  return hasPlatformLogin('netease') || hasPlatformLogin('qq') || hasPlatformLogin('kugou') || hasPlatformLogin('kugouMusic');
}
function loggedProviderCount() {
  return ['netease', 'qq', 'kugou', 'kugouMusic'].filter(function(provider){ return hasPlatformLogin(provider); }).length;
}
function firstLoggedProvider() {
  ...
  if (hasPlatformLogin('kugouMusic')) return 'kugouMusic';
  return 'netease';
}
function providerAvatarSrc(provider, status) {
  ...
  var meta = platformMeta(provider);
  var fill = provider === 'qq' ? '#bfd66b' : (provider === 'kugou' ? '#5ccca6' : (provider === 'kugouMusic' ? '#6aa6ff' : '#d95b67'));
  var bg = provider === 'qq' ? '#11150b' : (provider === 'kugou' ? '#071511' : (provider === 'kugouMusic' ? '#071020' : '#180b0f'));
...
async function refreshLoginStatus(force) {
  try {
    ...
    if (info && info.loggedIn) {
      ...
      qishuiImportedPlaylists = readQishuiImportedPlaylists();
      userPlaylists = qqPlaylists.concat(kugouPlaylists, kugouMusicPlaylists, qishuiImportedPlaylists);
      myPodcastCollections = [];
      myPodcastItems = {};
      likedSongMap = {};
      // 未登录状态:本地红心(阶段二)播种进 UI
      if (typeof SFLocalPlaylists !== 'undefined') SFLocalPlaylists.seedLikedMap(likedSongMap);
      updateLikeButtons();
    }
    return info;
  } catch (e) {
...
function normalizeKugouLoginStatus(info) {
  var fallback = { provider: 'kugou', platform: 'lite', loggedIn: false, nickname: '酷狗概念版', userId: '', avatar: '', vipType: 0, tokenReady: false, deviceReady: false };
  if (!info || !info.loggedIn) return Object.assign({}, fallback, info || {}, {
    provider: 'kugou',
    platform: 'lite',
    loggedIn: false,
    ...
    avatar: info && info.avatar || '',
    vipType: Number(info && (info.vipType || info.vip_type) || 0) || 0,
    tokenReady: !!(info && info.tokenReady),
    deviceReady: !!(info && info.deviceReady)
  });
  return Object.assign({}, fallback, info, {
    provider: 'kugou',
    platform: 'lite',
...
function normalizeKugouMusicLoginStatus(info) {
  var fallback = { provider: 'kugouMusic', platform: 'music', loggedIn: false, nickname: '酷狗音乐', userId: '', avatar: '', vipType: 0, tokenReady: false, deviceReady: false };
  if (!info || !info.loggedIn) return Object.assign({}, fallback, info || {}, {
    provider: 'kugouMusic',
    platform: 'music',
    loggedIn: false,
...
async function refreshKugouLoginStatus() {
  try {
    ...
    if (!kugouLoginStatus.loggedIn) {
      if (prevLogged || kugouLoginWasLoggedIn) showToast('酷狗概念版已退出登录');
      kugouPlaylists = [];
...
async function refreshKugouMusicLoginStatus() {
  try {
    var info = await apiJson('/api/kugou-music/login/status?t=' + Date.now());
    var prevLogged = !!kugouMusicLoginStatus.loggedIn;
    kugouMusicLoginStatus = normalizeKugouMusicLoginStatus(info);
    if (!kugouMusicLoginStatus.loggedIn) {
      if (prevLogged || kugouMusicLoginWasLoggedIn) showToast('酷狗音乐已退出登录');
      kugouMusicPlaylists = [];
      userPlaylists = userPlaylists.filter(function(pl){ return pl.provider !== 'kugouMusic'; });
      homeDiscoverState.loaded = false;
    } else if (!userPlaylists.some(function(pl){ return pl && pl.provider === 'kugouMusic'; })) {
      homeDiscoverState.loaded = false;
      homeDiscoverState.loggedIn = true;
      refreshUserPlaylists(true);
    }
    kugouMusicLoginWasLoggedIn = !!kugouMusicLoginStatus.loggedIn;
    if (!hasPlatformLogin(activeAccountProvider)) activeAccountProvider = firstLoggedProvider();
    renderUserBtn();
    return kugouMusicLoginStatus;
  } catch (e) {
    console.warn('KuGou Music login status failed:', e);
    kugouMusicLoginStatus = normalizeKugouMusicLoginStatus(null);
    renderUserBtn();
    return kugouMusicLoginStatus;
  }
}
...
function startKugouMusicLoginStatusAutoRefresh() {
  if (kugouMusicLoginAutoRefreshTimer) clearInterval(kugouMusicLoginAutoRefreshTimer);
  kugouMusicLoginAutoRefreshTimer = setInterval(function(){
    refreshKugouMusicLoginStatus().catch(function(e){ console.warn('KuGou Music login auto refresh failed:', e); });
  }, 45000);
}
function renderUserBtn() {
  ...
  if (dualAccountMode && hasAnyPlatformLogin()) {
    ...
    btn.innerHTML = renderTopAccountPill('netease') + renderTopAccountPill('qq') + renderTopAccountPill('kugou') + renderTopAccountPill('kugouMusic');
...
function updateLoginProviderUi() {
  ...
  var isKugouMusic = loginProvider === 'kugouMusic';
  var isKugouQr = isKugou || isKugouMusic;
  ...
  var kugouMusicBtn = document.getElementById('login-provider-kugou-music');
  var canOpenNeteaseWeb = !!(window.desktopWindow && typeof window.desktopWindow.openNeteaseMusicLogin === 'function');
  var canOpenQQWeb = !!(window.desktopWindow && window.desktopWindow.isDesktop && typeof window.desktopWindow.openQQMusicLogin === 'function');
  ...
  if (kugouMusicBtn) kugouMusicBtn.classList.toggle('active', isKugouMusic);
  if (title) title.textContent = '扫码登录' + meta.label;
  if (desc) desc.innerHTML = isQQ
    ? (canOpenQQWeb
      ? '打开 <b>QQ 音乐官方网页登录窗口</b> 扫码，成功后会自动同步账号会话。'
      : '使用 <b>QQ 或 QQ 音乐 App</b> 扫码，成功后会自动同步账号会话。')
    : (isKugouQr
      ? '使用 <b>' + escHtml(meta.app) + '</b> 扫码；登录后会同步搜索、播放和歌单读取。'
      : (canOpenNeteaseWeb
      ? '打开 <b>网易云音乐官方网页登录窗口</b> 扫码，避开接口二维码风控；成功后会自动同步账号会话。'
      : '使用 <b>网易云音乐 App</b> 扫码，可同步歌单、红心与播客。'));
  if (shell) {
    shell.classList.toggle('web-login-preview', !isKugouQr && ((isQQ && canOpenQQWeb) || (!isQQ && canOpenNeteaseWeb)));
    shell.classList.toggle('qq-preview', isQQ);
    shell.classList.toggle('netease-preview', !isQQ && !isKugouQr && canOpenNeteaseWeb);
  }
  ...
  if (st) {
    ...
    st.textContent = isQQ
      ? (qqLoginStatus.loggedIn ? ('已保存 QQ 音乐会话 · ' + (qqLoginStatus.nickname || '')) : (canOpenQQWeb ? '点击“扫码登录”打开 QQ 音乐官方窗口' : '正在生成 QQ 音乐二维码…'))
      : (isKugouQr
        ? (platformStatus(loginProvider).loggedIn ? ('已保存' + meta.label + '会话 · ' + (platformStatus(loginProvider).nickname || '')) : '正在生成' + meta.label + '二维码…')
        : (canOpenNeteaseWeb ? '点击“网页登录”打开网易云官方窗口' : '正在生成二维码…'));
  }
  if (refreshBtn) {
    refreshBtn.disabled = isQQ ? (canOpenQQWeb && !!qqWebLoginBusy) : (!isKugouQr && !!neteaseWebLoginBusy);
    refreshBtn.textContent = isQQ ? (canOpenQQWeb ? (qqWebLoginBusy ? '等待扫码…' : '扫码登录') : '刷新二维码') : (isKugouQr ? '刷新二维码' : (canOpenNeteaseWeb ? (neteaseWebLoginBusy ? '等待扫码…' : '网页登录') : '刷新二维码'));
    refreshBtn.onclick = isQQ ? (canOpenQQWeb ? openQQWebLogin : refreshQr) : (isKugouQr ? refreshQr : (canOpenNeteaseWeb ? openNeteaseWebLogin : refreshQr));
  }
}
async function refreshQr() {
  ...
  if (loginProvider === 'qq') {
    ...
    try {
      if (qqStatus) { qqStatus.textContent = '正在生成 QQ 音乐二维码…'; qqStatus.className = ''; }
      var qqQr = await apiJson('/api/qq/login/qr/key?t=' + Date.now());
      var qqData = (qqQr && qqQr.data) || {};
      qrKey = qqData.key || qqQr.key || qqData.qrcode || '';
      var qqQrImg = qqData.qrcode_img || qqQr.img || qqQr.qrcode_img || '';
      if (!qrKey) throw new Error('获取 QQ 二维码 key 失败');
      if (!qqQrImg) throw new Error('QQ 二维码图片为空');
      if (qqImg) qqImg.src = qqQrImg;
      if (qqStatus) { qqStatus.textContent = '请使用 QQ 或 QQ 音乐 App 扫码'; qqStatus.className = ''; }
      startQrPoll();
    } catch (e) {
      if (qqStatus) { qqStatus.textContent = 'QQ 二维码出错: ' + e.message; qqStatus.className = 'fail'; }
    }
    return;
  }
  if (loginProvider === 'kugou' || loginProvider === 'kugouMusic') {
    try {
      var qrProvider = loginProvider;
      var kgMeta = platformMeta(qrProvider);
      ...
      if (kgStatus) { kgStatus.textContent = '正在生成' + kgMeta.label + '二维码…'; kgStatus.className = ''; }
      if (kgImg) kgImg.src = '';
      var kg = await apiJson(kgApiBase + '/login/qr/key?t=' + Date.now());
      var kgData = kg && kg.data || {};
      qrKey = kgData.qrcode || kgData.key || kg.key || '';
      if (!qrKey) throw new Error('获取' + kgMeta.label + '二维码 key 失败');
      if (!kgData.qrcode_img) {
        await apiJson(kgApiBase + '/login/qr/create?key=' + encodeURIComponent(qrKey));
      }
      if (!kgData.qrcode_img) throw new Error(kgMeta.label + '二维码图片为空');
      if (kgImg) kgImg.src = kgData.qrcode_img;
      if (kgStatus) { kgStatus.textContent = '请使用' + kgMeta.app + '扫码'; kgStatus.className = ''; }
      startQrPoll();
    } catch (e) {
      var kgFail = document.getElementById('qr-status');
      if (kgFail) { kgFail.textContent = '酷狗二维码出错: ' + e.message; kgFail.className = 'fail'; }
    }
    return;
  }
...
async function openQQWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openQQMusicLogin !== 'function') {
    if (statusEl) { statusEl.textContent = '正在生成 QQ 音乐二维码…'; statusEl.className = ''; }
    return refreshQr();
  }
...
async function checkQr() {
  ...
  if (loginProvider === 'qq') {
    try {
      var qq = await apiJson('/api/qq/login/qr/check?key=' + encodeURIComponent(qrKey) + '&t=' + Date.now());
      var qqStatus = Number((qq && qq.data && qq.data.status) || qq.status || 0);
      var qqSt = document.getElementById('qr-status');
      if (qqStatus === 0) {
        if (qqSt) { qqSt.textContent = (qq && qq.message) || 'QQ 二维码已过期，请刷新'; qqSt.className = 'fail'; }
        stopQrPoll();
      } else if (qqStatus === 1) {
        if (qqSt) { qqSt.textContent = (qq && qq.message) || '请使用 QQ 或 QQ 音乐 App 扫码'; qqSt.className = ''; }
      } else if (qqStatus === 2) {
        if (qqSt) { qqSt.textContent = (qq && qq.message) || '已扫码，请在手机上确认登录'; qqSt.className = 'scan'; }
      } else if (qqStatus === 4 && qq && qq.loggedIn) {
        qqLoginStatus = normalizeQQLoginStatus(qq);
        activeAccountProvider = 'qq';
        qqManualCookieOpen = false;
        if (qqSt) { qqSt.textContent = qq.message || 'QQ 音乐登录成功'; qqSt.className = 'scan'; }
        stopQrPoll();
        renderUserBtn();
        refreshUserPlaylists(true);
        setTimeout(async function(){
          var fresh = await refreshQQLoginStatus();
          closeLoginModal();
          showToast('QQ 音乐已登录: ' + ((fresh && fresh.nickname) || qqLoginStatus.nickname || ''));
        }, 500);
      } else if (qqStatus === 4) {
        if (qqSt) { qqSt.textContent = (qq && (qq.message || qq.error)) || 'QQ 已确认，但没有拿到有效会话'; qqSt.className = 'fail'; }
        stopQrPoll();
      }
    } catch (e) {
      console.warn('QQ QR check failed:', e);
    }
    return;
  }
  if (loginProvider === 'kugou' || loginProvider === 'kugouMusic') {
    try {
      ...
      var kgMeta = platformMeta(qrProvider);
      var kgApiBase = qrProvider === 'kugouMusic' ? '/api/kugou-music' : '/api/kugou';
      var kg = await apiJson(kgApiBase + '/login/qr/check?key=' + encodeURIComponent(qrKey) + '&t=' + Date.now());
      var kgStatus = Number((kg && kg.data && kg.data.status) || kg.status || 0);
      var kgSt = document.getElementById('qr-status');
      if (kgStatus === 0) {
        if (kgSt) { kgSt.textContent = '酷狗二维码已过期，请刷新'; kgSt.className = 'fail'; }
        stopQrPoll();
      } else if (kgStatus === 1) {
        if (kgSt) { kgSt.textContent = '请使用' + kgMeta.app + '扫码'; kgSt.className = ''; }
      } else if (kgStatus === 2) {
        if (kgSt) { kgSt.textContent = '已扫码，请在手机确认…'; kgSt.className = 'scan'; }
      } else if (kgStatus === 4 && (kg.loggedIn || (kg.data && kg.data.loggedIn))) {
        if (kgSt) { kgSt.textContent = kgMeta.label + '登录成功'; kgSt.className = 'scan'; }
        stopQrPoll();
        if (qrProvider === 'kugouMusic') {
          kugouMusicLoginStatus = normalizeKugouMusicLoginStatus(Object.assign({}, kg.data || {}, { loggedIn: true }));
        } else {
          kugouLoginStatus = normalizeKugouLoginStatus(Object.assign({}, kg.data || {}, { loggedIn: true }));
        }
        activeAccountProvider = qrProvider;
        renderUserBtn();
        setTimeout(async function(){
          var fresh = qrProvider === 'kugouMusic' ? await refreshKugouMusicLoginStatus() : await refreshKugouLoginStatus();
          closeLoginModal();
          showToast(kgMeta.label + '已登录: ' + ((fresh && fresh.nickname) || ''));
        }, 500);
      }
    } catch (e) {
      console.warn('KuGou QR check failed:', e);
    }
    return;
  }
  try {
...
function updateUserModalUi() {
  ...
  var addKugouMusic = document.getElementById('account-add-kugou-music');
  if (chip) {
  ...
  if (vipEl) {
    if (activeAccountProvider === 'netease') {
      ...
      var kgVipLabel = hasProviderVip('kugou', st) ? '酷狗会员' : '酷狗概念版会话';
      vipEl.textContent = 'UID: ' + ((st && st.userId) || '-') + '  ·  ' + kgVipLabel;
      vipEl.style.color = hasProviderVip('kugou', st) ? 'rgba(92,204,166,0.86)' : 'rgba(92,204,166,0.58)';
    } else {
      var kgmVipLabel = hasProviderVip('kugouMusic', st) ? '酷狗音乐会员' : '酷狗音乐会话';
      vipEl.textContent = 'UID: ' + ((st && st.userId) || '-') + '  ·  ' + kgmVipLabel;
      vipEl.style.color = hasProviderVip('kugouMusic', st) ? 'rgba(106,166,255,0.86)' : 'rgba(106,166,255,0.58)';
    }
  }
  ['netease','qq','kugou','kugouMusic','both'].forEach(function(key){
  ...
  if (addKugou) addKugou.textContent = hasPlatformLogin('kugou') ? '查看酷狗' : '补登酷狗';
  if (addKugouMusic) addKugouMusic.textContent = hasPlatformLogin('kugouMusic') ? '查看酷狗音乐' : '补登酷狗音乐';
  if (logoutBtn) logoutBtn.textContent = activeAccountProvider === 'qq' ? '退出 QQ 音乐' : (activeAccountProvider === 'kugou' ? '退出酷狗概念版' : (activeAccountProvider === 'kugouMusic' ? '退出酷狗音乐' : '退出网易云'));
...
function enableDualAccountView() {
  ...
  if (loggedProviderCount() < 2) {
    openProviderLogin(!hasPlatformLogin('netease') ? 'netease' : (!hasPlatformLogin('qq') ? 'qq' : (!hasPlatformLogin('kugou') ? 'kugou' : 'kugouMusic')));
    return;
  }
...
async function logoutActiveAccount() {
  if (activeAccountProvider === 'kugouMusic') {
    try { await apiJson('/api/kugou-music/logout'); } catch (e) {}
    kugouMusicLoginStatus = { provider: 'kugouMusic', platform: 'music', loggedIn: false, nickname: '酷狗音乐', userId: '', avatar: '', vipType: 0, tokenReady: false, deviceReady: false };
    kugouMusicPlaylists = [];
    userPlaylists = userPlaylists.filter(function(pl){ return pl.provider !== 'kugouMusic'; });
    dualAccountMode = loggedProviderCount() >= 2;
    activeAccountProvider = firstLoggedProvider();
    renderUserBtn();
  ...
  if (activeAccountProvider === 'kugou') {
    ...
    kugouLoginStatus = { provider: 'kugou', platform: 'lite', loggedIn: false, nickname: '酷狗概念版', userId: '', avatar: '', vipType: 0, tokenReady: false, deviceReady: false };
    kugouPlaylists = [];
    userPlaylists = userPlaylists.filter(function(pl){ return pl.provider !== 'kugou'; });
    dualAccountMode = loggedProviderCount() >= 2;
    activeAccountProvider = firstLoggedProvider();
...
function resetParticleRotationTarget(syncVisual) {
  ...
  if (syncVisual && customShapeGroup) {
    customShapeGroup.rotation.set(0, 0, 0);
    customShapeGroup.userData.autoSpinY = 0;
  }
}
...
function rebaseParticleRotationAxis(axis) {
  ...
  if (customShapeGroup) customShapeGroup.rotation[axis] -= offset;
...
function tickGestureRotation(dt) {
  ...
  if (gestureActive && handLmSmooth && uniforms.uHandActive && performance.now() - handLmLastSeen > 200) {
...
var peekGuardUntil = { search:0, fx:0, pl:0 };
function setPeek(el, on, key) {
  if (!el) return;
  /* 影视态铁律：音乐态「歌单/队列」面板在影视空间彻底失效（不 peek、不聚焦）。
     video-space-active 仅在影视态存在，音乐态永不命中，故不影响音乐态逻辑。 */
  if (key === 'pl' && document.body.classList.contains('video-space-active')) return;
  ...
  if (on) {
    ...
    if (key === 'search') peekGuardUntil[key] = performance.now() + 500;
  ...
  } else if (!(key === 'search' && performance.now() < peekGuardUntil[key])) {
...
function isPlaylistPanelFocusActive(inTrigger, inPanel, pp, ex, ppRect) {
  /* 影视态铁律：歌单/队列聚焦区在影视空间不生效，避免鼠标移到左侧残留「聚焦效果」。 */
  if (document.body.classList.contains('video-space-active')) return false;
...
window.addEventListener('mousemove', function(e){
  ...
  if (document.body.classList.contains('splash-active')) {
    if (typeof updateShelfHoverCueFromPointer === 'function') updateShelfHoverCueFromPointer(null);
  ...
  if (ey < 66 || inSearchPanel || searchFocused || uploadTipOpen || sa.contains(e.target)) setPeek(sa, true, 'search');
...
var splashGlShaders = null;          // [vertexShader, fragmentShader] —— 无扩展时同步校验用
var splashParallelExt = null;         // KHR_parallel_shader_compile（splash 独立上下文）
var splashGlReady = false;            // 异步链接完成、可安全首绘
var splashGlCompilePending = false;   // 已提交编译/链接，尚未确认完成
// A1 (2026-08-19): Program Binary 磁盘缓存 —— WebGL2 下首次编译完成后 getProgramBinary 存
// localStorage，二次启动 programBinary 同步恢复（零 compileShader 阻塞，治入场期 ~1.2s 单帧）。
// 指纹含 shader 源码全文 + GPU vendor/renderer + GL 版本，任一变化自动失效回退原编译路径。
var SPLASH_PROGBIN_KEY = 'stellaflix-splash-progbin-v1';
var splashGlIsWebGL2 = false;         // A1: WebGL2 上下文（Program Binary API 需要）
var splashGlProgBinFp = null;         // A1: 本次编译指纹（供完成后保存）
function splashGlFingerprint(gl, vs, fs) {
  var vendor = '', renderer = '', version = '';
  try {
  ...
  var str = vs + '\n' + fs + '|' + vendor + '|' + renderer + '|' + version;
  var h = 5381;
  for (var i = 0; i < str.length; i++) h = ((h << 5) + h + str.charCodeAt(i)) | 0;
  return (h >>> 0).toString(36) + '.' + str.length;
}
function loadSplashProgBin(fp) {
  try {
    var raw = localStorage.getItem(SPLASH_PROGBIN_KEY);
    if (!raw) return null;
    var obj = JSON.parse(raw);
    if (!obj || obj.fp !== fp || !obj.bin) return null;
    var binStr = atob(obj.bin);
    var bytes = new Uint8Array(binStr.length);
    for (var i = 0; i < binStr.length; i++) bytes[i] = binStr.charCodeAt(i);
    return { format: obj.fmt, binary: bytes.buffer };
  } catch (e) { return null; }
}
function saveSplashProgBin(gl, program) {
  if (!splashGlProgBinFp || !gl || !program) return;
  if (typeof gl.getProgramBinary !== 'function') return;
  try {
    var ret = gl.getProgramBinary(program);
    if (!ret || !ret.binary) return;
    var bytes = new Uint8Array(ret.binary);
    var parts = [];
    for (var i = 0; i < bytes.length; i += 8192) parts.push(String.fromCharCode.apply(null, bytes.subarray(i, i + 8192)));
    localStorage.setItem(SPLASH_PROGBIN_KEY, JSON.stringify({ fp: splashGlProgBinFp, fmt: ret.format, bin: btoa(parts.join('')), ts: Date.now() }));
  } catch (e) {}
}
...
function initMineradioSplashWebgl(canvas) {
  ...
  try {
    ...
    // A1: 优先 WebGL2（Program Binary 缓存需要；GLSL1 shader 在 WebGL2 下向后兼容，视觉零变化）。
    gl = canvas.getContext('webgl2', _ctxAttrs) || canvas.getContext('webgl', _ctxAttrs) || canvas.getContext('experimental-webgl');
  } catch (e) {
    gl = null;
  }
  splashGlIsWebGL2 = !!(gl && typeof WebGL2RenderingContext !== 'undefined' && gl instanceof WebGL2RenderingContext);
  if (!gl) return false;
  ...
  var fragmentSource = [
    ...
    '    acc += 0.00715 * fi * fi / max(abs(f), 0.0065);',
    '  }',
  ...
  // X1-splash: 不再同步查 COMPILE_STATUS —— 那会阻塞主线程。编译/链接状态改在
  // rAF 轮询 COMPLETION_STATUS_KHR(有扩展)或首帧后同步校验(无扩展)时确认。
  ...
  // A1 (2026-08-19): Program Binary 缓存命中路径 —— WebGL2 下二次启动 programBinary 同步恢复
  // （零 compileShader 阻塞），指纹不匹配/链接失败自动弃缓存回退原编译路径。
  splashGlProgBinFp = null;
  var program = gl.createProgram();
  if (splashGlIsWebGL2) {
    splashGlProgBinFp = splashGlFingerprint(gl, vertexSource, fragmentSource);
    var _cached = loadSplashProgBin(splashGlProgBinFp);
    if (_cached) {
      var _binOk = false;
      try {
        gl.programBinary(program, _cached.format, _cached.binary);
        _binOk = !!gl.getProgramParameter(program, gl.LINK_STATUS);
      } catch (e) { _binOk = false; }
      if (_binOk) {
        splashGl = gl;
        ...
        splashGlShaders = null;
        splashGlReady = true;        // 缓存命中：立即可用，零编译
        splashGlCompilePending = false;
        splashCtx = null;            // WebGL 立即接管绘制
        return true;
      }
      // 缓存失效（驱动更新/源码变化）：弃此 program，走原编译路径
      try { gl.deleteProgram(program); } catch (e2) {}
      try { localStorage.removeItem(SPLASH_PROGBIN_KEY); } catch (e3) {}
  ...
  gl.linkProgram(program); // 仅提交链接，不在此同步等待 —— 同步 LINK_STATUS 是 X0/B 阻塞根因
  // 保留 shader 引用供无扩展时同步校验；有扩展则待 COMPLETION_STATUS_KHR 轮询
  splashGlShaders = [vertexShader, fragmentShader];
  splashParallelExt = gl.getExtension('KHR_parallel_shader_compile');
  ...
  gl.disable(gl.DEPTH_TEST);
  gl.disable(gl.CULL_FACE);
  // X1-splash: 标记"已提交编译、尚未就绪"。splashCtx 暂不置空 —— 2D 继续绘制直到
  // 异步链接完成(或失败回退)。无扩展时降级为首帧后同步校验(一次性轻微 hitch)。
  splashGlReady = false;
  splashGlCompilePending = true;
  if (!splashParallelExt) {
    // 无 KHR_parallel_shader_compile：首帧 2D 已绘制后，再同步校验(可能阻塞 ~1~1.5s，
    // 但已避开启动首绘，不会拖慢入场启动)。
    setTimeout(function () { finalizeSplashGlSync(); }, 0);
  }
  // 安全网(2026-08-18 补)：扩展异常/驱动未并行化导致异步链接 3s 内未确认时，降级为同步校验
  // 兜底，避免 WebGL 永远 pending → 开屏空白(空白比 2D 兜底更糟)。rAF 轮询已正常就绪则此处为空操作。
  setTimeout(function () { if (splashGlCompilePending) finalizeSplashGlSync(); }, 3000);
  return true; // 返回"已提交"，非"已就绪"
}


// X1-splash (2026-08-19): 无 KHR_parallel_shader_compile 时的同步校验兜底 + 失败清理。
function finalizeSplashGlSync() {
  if (!splashGl || !splashGlProgram || !splashGlCompilePending) return;
  var vs = splashGlShaders && splashGlShaders[0];
  var fs = splashGlShaders && splashGlShaders[1];
  if (!vs || !fs ||
      !splashGl.getShaderParameter(vs, splashGl.COMPILE_STATUS) ||
      !splashGl.getShaderParameter(fs, splashGl.COMPILE_STATUS) ||
      !splashGl.getProgramParameter(splashGlProgram, splashGl.LINK_STATUS)) {
    console.warn('[Splash] WebGL sync compile/link failed, keep 2D fallback');
    cleanupSplashGl();
    return;
  }
  splashGlCompilePending = false;
  splashGlReady = true;
  splashCtx = null; // WebGL 接管绘制
  saveSplashProgBin(splashGl, splashGlProgram); // A1: 同步校验路径同样存 binary 缓存
}

function cleanupSplashGl() {
  splashGl = null;
  ...
  splashGlReady = false;
  splashGlCompilePending = false;
  // WebGL 彻底失败：若 canvas 尚未被 webgl 占用则回退 2D 兜底；若已被 webgl 占用，
  // getContext('2d') 返 null，则该边缘情形由 body 背景兜底，不影响主流程。
  if (!splashCtx && splashCanvas) {
    try { splashCtx = splashCanvas.getContext('2d'); } catch (e) { splashCtx = null; }
  }
}

function drawMineradioSplashWebgl(elapsed) {
  var gl = splashGl;
  if (!gl || !splashGlReady || !splashGlProgram || !splashGlUniforms) return;
...
(function scheduleInitMineradioSplashCanvas() {
  if (typeof requestAnimationFrame === 'function') {
    requestAnimationFrame(initMineradioSplashCanvas);
  } else {
    setTimeout(initMineradioSplashCanvas, 0);
  }
})();
...
function initMineradioSplashCanvas() {
  ...
  // 恢复开屏 WebGL 霓虹特效（最早的 3D 效果）。
  // ⚠️ 关键铁律：必须先 getContext('webgl') 取得上下文，绝不能先 getContext('2d')。
  //    同一 <canvas> 元素只能拥有一种上下文类型 —— 先占 2D 会让后续 getContext('webgl')
  //    永远返回 null（X1-splash 失效 / 优化幻觉根因）。仅当 WebGL 不可用(无上下文/编译失败)
  //    时才回退 2D 兜底。异步编译(KHR_parallel_shader_compile)保持启动期不阻塞主线程，
  //    视觉与 earliest 完全一致，仅编译过程不阻塞首绘。
  if (!initMineradioSplashWebgl(splashCanvas)) {
    splashCtx = splashCanvas.getContext('2d'); // 仅 WebGL 不可用时回退
  }
  drawMineradioSplash(); // WebGL 异步编译窗口内 splashCtx 为 null → body 背景兜底；就绪后自动接管
}

// (initMineradioSplashWebglAfterScene 已移除：splash WebGL 恢复为启动期异步编译，见 X1-splash 注释)

function drawMineradioSplash() {
  if (!splashAnimating || (!splashCtx && !splashGl)) return;
  requestAnimationFrame(drawMineradioSplash);
  // X1-splash: 轮询异步编译完成(仅当扩展可用且尚未就绪)
  if (splashGl && !splashGlReady && splashGlCompilePending && splashParallelExt) {
    try {
      if (splashGl.getProgramParameter(splashGlProgram, splashParallelExt.COMPLETION_STATUS_KHR) === true) {
        if (splashGl.getProgramParameter(splashGlProgram, splashGl.LINK_STATUS)) {
          splashGlCompilePending = false;
          splashGlReady = true;
          splashCtx = null; // WebGL 接管绘制
          saveSplashProgBin(splashGl, splashGlProgram); // A1: 编译完成，存 binary 缓存供二次启动秒载
        } else {
          console.warn('[Splash] WebGL async link failed, keep 2D fallback');
          cleanupSplashGl();
        }
      }
    } catch (e) {
  ...
  if (splashGl && splashGlReady) {
    drawMineradioSplashWebgl(elapsed);
    return;
  }
  if (!splashCtx) {
    // WebGL 尚未就绪且 2D 兜底未启用（异步编译窗口）——本帧跳过绘制，body 背景兜底，rAF 继续轮询
    return;
  }
...
function dismissSplash() {
  ...
  // B (2026-08-18): 3D 场景在启动期早构建；重型 shader 编译推迟到入场播完(body 加 .splash-play)后
  // 由 04a 启动，并仍在 splash-active 遮挡期内完成（点击进入门控于 sfv:shader-compile-done）。
  // 故此处通常仅作保底（startupSceneBuilt 为 false 时一次性构建）。退出动画由合成器跑，不卡视觉。
  if (!mainSceneBuiltAfterSplash) {
    mainSceneBuiltAfterSplash = true;
    buildMainSceneAfterSplash();
  }
...
// X0 (2026-08-18): 启动策略对齐主枝 Mineradio-2.1.0 —— 3D 主场景在启动期经
// runStartupSequence 早构建（startupSceneBuilt=true），星空/fx/shelf 重型 shader 的 ~2s
// 同步编译落在 splash 完全遮挡期（经 04a-three-host.js 每 520ms 渲染真实场景）。
// dismiss 时场景已热，故本函数仅作"极早点击"的保底构建，不再触发重型编译。
var mainSceneBuiltAfterSplash = false;
var startupSceneBuilt = false;
function buildMainSceneAfterSplash() {
  markAppPerf('scene-build-start');
  // 仅当用户极早于早构建完成前点击（startupSceneBuilt 仍为 false）时，这里一次性兜底构建。
  if (!startupSceneBuilt) {
    try {
      if (typeof setShelfMode === 'function') setShelfMode(fx.shelf);
      if (typeof applyStartupStarfieldPreset === 'function') applyStartupStarfieldPreset();
      if (fx.floatLayer && typeof createFloatLayer === 'function') createFloatLayer();
      if (fx.particleLyrics && typeof createLyricsParticles === 'function') createLyricsParticles();
      if (fx.backCover && typeof createBackCoverLayer === 'function') createBackCoverLayer();
    } catch (e) {}
  }
  markAppPerf('scene-build-done');
  try {
    if (renderer && scene && camera) {
      markAppPerf('scene-compile-start');
      // B: 场景编译已在 .splash-play 之后的 splash-active 遮挡期内完成，此处仅兜底渲染一帧（通常近瞬时）。
      renderer.render(scene, camera);
      markAppPerf('scene-compile-end');
    }
  } catch (e) {}
  markAppPerf('scene-after-splash-done');
}

var splashCompileDone = false;
function markSplashReadyToEnter() {
  // B (2026-08-18): 点击进入必须等重型 shader 编译完成（04a 派发 sfv:shader-compile-done），
  // 否则早期点击会触发未编译场景的首帧同步编译冻结（dismiss 时 render 卡 ~2.3s）。
  // 编译已在入场播完(ENTRANCE_DONE_MS)后、body 加 .splash-play 时由 04a 启动；此处的"点击进入"
  // 门控在该完成事件上，兜底超时由 DOMContentLoaded 块的 .splash-play 定时器负责。
  var s = document.getElementById('splash');
  if (!s || s.classList.contains('hide') || s.classList.contains('exiting')) return;
  if (splashReadyToEnter) return;
  if (window.__sfvShaderCompileDone || splashCompileDone) { _showSplashReady(); return; }
  window.addEventListener('sfv:shader-compile-done', function onCompileDone() {
    window.removeEventListener('sfv:shader-compile-done', onCompileDone);
    splashCompileDone = true;
    _showSplashReady();
  });
}

function _showSplashReady() {
  var s = document.getElementById('splash');
  if (!s || s.classList.contains('hide') || s.classList.contains('exiting')) return;
  if (splashReadyToEnter) return;
  ...
  s.setAttribute('aria-label', '点击进入 Stellaflix');
}
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // B (2026-08-18): entrance-first sequencing。入场动画在合成器上从 0 帧流畅播放（无 paused 闸），
  // 重型 shader 编译推迟到入场播完之后——入场完成后才给 body 加 .splash-play，许可 04a 启动编译；
  // 这样 ~2.3s 编译被藏入场后的静态 splash 内，用户无感知。ENTRANCE_DONE_MS 之外不再长等。
  var ENTRANCE_DONE_MS = reduceSplashMotion ? 1000 : 5300;
  setTimeout(function(){
    document.body.classList.add('splash-play');
    // 兜底：编译异常/无 WebGL 时，最迟 ENTRANCE_DONE_MS+4s 后强制放行点击进入，避免永久卡死。
    setTimeout(function(){
      if (!splashReadyToEnter) { splashCompileDone = true; _showSplashReady(); }
    }, 4000);
  }, ENTRANCE_DONE_MS);
  ...
  // 点击进入必须等编译完成（见 markSplashReadyToEnter 的事件监听）；此处仅提前挂监听 + 兜底计时。
...
function currentDesktopSongMeta() {
  ...
  return {
    title: song.name || song.title || 'Stellaflix',
...
function currentDesktopLyricSnapshot() {
  var lyricWindow = audio ? selectCurrentLyricWindow() : null;
  if (lyricWindow && lyricWindow.current) {
    return {
      text: normalizeDesktopLyricText(lyricWindow.current.text || currentLyricFallbackText()),
      nextText: lyricWindow.next ? normalizeDesktopLyricText(lyricWindow.next.text) : '',
      progress: lyricWindow.progress,
      progressSpan: lyricWindow.progressSpan
    };
  }
  if (stageLyrics && stageLyrics.currentText) {
    return {
      text: normalizeDesktopLyricText(stageLyrics.currentText),
      nextText: fx.lyricNextLine !== false ? normalizeDesktopLyricText(stageLyrics.nextText) : '',
  ...
  return { text: normalizeDesktopLyricText(currentDesktopSongMeta().title || 'Stellaflix'), nextText: '', progress: 0, progressSpan: 4.8 };
}
...
function desktopLyricsPayload(forceBeatMap) {
  ...
  var payload = {
    ...
    text: lyric.text,
    nextText: fx.lyricNextLine !== false ? lyric.nextText : '',
...
// === [03-lyrics · 桌面歌词块] 区域横幅（2026-08-07 标订；2.3 仅标记边界，不搬迁） ===
...
function customSourceStatusText(item, active) {
  if (active) return '使用中';
  if (item && item.status === 'failed') return '初始化失败';
  if (item && item.status === 'warning') return '需要检查';
  return '未启用';
}

function customSourceCapabilityText(item) {
  var sources = item && item.sources && typeof item.sources === 'object' ? item.sources : {};
  var platformLabels = { wy: '网易云', tx: 'QQ 音乐', kg: '酷狗', kw: '酷我', mg: '咪咕', local: '本地' };
  var platforms = Object.keys(sources).filter(function(key){
    return sources[key] && Array.isArray(sources[key].actions) && sources[key].actions.indexOf('musicUrl') >= 0;
  }).map(function(key){ return platformLabels[key] || key; });
  var qualities = [];
  Object.keys(sources).forEach(function(key){
    var list = sources[key] && Array.isArray(sources[key].qualitys) ? sources[key].qualitys : [];
    list.forEach(function(quality){ if (qualities.indexOf(quality) < 0) qualities.push(quality); });
  });
  if (!platforms.length) return '脚本尚未声明可用的歌曲地址解析能力';
  return '平台: ' + platforms.join(' / ') + (qualities.length ? ' · 音质: ' + qualities.join(' / ') : '');
}
...
function setCustomSourceState(result) {
  result = result || {};
  if (Array.isArray(result.items)) customSourceState.items = result.items;
  customSourceState.activeId = String(result.activeId || '');
  var entry = document.getElementById('custom-source-btn');
  if (entry) entry.classList.toggle('active', !!customSourceState.activeId);
  renderCustomSourceList();
  if (result.updateAlert && result.updateAlert.log) showToast('第三方音源提示: ' + result.updateAlert.log);
  if (result.error) showToast('第三方音源: ' + result.error);
}
...
function renderCustomSourceList() {
  ...
  if (!customSourceState.items.length) {
    list.appendChild(makeCustomSourceElement('div', 'custom-source-empty', '尚未导入脚本。导入后仍需手动启用。'));
    return;
  }
  customSourceState.items.forEach(function(item){
    var active = item.id === customSourceState.activeId || item.active === true;
    var row = makeCustomSourceElement('article', 'custom-source-item' + (active ? ' active' : ''));
    var head = makeCustomSourceElement('div', 'custom-source-item-head');
    var identity = makeCustomSourceElement('div');
    identity.appendChild(makeCustomSourceElement('div', 'custom-source-name', item.name || '未命名音源'));
    identity.appendChild(makeCustomSourceElement('div', 'custom-source-meta', [
      item.version ? 'v' + item.version : '',
      item.author || ''
    ].filter(Boolean).join(' · ')));
    head.appendChild(identity);
    head.appendChild(makeCustomSourceElement('div', 'custom-source-status', customSourceStatusText(item, active)));
    row.appendChild(head);
    row.appendChild(makeCustomSourceElement('div', 'custom-source-capabilities', customSourceCapabilityText(item)));
    if (item.message) row.appendChild(makeCustomSourceElement('div', 'custom-source-message', item.message));

    var actions = makeCustomSourceElement('div', 'custom-source-actions');
    if (!active) {
      var activateButton = makeCustomSourceElement('button', 'primary', '启用');
      activateButton.type = 'button';
      activateButton.addEventListener('click', function(){ activateCustomSource(item.id); });
      actions.appendChild(activateButton);
    }
    var replaceButton = makeCustomSourceElement('button', '', '替换');
    replaceButton.type = 'button';
    replaceButton.addEventListener('click', function(){ replaceCustomSource(item.id); });
    actions.appendChild(replaceButton);
    var removeButton = makeCustomSourceElement('button', 'danger', '删除');
    ...
    toggle.appendChild(document.createTextNode('允许脚本更新提示'));
...
async function refreshCustomSources() {
  var api = getDesktopWindowApi();
  if (!api || typeof api.listCustomSources !== 'function') {
    customSourceState.items = [];
    customSourceState.activeId = '';
    var list = document.getElementById('custom-source-list');
    if (list) {
      list.textContent = '';
      list.appendChild(makeCustomSourceElement('div', 'custom-source-empty', '第三方音源管理仅在桌面应用中可用。'));
    }
    return;
  }
...
async function runCustomSourceAction(action, successText) {
  ...
  try {
    ...
    showToast('第三方音源操作失败: ' + (error && error.message ? error.message : error));
  } finally {
...
async function loadBundledSources() {
  var api = getDesktopWindowApi();
  var list = document.getElementById('custom-source-bundled-list');
  if (!list) return;
  if (!api || typeof api.listBundledCustomSources !== 'function') {
    list.textContent = '';
    list.appendChild(makeCustomSourceElement('div', 'custom-source-empty', '内置音源仅在桌面应用中可用。'));
    return;
  }
  var sources;
  try { sources = await api.listBundledCustomSources(); }
  catch (error) {
    list.textContent = '';
    list.appendChild(makeCustomSourceElement('div', 'custom-source-empty', '读取内置音源失败: ' + (error && error.message ? error.message : error)));
    return;
  }
  list.textContent = '';
  if (!Array.isArray(sources) || !sources.length) {
    list.appendChild(makeCustomSourceElement('div', 'custom-source-empty', '未检测到内置音源脚本。'));
    return;
  }
  var importedNames = (customSourceState.items || []).map(function(item){ return item.sourceFileName; });
  sources.forEach(function(src){
    var row = makeCustomSourceElement('article', 'custom-source-item');
    var head = makeCustomSourceElement('div', 'custom-source-item-head');
    var identity = makeCustomSourceElement('div');
    identity.appendChild(makeCustomSourceElement('div', 'custom-source-name', src.name || src.fileName));
    var updateState = bundledSourceUpdateState[src.fileName];
    var updateMetaBits = [src.version ? 'v' + src.version : '', src.author || '', src.fileName];
    if (updateState && updateState.ok && updateState.updateAvailable) {
      updateMetaBits.push('脚本 v' + (updateState.remoteVersion || '?') + ' 可更新');
    } else if (updateState && !updateState.ok) {
      var failReason = /404/.test(String(updateState.error || '')) ? '(404: 仓库可能未推送)' : '(网络异常)';
      updateMetaBits.push('脚本拉取失败' + failReason);
    }
    identity.appendChild(makeCustomSourceElement('div', 'custom-source-meta', updateMetaBits.filter(Boolean).join(' · ')));
    head.appendChild(identity);
    var imported = importedNames.indexOf(src.fileName) !== -1;
    head.appendChild(makeCustomSourceElement('div', 'custom-source-status', imported ? '已导入' : '内置'));
    row.appendChild(head);
    // 上游项目信号提示(与脚本更新独立,基于 QingMusic release 版本比对)
    if (updateState && updateState.upstreamSignal && updateState.upstreamSignal.available) {
      var dismissedVer = '';
      try { dismissedVer = window.localStorage.getItem('stellaflix-bundled-upstream-dismiss-' + src.fileName) || ''; } catch (e) {}
      if (dismissedVer !== updateState.upstreamSignal.latestVersion) {
        var upstreamHint = makeCustomSourceElement('div', 'custom-source-upstream-hint');
        upstreamHint.appendChild(makeCustomSourceElement('span', '', '上游 QingMusic 已更新到 ' + updateState.upstreamSignal.latestVersion + ',建议复核 qing 适配脚本'));
        var dismissBtn = makeCustomSourceElement('button', '', '标记已复核');
        dismissBtn.type = 'button';
        dismissBtn.addEventListener('click', function(){
          try { window.localStorage.setItem('stellaflix-bundled-upstream-dismiss-' + src.fileName, updateState.upstreamSignal.latestVersion); } catch (e) {}
          loadBundledSources().catch(function(){});
        });
        upstreamHint.appendChild(dismissBtn);
        row.appendChild(upstreamHint);
      }
    }
    var actions = makeCustomSourceElement('div', 'custom-source-actions');
    if (!imported) {
      var btn = makeCustomSourceElement('button', 'primary', '导入');
      btn.type = 'button';
      btn.addEventListener('click', function(){ importBundledSource(src.fileName); });
      actions.appendChild(btn);
    }
    if (updateState && updateState.ok && updateState.updateAvailable) {
      var updateBtn = makeCustomSourceElement('button', 'primary', '更新到上游版本');
      updateBtn.type = 'button';
      updateBtn.addEventListener('click', function(){ applyBundledSourceUpdate(src.fileName); });
...
// ---------- 内置音源跟随上游更新 ----------
var bundledSourceUpdateState = {};
var BUNDLED_SOURCES_CHECKED_AT_KEY = 'stellaflix-bundled-sources-checked-at';
var BUNDLED_SOURCES_AUTO_CHECK_INTERVAL = 24 * 60 * 60 * 1000;
async function checkBundledSourceUpdates(opts) {
  opts = opts || {};
  var api = getDesktopWindowApi();
  if (!api || typeof api.checkBundledSourceUpdates !== 'function') {
    if (!opts.silent) showToast('内置音源更新检查仅在桌面应用中可用');
    return;
  }
  var result;
  try { result = await api.checkBundledSourceUpdates(); }
  catch (error) {
    if (!opts.silent) showToast('内置音源更新检查失败: ' + (error && error.message ? error.message : error));
    return;
  }
  bundledSourceUpdateState = {};
  (result && result.results || []).forEach(function(item){
    if (item && item.fileName) bundledSourceUpdateState[item.fileName] = item;
  });
  var summaryApi = (typeof window !== 'undefined' && window.StellaflixBundledSummary) ? window.StellaflixBundledSummary : null;
  var decision = summaryApi ? summaryApi.decideBundledUpdateToast(result && result.results) : null;
  if (!opts.silent && decision) showToast(decision.text);
  loadBundledSources().catch(function(){});
}
async function applyBundledSourceUpdate(fileName) {
  var api = getDesktopWindowApi();
  if (!api || typeof api.applyBundledSourceUpdate !== 'function' || !fileName) return;
  showToast('正在更新内置音源…');
  try {
    var result = await api.applyBundledSourceUpdate(String(fileName || ''));
    if (!result || result.ok === false) throw new Error(result && result.error || 'UPDATE_FAILED');
    delete bundledSourceUpdateState[fileName];
    showToast(result.hotReplaced
      ? '已更新到 v' + (result.version || '?') + ',启用的音源已热替换'
      : (result.updated ? '已更新到 v' + (result.version || '?') : '内置音源已是最新'));
  } catch (error) {
    showToast('内置音源更新失败: ' + (error && error.message ? error.message : error));
  }
  loadBundledSources().catch(function(){});
  refreshCustomSources().catch(function(){});
}
function scheduleBundledSourcesAutoCheck() {
  var api = getDesktopWindowApi();
  if (!api || typeof api.checkBundledSourceUpdates !== 'function') return;
  var last = 0;
  try { last = Number(window.localStorage.getItem(BUNDLED_SOURCES_CHECKED_AT_KEY)) || 0; }
  catch (e) {}
  if (last && Date.now() - last < BUNDLED_SOURCES_AUTO_CHECK_INTERVAL) return;
  try { window.localStorage.setItem(BUNDLED_SOURCES_CHECKED_AT_KEY, String(Date.now())); } catch (e) {}
  setTimeout(function(){ checkBundledSourceUpdates({ silent: true }).catch(function(){}); }, 2500);
}
scheduleBundledSourcesAutoCheck();

function importBundledSource(fileName) {
  var api = getDesktopWindowApi();
  if (!api || typeof api.importBundledCustomSource !== 'function') return;
  runCustomSourceAction(function(){ return api.importBundledCustomSource(String(fileName || '')); }, '内置音源已导入，启用后参与播放')
    .then(function(){ return loadBundledSources(); })
    .catch(function(){});
}
...
function openCustomSourceModal() {
  ...
  refreshCustomSources().catch(function(error){ showToast('第三方音源列表读取失败: ' + error.message); });
  loadBundledSources().catch(function(){});
}
...
function importCustomSource() {
  ...
  runCustomSourceAction(function(){ return api.importCustomSource(); }, '脚本已验证并导入，启用后才会参与播放');
}

function replaceCustomSource(id) {
  var api = getDesktopWindowApi();
  if (!api || typeof api.replaceCustomSource !== 'function') return;
  runCustomSourceAction(function(){ return api.replaceCustomSource(id); }, '脚本已验证并替换');
}

function activateCustomSource(id) {
  var api = getDesktopWindowApi();
  if (!api || typeof api.activateCustomSource !== 'function') return;
  runCustomSourceAction(function(){ return api.activateCustomSource(id); }, '第三方音源已启用，官方播放仍然优先');
}

function deactivateCustomSource() {
  var api = getDesktopWindowApi();
  if (!api || typeof api.deactivateCustomSource !== 'function') return;
  runCustomSourceAction(function(){ return api.deactivateCustomSource(); }, '第三方音源已停用');
}

function removeCustomSource(id, name) {
  var api = getDesktopWindowApi();
  if (!api || typeof api.removeCustomSource !== 'function') return;
  if (!window.confirm('删除“' + (name || '这个脚本') + '”？此操作只删除本机导入副本。')) return;
  runCustomSourceAction(function(){ return api.removeCustomSource(id); }, '脚本已删除');
}
...
function bindCustomSourceManager() {
  var modal = document.getElementById('custom-source-modal');
  if (modal) modal.addEventListener('click', function(event){ if (event.target === modal) closeCustomSourceModal(); });
  var api = getDesktopWindowApi();
...
//  启动
// ============================================================
// 2.5b-regression-fix: defer the entire startup sequence to DOMContentLoaded.
// Legacy top-level previously executed this BEFORE 08-fx-visual.js (loaded LAST)
// ran, so fx (saved real values) and 08-owned functions (e.g. syncCustomShapeLayer,
// called by bindFxPanel) were unavailable → ReferenceErrors / wrong init.
// DOMContentLoaded fires after all 4 classic scripts (music → globals-bridge → host → 08).
function scheduleStartupTask(fn, timeout) {
  if (typeof requestIdleCallback === 'function') {
    try { requestIdleCallback(fn, { timeout: timeout || 2000 }); } catch (e) { setTimeout(fn, timeout || 1); }
  } else {
...
applySavedLyricPaletteState();
bindQualityControl();
bindEqualizerControl();
bindVolumeControls();
applyUserCapsuleAutoHideState();
applyFxFabAutoHideState();
applyControlsAutoHidePreference();
applyPlaylistPanelPinState(false);
startupLoginStatusPromise = Promise.all([refreshLoginStatus(), refreshQQLoginStatus(), refreshKugouLoginStatus(), refreshKugouMusicLoginStatus()]);

// X0 (2026-08-18): 早构建 3D 主场景，对齐主枝 Mineradio-2.1.0 启动策略。
// 星空/fx 重型 shader 在 splash 完全遮挡期经 04a-three-host.js 每 ~520ms 渲染真实场景编译，
// ~2s 同步编译完全藏在 splash 之下；dismiss 时场景已热，零编译、丝滑进入。
// （历史方案曾把 3D 场景构建推迟到 dismiss，结果把同一处 2s 编译在"启动期/点击后"间挪动，属打地鼠。）
try {
  if (typeof applyStartupStarfieldPreset === 'function') applyStartupStarfieldPreset();
  if (typeof setShelfMode === 'function') setShelfMode(fx.shelf);
  if (fx.floatLayer && typeof createFloatLayer === 'function') createFloatLayer();
  if (fx.particleLyrics && typeof createLyricsParticles === 'function') createLyricsParticles();
  if (fx.backCover && typeof createBackCoverLayer === 'function') createBackCoverLayer();
  startupSceneBuilt = true;
} catch (e) {}

// 启动期其余重负载拆到空闲切片，避免阻塞 splash 动画 rAF。
// 注意：3D 场景构建（上条）不再推迟到 dismiss——已在 splash 遮挡期完成。
scheduleStartupTask(function(){
...
startKugouMusicLoginStatusAutoRefresh();
