...
(function (global) {
  ...
  // T-拆债#1：抽出的叶子层经 SFV.* 别名绑定复用（须在电影筛选器子模块之前加载）
  var buildParams = (SFV.movieFilterParams && SFV.movieFilterParams.buildParams) || function () { return {}; };
  var repaintYearTags = (SFV.movieFilterRepaint && SFV.movieFilterRepaint.repaintYearTags) || function () {};
  var repaintGenreTags = (SFV.movieFilterRepaint && SFV.movieFilterRepaint.repaintGenreTags) || function () {};
  var repaintRegionTags = (SFV.movieFilterRepaint && SFV.movieFilterRepaint.repaintRegionTags) || function () {};
  ...
  function buildBar(st, onToggle, onChange) {
    ...
    CFG.SORT_OPTIONS.forEach(function (opt) {
  ...
  function buildPanel(st, onChange) {
    ...
    var yearOpts = CFG.buildYearOptions();
    var genres = st.category === 'movie' ? CFG.MOVIE_GENRES : CFG.DOC_GENRES;
    var visibleCount = st.category === 'movie' ? CFG.MOVIE_GENRE_VISIBLE : CFG.DOC_GENRE_VISIBLE;
    ...
    var regionRow = buildFilterRow('地区', function (rowBody) {
      ...
      rowBody.appendChild(allBtn);
      CFG.REGION_OPTIONS.forEach(function (r) {
