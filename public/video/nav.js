...
(function (global) {
  ...
  function buildNav() {
    ...
    // T157：影视态搜索入口（替代音乐态 #top-right 胶囊内的搜索按钮，杜绝穿透）。
    // 点击直接 toggle 影视全页搜索；与 #sfv-nav 同为影视态自带 chrome，不会穿透任何分页。
    var searchBtn = document.createElement('button');
    searchBtn.type = 'button';
    searchBtn.id = 'sfv-nav-search-btn';
    searchBtn.className = 'sfv-nav-search-btn';
    searchBtn.title = '搜索影片';
    searchBtn.setAttribute('aria-label', '搜索影片');
    searchBtn.innerHTML = '<svg width="19" height="19" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>';
    searchBtn.addEventListener('click', function (e) {
      e.preventDefault();
      e.stopPropagation();
      if (global._sfvTryToggleSearch) global._sfvTryToggleSearch();
      else if (SFV.online && typeof SFV.online.toggleSearchPage === 'function') SFV.online.toggleSearchPage();
    });
