...
(function (global) {
  ...
  // 标记：心动(liked)/片单(inList)/收藏(faved) 三类布尔旗，按 vod key 索引。
  var KEY_FLAG = NS + 'flag'; // { [vodKey]: { liked, faved, inList } }
  // 元信息缓存：vod key → 基本元数据，保证即便源被删/离线，四类列表仍能渲染标题封面。
  var KEY_META = NS + 'meta'; // { [vodKey]: { key, title, pic, year, sourceId, vodId } }

  var FLAG_FIELDS = ['liked', 'faved', 'inList'];
  var HISTORY_CAP = 300;
  ...
  function getFlag(key) {
    ...
    var f = all[key] || {};
    return { liked: !!f.liked, faved: !!f.faved, inList: !!f.inList };
  }
  ...
  function setFlag(key, field, on) {
    ...
    // 三类旗全为假时直接删键，避免脏数据膨胀
    var f = all[key];
    if (!f.liked && !f.faved && !f.inList) delete all[key];
