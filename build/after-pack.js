...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Stellaflix icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'Stellaflix';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Stellaflix executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Stellaflix icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting Stellaflix resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Stellaflix',
    '--set-version-string', 'ProductName', 'Stellaflix',
    '--set-version-string', 'CompanyName', 'Stellaflix',
