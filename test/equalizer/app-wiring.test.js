...
// 音乐逻辑(均衡器音频图/状态/UI 接线)在 Step 2.0 已从 index.html 抽取到 legacy-music.js;
// 2.4 将 equalizer 模块真抽取到 02-equalizer-glue.js；01-part2 进一步将音频引擎核心
// (initAudio/primeManualAudioPlayback/增益 ramp/音量控制) 抽取到 04e-audio-engine.js
// （经典脚本，构建出 assets/04e-audio-engine.js）。equalizer 专属符号（状态/UI/音频图块）位于 glue；
// 音频引擎核心位于 04e；其余音乐逻辑仍在 legacy-music.js。
// 故 JS 片段/函数/字面量断言读取(glue+legacy+04e)合并源码；DOM/CSS/文案断言仍读 indexHtml。
const legacyMusicSrc = fs.readFileSync(path.join(root, 'src', 'music', 'legacy-music.js'), 'utf8');
const glueSrc = fs.readFileSync(path.join(root, 'src', 'music', '02-equalizer-glue.js'), 'utf8');
const e04eSrc = fs.readFileSync(path.join(root, 'src', 'music', '04e-audio-engine.js'), 'utf8');
const musicSrc = glueSrc + '\n' + legacyMusicSrc + '\n' + e04eSrc;
...
test('keeps beat analysis before equalizer processing', () => {
  assert.match(musicSrc, /source\.connect\(beatAnalyser\)/);
  assert.match(musicSrc, /analyser\.connect\(candidateGraph\.filters\[0\]\)/);
  assert.match(musicSrc, /candidateGraph\.limiter\.connect\(gainNode\)/);
  assert.match(musicSrc, /filter\.type\s*=\s*'peaking'/);
  assert.match(musicSrc, /filter\.Q\.value\s*=\s*1\.4/);
});

test('applies headroom and limiter only through equalizer state', () => {
  assert.match(musicSrc, /function applyEqualizerAudioState\(/);
  assert.match(musicSrc, /MineradioEqualizer\.calculateHeadroomDb/);
  assert.match(musicSrc, /MineradioEqualizer\.shouldEnableLimiter/);
  assert.match(musicSrc, /(?:equalizerLimiter|graph\.limiter)\.ratio/);
});

test('uses a safe default before equalizer UI state is initialized', () => {
  assert.match(musicSrc, /typeof equalizerState === 'undefined'/);
  assert.match(musicSrc, /MineradioEqualizer\.defaultState\(\)/);
});

test('falls back to the existing audible path when equalizer setup fails', () => {
  assert.match(musicSrc, /catch \(equalizerError\)[\s\S]*?analyser\.connect\(gainNode\)/);
});

// 2.4：equalizer 音频图函数已迁至 glue；01-part2：initAudio…resumeAudioAnalysis 段已迁至 04e。
// 重建 audioFunctionsSource = glue 音频图块 + 04e 的 initAudio…resumeAudioAnalysis 段，
// 使 initAudio 可调用已迁出的 equalizer 函数（setEqualizerAudioParam/applyEqualizerAudioState 等）。
const glueAudioStart = glueSrc.indexOf('function setEqualizerAudioParam(');
assert.notEqual(glueAudioStart, -1, 'equalizer audio functions must exist in glue');
const initAudioStart = e04eSrc.indexOf('function initAudio(');
const initAudioEnd = e04eSrc.indexOf('function resumeAudioAnalysis(');
assert.notEqual(initAudioStart, -1, 'initAudio must exist in 04e-audio-engine');
assert.notEqual(initAudioEnd, -1, 'audio function boundary must exist');
const audioFunctionsSource = glueSrc.slice(glueAudioStart) + '\n' + e04eSrc.slice(initAudioStart, initAudioEnd);
...
test('renders and binds an independent equalizer control', () => {
  ...
  assert.match(musicSrc, /function bindEqualizerControl\(/);
  assert.match(musicSrc, /bindEqualizerControl\(\)/);
  assert.match(musicSrc, /function closeEqualizerPanel\(/);
  assert.match(musicSrc, /function isEqualizerPanelOpen\(/);
});
...
test('persists a versioned default-off equalizer state', () => {
  assert.match(musicSrc, /stellaflix-equalizer-state-v1/);
  assert.match(musicSrc, /MineradioEqualizer\.normalizeState/);
  assert.match(musicSrc, /localStorage\.setItem\(EQUALIZER_STORE_KEY/);
  assert.match(musicSrc, /equalizerState\.enabled/);
});
...
test('uses the corrected equalizer labels and responsive overflow contract', () => {
  [
    ...
    '重置为原声',
    '当前环境不支持均衡器',
  ].forEach((label) => assert.ok(indexHtml.includes(label) || musicSrc.includes(label), 'missing corrected label: ' + label));
  assert.match(musicSrc, /formatEqualizerFrequency\(frequency\) \+ ' Hz 增益'/);
...
function extractSource(startMarker, endMarker, source = indexHtml) {
...
function createEqualizerUiHarness(options = {}) {
  const key = 'stellaflix-equalizer-state-v1';
  ...
  vm.createContext(sandbox);
  // 2.4：equalizer 状态块与 UI 块均在 glue；用 glue 内相邻函数作截取边界。
  const stateSource = extractSource('var EQUALIZER_STORE_KEY', 'function formatEqualizerFrequency(', glueSrc);
  const uiSource = extractSource('function formatEqualizerFrequency(', 'function setEqualizerAudioParam(', glueSrc);
  vm.runInContext(`
...
test('does not auto-hide bottom controls while the equalizer panel is open', () => {
  const source = extractSource('function scheduleControlsHide(', 'function revealBottomControls(', musicSrc);
