...
test('custom-first mode resolves despite rights restrictions and retries alternate lx sources', async () => {
  const sources = {
    wy: { actions: ['musicUrl'], qualitys: ['128k'] },
    kw: { actions: ['musicUrl'], qualitys: ['128k'] },
  };
  const store = createStore([{ id: 'a', name: 'A', sources }], 'a');
  const requested = [];
  const runtime = {
    ...
    request: async payload => {
      requested.push(payload.source);
      if (payload.source === 'wy') throw new Error('wy offline');
      return 'https://audio.example.com/kw.mp3';
    },
    stop() {},
  };
  const manager = new CustomSourceManager({ store, runtimeFactory: () => runtime, validateResolvedUrl: async url => url });
  await manager.startActive();

  // custom-first 模式下 vip_required 也尝试第三方解析,自身线路失败后换 kw 线路命中。
  const resolved = await manager.resolveFallback({
    song: { provider: 'netease', id: 1 },
    quality: 'standard',
    mode: 'custom-first',
    officialResult: { url: '', reason: 'vip_required' },
  });
  assert.equal(resolved.url, 'https://audio.example.com/kw.mp3');
  assert.equal(resolved.source, 'kw');
  assert.deepEqual(requested, ['wy', 'kw']);
  assert.equal(resolved.alternatesTried.length, 1);

  // official-first(缺省)维持旧行为:vip_required 不触发第三方解析。
  const legacyDenied = await manager.resolveFallback({
    song: { provider: 'netease', id: 1 },
    quality: 'standard',
    officialResult: { url: '', reason: 'vip_required' },
  });
  assert.equal(legacyDenied.attempted, false);
});
