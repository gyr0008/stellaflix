...
- 播放器会为提升的频段自动预留响度余量并进行峰值保护；均衡器默认关闭，不修改歌曲文件，也不替代平台音质或官方音效。
- 软件内歌词默认显示下一句预览，方便跟唱；可在歌词设置中关闭。
- 新增落雪自定义音源 API 2.0.0 兼容层，用户可自行导入可信脚本作为官方播放地址发生技术故障时的可选回退。
- 网易云、酷狗概念版、普通酷狗音乐和汽水导入歌单的单歌单读取上限提高到 5000 首，并修复主页歌单数量与 3D 歌单架排序问题。

### 十段均衡器

- 新增独立十段均衡器，提供六种预设、一条自定义曲线、本地保存和重置为原声。
- 均衡器默认关闭，开启后作用于所有播放来源；它不会修改歌曲文件，也不会改变平台音质或官方音效。
- 提升频段时自动预留响度余量并进行峰值保护，节拍与粒子分析继续使用均衡前的原始信号。

## main / 2026-07-11 落雪自定义音源兼容层

- 新增落雪自定义音源 API 2.0.0 兼容层，支持导入、验证、启停、替换和删除本机 `.js` 脚本；同一时间只启用一个脚本。
- 播放策略保持官方接口优先，只有网络、地址失效和格式兼容等技术故障才尝试第三方解析；登录、会员、购买、试听和版权限制不会触发。
- 网易云、QQ、酷狗概念版和普通酷狗分别映射到落雪 `wy`、`tx`、`kg` 平台键；汽水音乐暂无标准映射。
- 第三方脚本运行在隐藏 Electron 沙箱中，不接触账号 Cookie、本地文件或 Node.js；网络代理拒绝本机、局域网、保留地址、危险协议和不安全重定向。
- 第三方音频通过短期本机票据代理播放，远程地址不会直接交给原有通用音频代理。
- 本仓库不内置、推荐、下载或自动更新任何第三方音源脚本。

## main / 2026-07-10 大歌单与 3D 歌单架顺序修复

- 网易云、酷狗概念版、酷狗音乐和汽水导入歌单的单歌单读取上限统一提高到 5000 首；列表继续采用首屏与滚动分批渲染，避免大歌单一次性生成全部界面节点。
- 3D 歌单架详情与左侧歌单详情共用同一套排序规则，修复酷狗概念版按收藏时间查看时两处歌曲顺序完全相反的问题。
- 主页“我的歌单”改为读取本人各平台歌单，优先显示歌曲最多的“我喜欢/喜欢的音乐”，不再误用网易云推荐歌单的歌曲数。

## main / 2026-07-05 公开版文案与 DIY 边界整理

- 移除二创过程中的个人实验形态自动注入逻辑，公开版不再内置个人试玩形态。
- 保留通用 DIY 形态工坊能力：用户仍可用基础粒子创建自己的 3D 形态，保存到“我的形态”，并通过 `.mineradio-shape.json` 导入/导出。
- 基础粒子库新增通用“曲线环”，用于用户自己搭建双环、空间曲线等形态；公开版仍不内置二创作者个人实验形态。
- 自定义形态的封面粒子化、拖动旋转、静态编辑、律动预览、撤销/重做、漂移锁定和一键稳定作为通用能力继续保留。
- DIY 形态工坊新增“底片落点”提示和层级材质标签，明确当前歌曲封面或上传图片会落在哪些面/卡片层；没有面/卡片层时提示自动后置封面粒子底板，降低用户自定义形态时的判断成本。
- README 重写为面向普通用户的 GitHub 首页说明，补齐下载入口、扩展功能、使用边界和支持渠道。
- README 与支持页新增微信群聊、抖音群聊入口，欢迎感兴趣的朋友加群交流使用体验和二创想法。
- 新增 `docs/USAGE_GUIDE.md`，把下载、登录、搜索、酷狗/汽水使用、歌单排序、收藏写入、Home 海报和 DIY 形态工坊整理成步骤化教程。
- 更新形态验证脚本，禁止个人实验形态标记重新进入公开页面代码。

## main / 2026-07-04 扩展版预览更新

- README 和支持页新增“二创作者支持渠道”，并保留“原作者支持渠道”，避免用户把两个支持入口混淆。
- 酷狗概念版和普通酷狗音乐的音质菜单对齐为 `Hi-Res音质`、`无损音质`、`高品音质`、`标准音质` 四档；旧的通用 `超清母带` 档不再在酷狗系歌曲上重复显示。
...
- DIY 视觉扩展第一阶段落地：视觉控制台新增“基础积木”，支持从素材、形态、粒子层、律动和镜头组合效果；“用户存档”升级为“我的作品”，支持复制、导入和 `.mineradio-visual.json` 导出。
- DIY 形态工坊开始接入：形态预设区新增“创建形态”入口，支持用点、线、环、曲线环、面、螺旋、波、隧道、球面、星尘、卡片等基础粒子编辑 `shapeRecipe` 并保存为本地“我的形态”；新增空白创作画布，点击基础粒子会生成可拖动图层，基础结构包会追加到当前形态而不是覆盖，拖动位置会写回形态配方；自定义形态已接入主 Three.js 场景，可作为真实 3D 点云预设应用到播放器；“我的形态”支持编辑更新、另存、应用、复制、两步确认删除，以及 `.mineradio-shape.json` 导入/导出；保存后的自定义形态会直接出现在形态预设网格里，点击即可像内置预设一样应用。
