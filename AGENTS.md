...
**已解决：GSAP 授权冲突（GreenSock 专有依赖已移除）。** 原 `public/vendor/gsap.min.js`（GreenSock 专有授权，非 OSI 开源，与 GPLv3 §7 冲突）**已从仓库物理删除**；`index.html` 改用项目自研的 MIT 垫片 `public/vendor/gsap-waapi-shim.js` 提供 `window.gsap`（覆盖 6 个基础 API / 零插件，业务 `window.gsap.*` 调用点零改动）。GPLv3 分发义务已消除，不再构成发布阻塞。详见 `NOTICE.md` 的 GSAP 授权说明。
