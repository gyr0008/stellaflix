 ...
 *
 * B1. 图片搜索 handler 已从 online-search.js 彻底移除（负向断言，防死代码回归）
 * B2. .sfv-search-close click 仍触发 closeSearchPage（关闭按钮工作，正向断言）
 */
...
// ============================================================
// B. 行为测试：图片搜索已删除 + 关闭按钮仍工作
// ============================================================
console.log('--- B. 行为：图片搜索移除 + 关闭按钮 ---');

// 图片搜索 feature 已于 T116 删除 DOM 按钮，其死代码 handler 于 2026-08-15 从 online-search.js 移除。
// 此处确认已从搜索源码彻底移除（避免死代码回归），而非断言其存在。
var onlineSrc = fs.readFileSync(path.join(root, 'public', 'video', 'online-search.js'), 'utf8');
assert(/sfv-search-image-search/.test(onlineSrc) === false,
  'B1.1 图片搜索 handler 已从 online-search.js 移除（无 .sfv-search-image-search）');
assert(/图片搜索功能即将上线/.test(onlineSrc) === false,
  'B1.2 图片搜索 toast 文案已从 online-search.js 移除（无「图片搜索功能即将上线」）');

// 关闭按钮仍然工作（handler 仍存在）
var closeBtnBlock = onlineSrc.match(/closest\(\s*['"]\.sfv-search-close['"]\s*\)/);
assert(closeBtnBlock !== null, 'B4 online-search.js 检测 .sfv-search-close 类（仍触发关闭）');
