...
const CORE = path.join(__dirname, '..', 'public', 'video', 'source-adapter-core.js');
const coreCode = fs.readFileSync(CORE, 'utf8');
...
function makeSandbox(hlsNative) {
  ...
  vm.runInContext(coreCode, sandbox, { filename: 'source-adapter-core.js' });
...
// 合规口径：仅扫描「代码」中的硬编码 http(s) 片源地址，忽略文档注释里的示例 URL
// （如 sanitizeHlsUrl 注释中的 https://cdn.example.com/... 不是片源，不应误报）。
function stripComments(c) {
  return c
    .replace(/\/\*[\s\S]*?\*\//g, '')            // 块注释
    .replace(/(^|[^:])\/\/.*$/gm, '$1');         // 行注释（保留 https:// 中 : 后的 //）
}
const codeNoComments = stripComments(code);
const urlLiterals = codeNoComments.match(/https?:\/\/[^\s'"`)]+/g) || [];
assert(urlLiterals.length === 0,
  '源码不含任何硬编码 http(s) 片源地址' + (urlLiterals.length ? '（命中: ' + urlLiterals.join(', ') + '）' : ''));
