...
// T-#6-A 把「纯函数/存储/缓存」拆分为 sources-core.js，sources.js 仅在其上叠加 IO 层。
// 二者必须按序加载：core 先建立 SFV.sources.__core facade，sources.js 才能叠加。
const sourcesCoreSrc = read('sources-core.js');
...
function loadSources(ctx) {
  vm.runInContext(sourcesCoreSrc, ctx.sandbox, { filename: 'sources-core.js' });
...
function runStaticTests() {
  ...
  // core（KEY_SOURCES/readJSON）、io（sources.js）、fx 三层都纳入硬编码地址扫描
  const codeOnly = stripComments(sourcesCoreSrc) + '\n'
    + stripComments(sourcesSrc) + '\n' + stripComments(fxSrc);
  ...
  assert(/KEY_SOURCES\s*=\s*NS\s*\+\s*'sources'/.test(sourcesCoreSrc), '片源存 stellaflix-video-sources');
  assert(/readJSON\(KEY_SOURCES,\s*\[\]\)/.test(sourcesCoreSrc), '读取片源时默认空数组（出厂无源）');
  ...
  const loaderUrls = loaderMatch ? (loaderMatch[1].match(/"([^"]+)"/g) || []).map(s => s.replace(/^"|"$/g, '')) : [];
  assert(loaderUrls.indexOf('video/sources.js') > 0, 'index.html(SFV_SCRIPTS) 引入 sources.js');
  assert(loaderUrls.indexOf('video/fx-sources.js') > 0, 'index.html(SFV_SCRIPTS) 引入 fx-sources.js');
