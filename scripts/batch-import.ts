/**
 * 批量导入脚本 - 从多个CMS源导入视频数据
 *
 * 使用方法：
 * npx tsx scripts/batch-import.ts [起始页] [结束页] [源编号]
 *
 * 示例：
 * npx tsx scripts/batch-import.ts 1 100      // 导入所有源的第1-100页
 * npx tsx scripts/batch-import.ts 1 100 1    // 只导入源1（暴风影视）
 * npx tsx scripts/batch-import.ts 1 100 3    // 只导入源3（非凡资源）
 */
...
// 6. 地区过滤（跳过这些地区的内容）
const SKIP_REGIONS = ["台湾", "泰国", "大陆", "中国大陆", "内地"];
  ...
  try {
    ...
    for (const vod of data.list) {
      try {
        ...
        // 6. 韩国内容过滤（保留经典高分电影）
        if (vodArea === "韩国") {
          const isKoreanClassic = KOREAN经典MOVIES.some(name => vodName.includes(name));
          if (!isKoreanClassic) {
            skipped++;
            continue;
          }
  ...
  for (const source of sourcesToImport) {
    ...
    for (let page = startPage; page <= endPage; page++) {
      ...
      process.stdout.write(
        `\r📊 ${source.name} | 进度: ${progress}% | 页: ${page}/${endPage} | 导入: ${sourceImported} | 跳过: ${sourceSkipped} | 错误: ${sourceErrors} | ${elapsed}s`
      );
