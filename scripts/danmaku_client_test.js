...
sandbox.fetch = function (url, opts) {
  ...
  const r = fetchResponder ? fetchResponder() : { ok: true, status: 200, json: { success: true, animes: [], comments: [] } };
...
(async function run() {
  ...
  // DanDanPlay /search/episodes 返回 animes 数组，每项含 episodes
  fetchResponder = function () { return { ok: true, status: 200, json: { success: true, animes: [{ animeId: 999, animeTitle: '进击的巨人', episodes: [{ episodeId: 123450001, episodeTitle: '第1话' }] }] } }; };
  ...
  check('代理目标含 /api/v2/search/episodes', decodeURIComponent(lastFetch.url).indexOf('/api/v2/search/episodes') > -1);
  ...
  fetchResponder = function () { return { ok: true, status: 200, json: { success: true, animes: [{ animeId: 888, animeTitle: '测试番', episodes: [{ episodeId: 555, episodeTitle: '全集' }] }] } }; };
