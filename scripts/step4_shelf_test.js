...
function testCurrentItemsVideoMode() {
  ...
  if (SM) {
    var flagMap = [
      { flag:'liked',    title:'心动',  tag:'心动' },
      { flag:'playlist', title:'片单',  tag:'片单' },
      { flag:'faved',    title:'收藏',  tag:'收藏' },
      { flag:'history',  title:'历史',  tag:'历史' },
    ];
    flagMap.forEach(function(entry){
      ...
      if (entry.flag === 'history') {
        vItems.push({ type:'videoCategory', flag:entry.flag, title:entry.title, sub: count ? (count + ' 条观看记录') : '还没有观看记录', tag:entry.tag, cover:'', videoCount:count });
      } else {
        vItems.push({ type:'videoCategory', flag:entry.flag, title:entry.title, sub: count ? (count + ' 部影片') : '暂无内容', tag:entry.tag, cover:'', videoCount:count });
      }
    });
  }
  return vItems;
...
assert(vidItems[3].sub.indexOf('观看记录') >= 0, '历史副标题含"观看记录"');
...
assert(vidItems2[0].sub.indexOf('1 部影片') >= 0, '心动副标题显示"1 部影片"');
...
var vcItem = { type:'videoCategory', flag:'liked', title:'心动', sub:'1 部影片', tag:'心动', cover:'' };
