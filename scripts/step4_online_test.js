...
// ================================================================ B. 进度键透传
section('B. source-adapter 进度键透传');
{
  const { sandbox } = makeSandbox();
  vm.runInContext(read('model.js'), sandbox, { filename: 'model.js' });
  ...
  const opened = [];
  // 用假 player 捕获 openUrl 的 opts.id
  SFV.player = {
    openUrl: (url, opts) => { opened.push({ url, opts }); return true; },
    openFile: () => true,
  };
  const r = SFV.source.open({ url: 'https://v.test/1.m3u8', title: '片A·第1集', id: 's1:7:0' });
  assert(r === true, 'source.open 返回成功');
  ...
  // 无显式 id 时回落 url: 原始地址
  opened.length = 0;
  SFV.source.open('https://v.test/2.mp4');
  assert(opened[0].opts.id === 'url:https://v.test/2.mp4', '无 id 时回落 url:原始地址');
}
...
(async function onlineTests() {
  ...
  SFV.sources = {
    ...
    detail: (s, vodId) => Promise.resolve({
      ...
      plays: [{ from: '线路A', episodes: [{ name: '第1集', url: 'https://v.test/1.m3u8', index: 0 }] }],
    }),
  };
