...
const setModeCalls = [];
g.shelfManager = { setMode: function (m) { setModeCalls.push(m); } };
g.fx = { shelf: 'side' };
...
ok(setModeCalls.length === 1 && setModeCalls[0] === 'off', "进影视态调用 shelfManager.setMode('off')");
...
setModeCalls.length = 0;
scListeners.forEach(fn => fn({ detail: { spaceMode: 'video' } }));
ok(setModeCalls[0] === 'off', "再次进影视态仍调用 setMode('off')");
