 ...
 *
 * 重点验证三条高风险设计：
 *   1) 影视态改写四卡文案（心动/片单/收藏/历史），音乐态还原（交还 renderHomeDiscover）
 *   2) 捕获阶段拦截卡片内联 onclick（音乐态不得干预）
 *   3) 海报墙渲染 + XSS 转义 + 空库提示
 *
 * 注：本测试对应「铁律四卡」设计（非被驳回的本地入口六卡）。
 */
'use strict';
const fs = require('fs');
...
function makeSandbox() {
  ...
  ['state.js', 'dispatch.js', 'model.js', 'source-adapter-core.js', 'source-adapter.js', 'home-core.js'].forEach(f => {
...
console.log('=== Step 3c: 影视态首页逻辑测试（四卡：心动/片单/收藏/历史）===\n');

const sb = makeSandbox();
const SFV = sb.StellaflixVideo;
const dom = sb._dom;
// 提供 online 桩，验证卡片点击正确路由到分类浏览
SFV.online = {
  openCategory(flag) { sb._calls.push('openCategory:' + flag); },
  openBrowse(flag) { sb._calls.push('openBrowse:' + flag); },
  toast(msg) { sb._calls.push('toast:' + msg); },
};
...
assert(dom.cards[0].querySelector('.home-card-title').textContent === '心动', '第 1 卡 → 心动');
assert(dom.cards[1].querySelector('.home-card-title').textContent === '片单', '第 2 卡 → 片单');
assert(dom.cards[2].querySelector('.home-card-title').textContent === '收藏', '第 3 卡 → 收藏');
assert(dom.cards[3].querySelector('.home-card-title').textContent === '历史', '第 4 卡 → 历史');
assert(dom.cards[0].querySelector('.home-card-sub').textContent === '在详情页点亮 ♥ 加入', '心动空态副文案');
assert(dom.cards[4].style.display === 'none' && dom.cards[5].style.display === 'none', '多余卡位(5/6)在影视态隐藏');
assert(dom.posterTitle.textContent === '我的影视空间', 'Hero 标题切换为影视');
assert(/不存储任何视频资源/.test(dom.posterQuote.textContent), 'Hero 副文案含合规声明（不存储资源）');
...
assert(dom.railNote.textContent === '还没有影片，先从「打开本地影片」开始', '空库 rail 提示正确');

console.log('\n[捕获拦截 → 路由到分类浏览]');
let r = fireCapture(dom.grid, dom.cards[0]);
assert(r.stopped === true && r.prevented === true, '影视态点击被 stopPropagation+preventDefault 拦截');
assert(sb._calls.indexOf('openCategory:liked') >= 0, '第 1 卡(心动) → openCategory("liked")');
sb._calls.length = 0;
fireCapture(dom.grid, dom.cards[1]);
assert(sb._calls.indexOf('openCategory:inList') >= 0, '第 2 卡(片单) → openCategory("inList")');
sb._calls.length = 0;
fireCapture(dom.grid, dom.cards[2]);
assert(sb._calls.indexOf('openCategory:faved') >= 0, '第 3 卡(收藏) → openCategory("faved")');
sb._calls.length = 0;
fireCapture(dom.grid, dom.cards[3]);
assert(sb._calls.indexOf('openCategory:history') >= 0, '第 4 卡(历史) → openCategory("history")');
// 点击卡片内部子元素也应命中（事件冒泡起点是子节点）
sb._calls.length = 0;
fireCapture(dom.grid, dom.cards[0].querySelector('.home-card-title'));
assert(sb._calls.indexOf('openCategory:liked') >= 0, '点击卡内子元素同样命中该卡');

console.log('\n[片库有内容 · 三类旗 + 历史计数]');
const vodKey = 'url:https://cdn.example.com/a.mp4';
SFV.model.addToLibrary({ id: vodKey, title: '示例影片', source: 'url' });
SFV.model.setFlag(vodKey, 'liked', true);
SFV.model.setFlag(vodKey, 'inList', true);
SFV.model.setFlag(vodKey, 'faved', true);
SFV.model.addHistory({ key: vodKey, title: '示例影片' });
SFV.model.setProgress(vodKey, 300, 1200);
SFV.home.render();
assert(dom.cards[0].querySelector('.home-card-sub').textContent === '已心动 1 部', '心动计数正确');
assert(dom.cards[1].querySelector('.home-card-sub').textContent === '片单 1 部', '片单计数正确');
assert(dom.cards[2].querySelector('.home-card-sub').textContent === '已收藏 1 部', '收藏计数正确');
assert(dom.cards[3].querySelector('.home-card-sub').textContent === '最近观看 1 部', '历史计数正确');
...
tile.setAttribute('data-sfv-id', vodKey);
