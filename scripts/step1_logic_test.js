...
// server.js 只用到 listen / setCustomSourceBridge；桩掉即可，不真起服务。
const fakeServer = { listen: () => {}, setCustomSourceBridge: () => {}, on: () => {}, close: () => {} };

// 关键：server.js 顶层是 `const http = require('http')`，会覆盖 sandbox.http。
// 因此必须在 require 层拦截 http.createServer，否则会真的 listen(3000) 触发 EADDRINUSE。
const httpStub = Object.assign({}, require('http'), { createServer: () => fakeServer });

function stubRequire(name) {
  if (name === 'http' || name === 'node:http') return httpStub;
