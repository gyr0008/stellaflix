...
class WallpaperEngineRuntime {
  ...
  emitStatus(status) {
    try {
      const payload = (status && typeof status === 'object') ? status : this.getStatus();
      this.onStatus(payload);
    } catch (_) { /* onStatus 由调用方兜底，不向上抛 */ }
  }
  ...
  async start(id, options = {}) {
    ...
    this.pending = session;
    this.emitStatus({ ...this.getStatus(), pending: true, phase: 'starting' });
    ...
    try {
      ...
      this.emitStatus(this._publicSession(session));
      return this._publicSession(session);
    } catch (error) {
