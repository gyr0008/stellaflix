...
const { WallpaperMediaTranscoder } = require('./wallpaper-media-transcoder');

const WALLPAPER_ENGINE_SCHEME = 'stellaflix-wallpaper-engine';
...
class WallpaperEngineLibrary {
  constructor(options = {}) {
    ...
    this.disposed = false;
    this.generation = 0;
    // 壁纸媒体兼容转码器：仅 media 分支按需把非 H.264 视频转码；
    // ffmpeg 缺失/失败静默回退原文件，不影响取流。
    this.mediaTranscoder = new WallpaperMediaTranscoder({
      appDir: __dirname,
      cacheDir: options.wallpaperCacheDir || path.join(os.tmpdir(), 'stellaflix-wallpaper-cache'),
  ...
  async performScan() {
    ...
    const snapshot = {
      ...
      transcoder: this.mediaTranscoder ? this.mediaTranscoder.capabilities() : { ffmpeg: false, h264Encoders: [], outputFormat: '', inputFormats: [] },
    };
  ...
  async mediaResponse(request) {
    ...
    if (method !== 'GET' && method !== 'HEAD') {
      return { status: 405, headers: { 'Allow': 'GET, HEAD', 'X-Content-Type-Options': 'nosniff' }, body: Buffer.from('Method not allowed') };
    }
    if (!this.snapshot && !this.scanPromise) await this.list({ force: false });
    let url;
    let id;
    try {
      ...
      console.info('[WallpaperEngineLibrary]', 'mediaResponse', request.url, 'hostname=' + url.hostname, 'id=' + id);
    } catch (_) {
      return { status: 404, headers: { 'X-Content-Type-Options': 'nosniff' }, body: Buffer.from('Not found') };
    }
    const kind = url.hostname === 'media' ? 'media' : (url.hostname === 'preview' ? 'preview' : '');
    if (url.searchParams.get('token') !== this.mediaToken) {
      return { status: 404, headers: { 'X-Content-Type-Options': 'nosniff' }, body: Buffer.from('Not found') };
    }
    if (!/^[a-f0-9]{24}$/i.test(id)) return { status: 404, headers: { 'X-Content-Type-Options': 'nosniff' }, body: Buffer.from('Not found') };
    id = id.toLowerCase();
    const record = kind && this.index.get(id);
    if (!record) return { status: 404, headers: { 'X-Content-Type-Options': 'nosniff' }, body: Buffer.from('Not found') };
    const target = await this.validatedRecordFile(record, kind);
    if (!target) return { status: 404, headers: { 'X-Content-Type-Options': 'nosniff' }, body: Buffer.from('Not found') };
    // 仅 media 走 H.264/yuv420p 兼容转码；ffmpeg 缺失/失败静默回退原文件。
    const servedTarget = (kind === 'media')
      ? await this.compatibleWallpaperMedia(target)
      : target;
    let stat;
    try { stat = await fs.promises.stat(servedTarget); } catch (_) {
      return { status: 404, headers: { 'X-Content-Type-Options': 'nosniff' }, body: Buffer.from('Not found') };
    }
    const size = Number(stat.size) || 0;
    const rangeHeader = request.headers && request.headers.range ? String(request.headers.range) : '';
    const range = rangeHeader ? parseByteRange(rangeHeader, size) : null;
    if (range && range.invalid) {
      return { status: 416, headers: { 'Content-Range': `bytes */${size}`, 'X-Content-Type-Options': 'nosniff' }, body: Buffer.alloc(0) };
    }
    const start = range ? range.start : 0;
    const end = range ? range.end : Math.max(0, size - 1);
    const headers = {
      'Content-Type': mimeForPath(servedTarget),
    ...
    if (method === 'HEAD' || !size) {
      return { status: range ? 206 : 200, headers, body: Buffer.alloc(0) };
    }
    const stream = fs.createReadStream(servedTarget, { start, end });
    return { status: range ? 206 : 200, headers, stream };
  }

  async compatibleWallpaperMedia(filePath) {
    if (!this.mediaTranscoder) return filePath;
    try {
      return await this.mediaTranscoder.compatibleMediaFile(filePath, 0);
    } catch (_) {
      return filePath;
    }
  }

  // v24 兼容：用 registerStreamProtocol 替代 protocol.handle（v25+ 才支持 Web Response）。
  // 返回 false 表示当前环境不支持（调用方应降级为跳过）。
  installProtocol(protocol) {
    if (this.protocolInstalled) return true;
    if (!protocol || typeof protocol.registerStreamProtocol !== 'function') {
      console.warn('[WallpaperEngineLibrary]', 'registerStreamProtocol unavailable');
      return false;
    }
    const self = this;
    console.info('[WallpaperEngineLibrary]', 'registering protocol', WALLPAPER_ENGINE_SCHEME);
    protocol.registerStreamProtocol(WALLPAPER_ENGINE_SCHEME, (request, callback) => {
      Promise.resolve().then(() => self.mediaResponse(request)).then((res) => {
        const statusCode = res.status;
        const headers = res.headers || {};
        if (res.stream) {
          callback({ statusCode, headers, data: res.stream });
        } else {
          callback({ statusCode, headers, data: res.body || Buffer.alloc(0) });
        }
      }).catch(() => {
        callback({ statusCode: 404, headers: { 'X-Content-Type-Options': 'nosniff' }, data: Buffer.alloc(0) });
      });
    });
